...
public class XincoAddAttributeServer extends XincoAddAttribute {
  ...
  public XincoAddAttributeServer(int attrID1, int attrID2, XincoDBManager DBM)
      throws XincoException {
    try {
      Statement stmt = DBM.con.createStatement();
      ResultSet rs =
          stmt.executeQuery(
              "SELECT * FROM xinco_add_attribute "
    ...
    try {
      ...
      if (getAttrib_datetime() != null) {
        ...
        cal.add(
            ...
            (ngc.get(Calendar.ZONE_OFFSET) - cal.get(Calendar.ZONE_OFFSET))
      ...
      stmt.executeUpdate(
          "INSERT INTO xinco_add_attribute VALUES ("
  ...
  public static Vector getXincoAddAttributes(int attrID, XincoDBManager DBM) {
    ...
    try {
      ...
      ResultSet rs =
          stmt.executeQuery(
              "SELECT * FROM xinco_add_attribute WHERE xinco_core_data_id ="
                  + attrID
      ...
      while (rs.next()) {
        ...
        addAttributes.addElement(
            new XincoAddAttributeServer(
                rs.getInt("xinco_core_data_id"),
                rs.getInt("attribute_id"),
                rs.getInt("attrib_int"),
                rs.getLong("attrib_unsignedint"),
                rs.getDouble("attrib_double"),
                rs.getString("attrib_varchar"),
                rs.getString("attrib_text"),
                cal));
      }
