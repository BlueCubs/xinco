...
public class XincoArchiveThread extends Thread {
  ...
  public static synchronized boolean archiveData(XincoDBManager DBM) {
    ...
    try {
      ...
      query[querycount] =
          "SELECT DISTINCT xcd.id FROM xinco_core_data xcd, xinco_add_attribute xaa1,"
              + " xinco_add_attribute xaa2 WHERE xcd.xinco_core_data_type_id = 1 AND"
              + " xcd.status_number <> 3 AND xcd.id = xaa1.xinco_core_data_id AND xcd.id ="
              + " xaa2.xinco_core_data_id AND xaa1.attribute_id = 5 AND xaa1.attrib_unsignedint = 1"
              + " AND xaa2.attribute_id = 6 AND xaa2.attrib_datetime < now() ORDER BY xcd.id";
      querycount++;

      query[querycount] =
          "SELECT DISTINCT xcd.id FROM xinco_core_data xcd, xinco_add_attribute xaa1,"
              + " xinco_add_attribute xaa2, xinco_core_log xcl WHERE xcd.xinco_core_data_type_id ="
              + " 1 AND xcd.status_number <> 3 AND xcd.id = xaa1.xinco_core_data_id AND xcd.id ="
              + " xaa2.xinco_core_data_id AND xcd.id = xcl.xinco_core_data_id AND xaa1.attribute_id"
              + " = 5 AND xaa1.attrib_unsignedint = 2 AND xaa2.attribute_id = 7 AND"
              + " ADDDATE(DATE(xcl.op_datetime), xaa2.attrib_unsignedint) < now() ORDER BY xcd.id";
      querycount++;
      ...
      for (j = 0; j < querycount; j++) {
        ...
        while (rs.next()) {
          ...
          XincoArchiver.archiveData(
              xdata_temp,
              XincoCoreNodeServer.getXincoCoreNodeParents(xdata_temp.getXinco_core_node_id(), DBM),
              DBM);
          sleep(10000);
        }
        stmt.close();
      ...
      Logger.getLogger(XincoArchiveThread.class.getSimpleName()).log(Level.SEVERE, null, e);
      try {
        if (fcis != null) {
          fcis.close();
        }
