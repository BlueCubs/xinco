...
/** This class handles the server configuration of xinco. Edit values in context.xml */
public class XincoConfigSingletonServer {
  ...
  private XincoConfigSingletonServer() {
    try {
      FileRepositoryPath =
          (String) (new InitialContext()).lookup("java:comp/env/xinco/FileRepositoryPath");
    } catch (NamingException ex) {
      FileRepositoryPath = "";
      Logger.getLogger(XincoConfigSingletonServer.class.getName()).log(Level.SEVERE, null, ex);
    }
    if (!(FileRepositoryPath.substring(FileRepositoryPath.length() - 1)
    ...
    if (!(FileIndexPath.substring(FileIndexPath.length() - 1)
        .equals(System.getProperty("file.separator")))) {
      FileIndexPath = FileIndexPath + System.getProperty("file.separator");
    }
    try {
      FileArchivePath =
          (String) (new InitialContext()).lookup("java:comp/env/xinco/FileArchivePath");
    } catch (NamingException ex) {
      FileArchivePath = "";
      Logger.getLogger(XincoConfigSingletonServer.class.getName()).log(Level.SEVERE, null, ex);
    }
    if (!(FileArchivePath.substring(FileArchivePath.length() - 1)
        .equals(System.getProperty("file.separator")))) {
      FileArchivePath = FileArchivePath + System.getProperty("file.separator");
    }
    try {
      FileArchivePeriod =
          ((Long) (new InitialContext()).lookup("java:comp/env/xinco/FileArchivePeriod"));
    } catch (NamingException ex) {
    ...
    try {
      FileIndexOptimizerPeriod =
          ((Long) (new InitialContext()).lookup("java:comp/env/xinco/FileIndexOptimizerPeriod"));
    } catch (NamingException ex) {
    ...
    try {
      FileIndexerCount =
          ((Integer) (new InitialContext()).lookup("java:comp/env/xinco/FileIndexerCount"));
    } catch (NamingException ex) {
      FileIndexerCount = 4;
      Logger.getLogger(XincoConfigSingletonServer.class.getName()).log(Level.SEVERE, null, ex);
    }
    ...
    for (int i = 0; i < FileIndexerCount; i++) {
      try {
        IndexFileTypesClass.add(
            (String)
                (new InitialContext())
                    .lookup("java:comp/env/xinco/FileIndexer_" + (i + 1) + "_Class"));
      } catch (NamingException ex) {
      ...
      try {
        IndexFileTypesExt.add(
            ((String)
                    (new InitialContext())
                        .lookup("java:comp/env/xinco/FileIndexer_" + (i + 1) + "_Ext"))
                .split(";"));
      } catch (NamingException ex) {
        String[] tsa = null;
        tsa = new String[1];
    ...
    try {
      IndexNoIndex =
          ((String) (new InitialContext()).lookup("java:comp/env/xinco/IndexNoIndex")).split(";");
    } catch (NamingException ex) {
    ...
    try {
      allowOutsideLinks =
          ((String) (new InitialContext()).lookup("java:comp/env/setting.allowoutsidelinks"))
              .equals("True");
    } catch (NamingException ex) {
    ...
    try {
      guessLanguage =
          ((String) (new InitialContext()).lookup("java:comp/env/setting.guessLanguage"))
              .equals("True");
    } catch (NamingException ex) {
      guessLanguage = false;
      Logger.getLogger(XincoConfigSingletonServer.class.getName()).log(Level.SEVERE, null, ex);
    ...
    try {
      allowPublisherList =
          ((String) (new InitialContext()).lookup("java:comp/env/setting.allowpublisherlist"))
              .equals("True");
    } catch (NamingException ex) {
    ...
    try {
      MaxSearchResult =
          ((Integer) (new InitialContext()).lookup("java:comp/env/xinco/MaxSearchResult"));
    } catch (NamingException ex) {
      MaxSearchResult = 30;
      Logger.getLogger(XincoConfigSingletonServer.class.getName()).log(Level.SEVERE, null, ex);
    }
