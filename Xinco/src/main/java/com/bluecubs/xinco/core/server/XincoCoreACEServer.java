 ...
 *
 * <p>Name: XincoCoreACEServer
 *
 * <p>Description: access control entry
 *
...
public class XincoCoreACEServer extends XincoCoreACE {
    ...
    try {
      ...
      if (getId() > 0) {
        ...
        audit.updateAuditTrail(
            ...
            this.getChangerID());
        stmt.executeUpdate(
            "UPDATE xinco_core_ace SET xinco_core_user_id="
                + xcuid
                + ", xinco_core_group_id="
                + xcgid
                + ", xinco_core_node_id="
        ...
        Statement stmt = DBM.con.createStatement();
        stmt.executeUpdate(
            "INSERT INTO xinco_core_ace VALUES ("
                + getId()
                + ", "
                + xcuid
                + ", "
  ...
  // remove from db
  public static int removeFromDB(XincoCoreACE attrCACE, XincoDBManager DBM, int userID)
      throws XincoException {
  ...
  public static Vector getXincoCoreACL(int attrID, String attrT, XincoDBManager DBM) {
    ...
    try {
      ...
      ResultSet rs =
          stmt.executeQuery(
              "SELECT * FROM xinco_core_ace WHERE "
                  + attrT
                  + "="
                  + attrID
                  + " ORDER BY xinco_core_user_id, xinco_core_group_id, xinco_core_node_id,"
                  + " xinco_core_data_id");
      ...
      while (rs.next()) {
        core_acl.addElement(
            new XincoCoreACEServer(
                rs.getInt("id"),
                rs.getInt("xinco_core_user_id"),
                rs.getInt("xinco_core_group_id"),
                rs.getInt("xinco_core_node_id"),
                rs.getInt("xinco_core_data_id"),
                rs.getBoolean("read_permission"),
                rs.getBoolean("write_permission"),
                rs.getBoolean("execute_permission"),
                rs.getBoolean("admin_permission")));
      }
    ...
    for (i = 0; i < attrACL.size(); i++) {
      ...
      if (match_ace) {
        ...
        if (!core_ace.isExecute_permission()) {
          core_ace.setExecute_permission(
              ((XincoCoreACE) attrACL.elementAt(i)).isExecute_permission());
        }
