...
public class XincoCoreDataTypeAttributeServer extends XincoCoreDataTypeAttribute {
  ...
  public XincoCoreDataTypeAttributeServer(int attrID1, int attrID2, XincoDBManager DBM)
    ...
    try {
      ...
      ResultSet rs =
          stmt.executeQuery(
              "SELECT * FROM xinco_core_data_type_attribute WHERE xinco_core_data_type_id="
                  + attrID1
                  + " AND attribute_id="
                  + attrID2);
  ...
  public int write2DB(XincoDBManager DBM) throws XincoException {
    ...
    try {
      ...
      stmt.executeUpdate(
          "INSERT INTO xinco_core_data_type_attribute VALUES ("
      ...
      stmt = DBM.con.createStatement();
      stmt.executeUpdate(
          "INSERT INTO xinco_add_attribute SELECT id, "
              + getAttribute_id()
              + ", 0, 0, 0, '', '', now() FROM xinco_core_data WHERE xinco_core_data_type_id = "
              + getXinco_core_data_type_id());
      stmt.close();
  ...
  public static int deleteFromDB(
      XincoCoreDataTypeAttribute attrCDTA, XincoDBManager DBM, int userID) throws XincoException {
    ...
    try {
       ...
       */
      stmt.executeUpdate(
          "DELETE FROM xinco_add_attribute WHERE xinco_add_attribute.attribute_id="
              + attrCDTA.getAttribute_id()
              + " AND xinco_add_attribute.xinco_core_data_id IN (SELECT id FROM xinco_core_data"
              + " WHERE xinco_core_data.xinco_core_data_type_id="
              + attrCDTA.getXinco_core_data_type_id()
              + ")");
      stmt.close();
      ...
      audit.updateAuditTrail(
          "xinco_core_data_type_attribute",
          new String[] {
          ...
          userID);
      stmt.executeUpdate(
          "DELETE FROM xinco_core_data_type_attribute WHERE xinco_core_data_type_id="
              + attrCDTA.getXinco_core_data_type_id()
              + " AND attribute_id="
              + attrCDTA.getAttribute_id());
      stmt.close();
  ...
  public static Vector getXincoCoreDataTypeAttributes(int attrID, XincoDBManager DBM) {
    ...
    try {
      ...
      while (rs.next()) {
        coreDataTypeAttributes.addElement(
