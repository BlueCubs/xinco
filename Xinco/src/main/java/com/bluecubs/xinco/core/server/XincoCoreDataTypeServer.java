...
public class XincoCoreDataTypeServer extends XincoCoreDataType {
  ...
  public XincoCoreDataTypeServer(int attrID, XincoDBManager DBM) throws XincoException {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        setXinco_core_data_type_attributes(
            XincoCoreDataTypeAttributeServer.getXincoCoreDataTypeAttributes(getId(), DBM));
      }
      if (RowCount < 1) {
  ...
  // create data type object for data structures
  public XincoCoreDataTypeServer(int attrID, String attrD, String attrDESC, Vector attrA)
  ...
  public static Vector getXincoCoreDataTypes(XincoDBManager DBM) {
    ...
    try {
      ...
      while (rs.next()) {
        coreDataTypes.addElement(
            new XincoCoreDataTypeServer(
                rs.getInt("id"),
                rs.getString("designation"),
                rs.getString("description"),
                XincoCoreDataTypeAttributeServer.getXincoCoreDataTypeAttributes(
                    rs.getInt("id"), DBM)));
      }
