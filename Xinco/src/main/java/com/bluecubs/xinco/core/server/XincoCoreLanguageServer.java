 ...
 *
 * <p>Name: XincoCoreLanguageServer
 *
...
public class XincoCoreLanguageServer extends XincoCoreLanguage {
    ...
    try {
      ...
      if (getId() > 0) {
        ...
        audit.updateAuditTrail(
            ...
            new String[] {"id =" + getId()},
            DBM,
            xerb.getString("audit.language.change"),
            this.getChangerID());
        stmt.executeUpdate(
            "UPDATE xinco_core_language SET sign='"
                + getSign().replaceAll("'", "\\\\'")
                + "', designation='"
        ...
        stmt = DBM.con.createStatement();
        stmt.executeUpdate(
            "INSERT INTO xinco_core_language VALUES ("
                + getId()
                + ", '"
                + getSign().replaceAll("'", "\\\\'")
                + "', '"
  ...
  // delete from db
  public static int deleteFromDB(XincoCoreLanguage attrCL, XincoDBManager DBM, int userID)
  ...
  public static Vector getXincoCoreLanguages(XincoDBManager DBM) {
    ...
    try {
      ...
      while (rs.next()) {
        coreLanguages.addElement(
            new XincoCoreLanguageServer(
                rs.getInt("id"), rs.getString("sign"), rs.getString("designation")));
      }
  ...
  public static boolean isLanguageUsed(XincoCoreLanguage xcl, XincoDBManager DBM) {
    ...
    try {
      ...
      rs =
          stmt.executeQuery(
              "SELECT 1 FROM xinco_core_node WHERE xinco_core_language_id = " + xcl.getId());
      while (rs.next()) {
        is_used = true;
      }
      stmt.close();
      ...
      if (!is_used) {
        ...
        rs =
            stmt.executeQuery(
                "SELECT 1 FROM xinco_core_data WHERE xinco_core_language_id = " + xcl.getId());
        while (rs.next()) {
          is_used = true;
        }
        stmt.close();
