...
public class XincoCoreLogServer extends XincoCoreLog {
    ...
    try {
      ...
      if (getId() > 0) {
        ...
        audit.updateAuditTrail(
            ...
            new String[] {"id =" + getId()},
            DBM,
            xerb.getString("audit.log.change"),
            this.getChangerID());
        stmt.executeUpdate(
            "UPDATE xinco_core_log SET xinco_core_data_id="
                + getXinco_core_data_id()
                + ", xinco_core_user_id="
                + getXinco_core_user_id()
                + ", op_code="
                + getOp_code()
        ...
        Statement stmt = DBM.con.createStatement();
        stmt.executeUpdate(
            "INSERT INTO xinco_core_log VALUES ("
                + getId()
                + ", "
                + getXinco_core_data_id()
                + ", "
  ...
  public static Vector getXincoCoreLogs(int attrID, XincoDBManager DBM) {
    ...
    try {
      Statement stmt = DBM.con.createStatement();
      ResultSet rs =
          stmt.executeQuery("SELECT * FROM xinco_core_log WHERE xinco_core_data_id=" + attrID);
      ...
      while (rs.next()) {
        ...
        core_log.addElement(
            new XincoCoreLogServer(
                ...
                rs.getInt("op_code"),
                cal,
                rs.getString("op_description"),
                rs.getInt("version_high"),
                rs.getInt("version_mid"),
                rs.getInt("version_low"),
                rs.getString("version_postfix")));
      }
