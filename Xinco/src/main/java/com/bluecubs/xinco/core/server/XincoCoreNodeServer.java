...
public class XincoCoreNodeServer extends XincoCoreNode {
  ...
  public XincoCoreNodeServer(int attrID, XincoDBManager DBM) throws XincoException {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        setXinco_core_acl(
            XincoCoreACEServer.getXincoCoreACL(rs.getInt("id"), "xinco_core_node_id", DBM));
      }
      if (RowCount < 1) {
  ...
  public XincoCoreNodeServer(
      int attrID, int attrCNID, int attrLID, String attrD, int attrSN, XincoDBManager DBM)
    ...
    try {
      ...
      if (getId() > 0) {
        ...
        audit.updateAuditTrail(
            ...
            new String[] {"id =" + getId()},
            DBM,
            "audit.corenode.change",
            this.getChangerID());
        stmt.executeUpdate(
            "UPDATE xinco_core_node SET xinco_core_node_id="
                + xcnid
        ...
        stmt.executeUpdate(
            "INSERT INTO xinco_core_node VALUES ("
                + getId()
                + ", "
                + xcnid
                + ", "
    ...
    try {
      ...
      for (i = 0; i < getXinco_core_data().size(); i++) {
        XincoIndexer.removeXincoCoreData(
            (XincoCoreDataServer) getXinco_core_data().elementAt(i), DBM);
        XincoCoreDataServer.removeFromDB(
            DBM, userID, ((XincoCoreDataServer) getXinco_core_data().elementAt(i)).getId());
  ...
  public void fillXincoCoreNodes(XincoDBManager DBM) {
    ...
    try {
      ...
      ResultSet rs =
          stmt.executeQuery(
              "SELECT * FROM xinco_core_node WHERE xinco_core_node_id = "
                  + getId()
        ...
        getXinco_core_nodes()
            .addElement(
                new XincoCoreNodeServer(
                    rs.getInt("id"),
                    rs.getInt("xinco_core_node_id"),
                    rs.getInt("xinco_core_language_id"),
  ...
  public void fillXincoCoreData(XincoDBManager DBM) {
    ...
    try {
      ...
      ResultSet rs =
          stmt.executeQuery(
              "SELECT * FROM xinco_core_data WHERE xinco_core_node_id = "
                  + getId()
                  + " ORDER BY designation");
        ...
        getXinco_core_data()
            .addElement(
                new XincoCoreDataServer(
                    ...
                    rs.getInt("xinco_core_node_id"),
                    rs.getInt("xinco_core_language_id"),
                    rs.getInt("xinco_core_data_type_id"),
                    rs.getString("designation"),
                    rs.getInt("status_number"),
                    DBM));
      }
  ...
  public static Vector findXincoCoreNodes(String attrS, int attrLID, XincoDBManager DBM) {
    ...
    try {
      ...
      ResultSet rs =
          stmt.executeQuery(
              "SELECT * FROM xinco_core_node WHERE xinco_core_language_id = "
                  + attrLID
