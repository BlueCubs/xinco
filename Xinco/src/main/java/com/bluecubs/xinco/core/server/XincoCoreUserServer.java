 ...
 * twice the attempts in the DB when wrong password was used Javier A. Ortiz 01/08/2010
 * ************************************************************
 */
...
public class XincoCoreUserServer extends XincoCoreUser {
  ...
  private void fillXincoCoreGroups(XincoDBManager DBM) throws XincoException {
    ...
    try {
      ...
      ResultSet rs =
          stmt.executeQuery(
              "SELECT * FROM xinco_core_user_has_xinco_core_group WHERE xinco_core_user_id="
                  + getId());
      while (rs.next()) {
        getXinco_core_groups()
            .addElement(new XincoCoreGroupServer(rs.getInt("xinco_core_group_id"), DBM));
      }
      stmt.close();
    } catch (Exception e) {
  ...
  private void writeXincoCoreGroups(XincoDBManager DBM) throws XincoException {
    ...
    try {
      ...
      stmt.executeUpdate(
          "DELETE FROM xinco_core_user_has_xinco_core_group WHERE xinco_core_user_id=" + getId());
      stmt.close();
  ...
  public XincoCoreUserServer(String attrUN, String attrUPW, XincoDBManager DBM)
    ...
    try {
      stmt = DBM.con.createStatement();
      sql =
          "SELECT * FROM xinco_core_user WHERE username='"
              + attrUN
      ...
      try {
        ...
        sql =
            "SELECT * FROM xinco_core_user WHERE username='" + attrUN + "' AND status_number <> 2";
        stmt = DBM.con.createStatement();
        rs = stmt.executeQuery(sql);
      ...
      if (getId() > 0) {
        ...
        if (isChange()) {
          ...
          audit.updateAuditTrail(
              "xinco_core_user", new String[] {"id =" + getId()}, DBM, getReason(), getId());
          ts = new Timestamp(System.currentTimeMillis());
          setLastModified(ts);
          setChange(false);
        } else {
        ...
        stmt.executeUpdate(
            "UPDATE xinco_core_user SET username='"
        ...
        stmt = DBM.con.createStatement();
        sql =
            "INSERT INTO xinco_core_user VALUES ("
                + getId()
                + ", '"
                + getUsername().replaceAll("'", "\\\\'")
                + "', MD5('"
