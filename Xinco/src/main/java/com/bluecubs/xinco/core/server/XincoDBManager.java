 ...
 * <p>Modifications:
 *
 * <p>Who? When? What? Javier A. Ortiz 01/04/2010 Added methods for result set manipulation and
 * formating
 *
 * <p>************************************************************
 */
    ...
    try {
      ...
      while (rs.next()) {
        for (int i = 1; i <= size; i++) {
          ...
          if (i == size && details) {
            out.println(
                "<td><form action='Detail.jsp' method='post'><input type='submit' value='Get"
                    + " Details' onclick='Detail.jsp'><input type='hidden' name = 'key' value='"
                    + value
                    + "'><input type='hidden' name='Page' value='ProcessData.jsp'></form></td>");
          } else {
            ...
            if (i == columnAsLink && linkType == this.DataLink) {
              if (value == null) {
                ...
                out.println(
                    "<td>"
                        + value
                        + "</td><td><form action='Detail.jsp' method='post'><input type='submit'"
                        + " value='Get Details' onclick='Detail.jsp'><input type='hidden' name ="
                        + " 'key' value='"
                        + value
                        + "'><input type='hidden' name='Page' value='Codes.jsp'></form></td>");
              }
            } else {
              if (value == null) {
