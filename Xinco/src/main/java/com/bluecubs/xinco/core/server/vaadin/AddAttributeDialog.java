...
class AddAttributeDialog extends CustomComponent {
  ...
  AddAttributeDialog(XincoCoreData data) {
    com.vaadin.ui.Panel panel =
        new com.vaadin.ui.Panel(
            getInstance().getResource().getString("window.addattributesuniversal"));
    panel.setContent(new VerticalLayout());
    /*
     * Define the names and data types of columns. The "default value"
     * parameter is meaningless here.
     */
    attrTable.addContainerProperty(
        getInstance().getResource().getString("general.attribute"), String.class, null);
    attrTable.addContainerProperty(
        getInstance().getResource().getString("general.details"), String.class, null);
    ...
    if (data.getId() == 0) {
      ...
      attributes = getXincoAddAttributes(data.getId());
    }
    ...
    for (int i = start; i < attributes.size(); i++) {
      ...
      String type =
          getInstance().getResource().containsKey(dataTypeAttributes.get(i).getDesignation())
              ? getInstance().getResource().getString(dataTypeAttributes.get(i).getDesignation())
              : dataTypeAttributes.get(i).getDesignation();
      attrTable.addItem(new Object[] {type, value}, i);
    }
    attrTable.setTableFieldFactory(
        (com.vaadin.data.Container container,
            Object itemId,
            Object propertyId,
            com.vaadin.ui.Component uiContext) -> {
          if (propertyId.equals(getInstance().getResource().getString("general.attribute"))) {
  ...
  public void updateAttributes() {
    for (Iterator<?> it = attrTable.getItemIds().iterator(); it.hasNext(); ) {
      ...
      getInstance()
          ...
          .get(id)
          .setAttribVarchar(
              item.getItemProperty(getInstance().getResource().getString("general.details"))
                  .getValue()
                  .toString());
    }
  }
