...
class DataDialog extends CustomComponent {
  ...
  DataDialog(boolean newData) {
    ...
    com.vaadin.ui.Panel panel =
        new com.vaadin.ui.Panel(getInstance().getResource().getString("window.datadetails"));
    ...
    designationField.setValue(getInstance().getXincoCoreData().getDesignation());
    // Language selection
      ...
      if (getInstance().getResource().containsKey(designation)) {
        ...
        if (newData && ((XincoCoreLanguageServer) language).getSign().equals("en")) {
          languages.setValue(((XincoCoreLanguageServer) language).getId());
            ...
            && ((XincoCoreLanguageServer) language).getId()
                == getInstance().getXincoCoreData().getXincoCoreLanguage().getId()) {
          languages.setValue(((XincoCoreLanguageServer) language).getId());
        }
      } else {
        getLogger(DataDialog.class.getName())
            .log(WARNING, "{0} not defined in XincoMessagesLocale", "Locale." + designation);
      }
      if (getInstance().getXincoCoreData() != null
          && getInstance().getXincoCoreData().getXincoCoreLanguage() != null
          && getInstance()
              .getXincoCoreData()
              .getXincoCoreLanguage()
              .getSign()
              .equals(designation)) {
    ...
    // Status
    statusField =
        new com.vaadin.ui.TextField(getInstance().getResource().getString("general.status") + ":");
    // Not editable
    statusField.setEnabled(false);
    String text = "";
    if (newData || getInstance().getXincoCoreData().getStatusNumber() == 1) {
      text = getInstance().getResource().getString("general.status.open");
    }
    if (getInstance().getXincoCoreData() != null
        && getInstance().getXincoCoreData().getStatusNumber() == 2) {
      text = getInstance().getResource().getString("general.status.locked") + " (-)";
    }
    if (getInstance().getXincoCoreData() != null
        && getInstance().getXincoCoreData().getStatusNumber() == 3) {
      text = getInstance().getResource().getString("general.status.archived") + " (->)";
    }
    if (getInstance().getXincoCoreData() != null
        && getInstance().getXincoCoreData().getStatusNumber() == 4) {
      text = getInstance().getResource().getString("general.status.checkedout") + " (X)";
    }
    if (getInstance().getXincoCoreData() != null
        && getInstance().getXincoCoreData().getStatusNumber() == 5) {
      text = getInstance().getResource().getString("general.status.published") + " (WWW)";
    }
