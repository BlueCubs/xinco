...
public class LuceneSearchWindow extends Window {
  ...
  public LuceneSearchWindow() {
    allLanguages =
        new CheckBox(
            getInstance().getResource().getString("window.search.alllanguages"),
    ...
    com.vaadin.ui.Button addToQuery =
        new com.vaadin.ui.Button(
            getInstance().getResource().getString("window.search.addtoquery"),
            (com.vaadin.ui.Button.ClickEvent event) -> {
              ...
              // append to query
              queryField.setValue(
                  queryField.getValue().toString()
                      + (operator.getValue() == null ? "" : operator.getValue())
                      + field
                      + variableField.getValue().toString()
                      + " ");
              variableField.setValue("");
            });
    com.vaadin.ui.Button reset =
        new com.vaadin.ui.Button(
            getInstance().getResource().getString("general.reset"),
            (com.vaadin.ui.Button.ClickEvent event) -> {
              queryField.setValue("");
            });
    com.vaadin.ui.Button searchButton =
        new com.vaadin.ui.Button(
            getInstance().getResource().getString("window.search"),
            (com.vaadin.ui.Button.ClickEvent event) -> {
    ...
    options.setItemCaption(
        "file", getInstance().getResource().getString("window.search.filecontent") + " (file)");
    String text;
    ...
    for (Iterator<XincoCoreDataTypeServer> it = alldatatypes.iterator(); it.hasNext(); ) {
      ...
      for (com.bluecubs.xinco.server.service.XincoCoreDataTypeAttribute attr :
          type.getXincoCoreDataTypeAttributes()) {
        text = attr.getDesignation();
        options.addItem(text);
        options.setItemCaption(
            text,
            getInstance().getResource().containsKey(text)
                ? getInstance().getResource().getString(text)
                : text);
      }
    }
    form.getLayout()
        .addComponent(
            new com.vaadin.ui.Label(
                getInstance().getResource().getString("window.search.querybuilder") + ":"));
    form.getLayout()
        .addComponent(
            new com.vaadin.ui.Label(
                getInstance().getResource().getString("window.search.querybuilderhints")));
    form.getLayout()
        .addComponent(
            new com.vaadin.ui.Label(
                getInstance().getResource().getString("window.search.querybuilderhintslabel")));
    ...
    hl4.addComponent(
        new com.vaadin.ui.Label(getInstance().getResource().getString("general.language") + ":"));
        ...
        (com.vaadin.ui.Button.ClickEvent event) -> {
          ...
          XincoCoreDataServer data =
              new XincoCoreDataServer(
                  valueOf(table.getItem(table.getValue()).getItemProperty("id").toString()));
          // Now check permissions on the file for the user
          data.loadACL();
          if (loggedIn) {
            XincoCoreACE ace =
                checkAccess(getInstance().getLoggedUser(), (ArrayList) data.getXincoCoreAcl());
          ...
          if (haveAccess) {
            ...
            while (parent1 != null) {
              if ((loggedIn
                      && checkAccess(
                              getInstance().getLoggedUser(), (ArrayList) data.getXincoCoreAcl())
                          .isReadPermission())
          ...
          if (!(haveAccess && getInstance().expandTreeNodes(parents))) {
            ...
            getApplication()
                .getMainWindow()
                .showNotification(
                    getInstance().getResource().getString("error.data.sufficientrights"),
  ...
  private void fixTable() {
    for (Object id : table.getContainerPropertyIds()) {
      ...
      table.setColumnHeader(
          id,
          getInstance().getResource().containsKey(id.toString())
              ? getInstance().getResource().getString(id.toString())
              : id.toString());
    }
  }
