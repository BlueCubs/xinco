 ...
 *
 * <p>Name: XincoDBManager
 *
 * <p>Description: server-side database manager
 *
...
import static com.vaadin.terminal.gwt.client.ui.dd.VerticalDropLocation.MIDDLE;
...
import static com.vaadin.ui.Tree.TargetItemAllowsChildren.get;
...
public class Xinco extends Application implements HttpServletRequestListener {
  ...
  private final ThemeResource smallIcon = new ThemeResource("img/blueCubsIcon16x16.GIF");
  ...
  public void init() {
    ...
    try {
      ...
      if (getState().equals(VALID)) {
        try {
          xincoVersion.setVersionHigh(
              getSetting(new XincoCoreUserServer(1), "version.high").getIntValue());
          xincoVersion.setVersionMid(
              getSetting(new XincoCoreUserServer(1), "version.mid").getIntValue());
          xincoVersion.setVersionLow(
              getSetting(new XincoCoreUserServer(1), "version.low").getIntValue());
          xincoVersion.setVersionPostfix(
              getSetting(new XincoCoreUserServer(1), "version.postfix").getStringValue());
        ...
        icon =
            new Embedded(
                "Xinco - "
                    ...
                    + " "
                    + version,
                new ThemeResource("img/blueCubsSmall.gif"));
        icon.setType(TYPE_IMAGE);
        login =
            new com.vaadin.ui.Button(
            ...
            if (!parameters.isEmpty()) {
              ...
              for (Entry<String, String[]> e : parameters.entrySet()) {
                ...
                if (e.getKey().equals("dataId")) {
                  ...
                  XincoCoreDataServer data1;
                  try {
                    data1 = new XincoCoreDataServer(dataId);
                    switch (data1.getXincoCoreDataType().getId()) {
                      case 1:
                        try {
                          // Download a file
                          downloadFile(data1);
                    ...
                    getMainWindow()
                        .showNotification(
                            getInstance().getResource().getString("general.error") + xe,
                            TYPE_ERROR_MESSAGE);
                  }
                  ...
                  getMainWindow()
                      .showNotification(
                          "We should search the folder here!", TYPE_HUMANIZED_MESSAGE);
                }
              }
            } else {
          ...
          .addComponent(
              new com.vaadin.ui.Label(
                  "Unable to start application, please contact your "
                      + "administrator!\n"
                      + displayDBStatus()));
      LOG.log(SEVERE, "An uncaught exception occurred: ", ex);
    }
  }
  ...
  public void setLocale(Locale locale) {
    this.locale = locale;
    xerb = getBundle("com.bluecubs.xinco.messages.XincoMessages", getInstance().getLocale());
    if (login != null) {
    ...
    fileStep =
        new WizardStep() {
          ...
          final Upload upload =
              new Upload(getInstance().getResource().getString("general.file.select"), um);
    ...
    if (menu != null && accountPanel != null) {
      menu.getTab(accountPanel).setCaption(rb.getString("window.connection.profile"));
    }
  }
  ...
  private void addHeader() {
    ...
    com.vaadin.ui.Label title =
        new com.vaadin.ui.Label(
            getSetting(new XincoCoreUserServer(1), "general.title").getStringValue());
    ...
    getMainWindow()
        .addListener(
            (MouseEvents.ClickListener)
                (ClickEvent event) -> {
                  // Do nothing. Based on
                  // https://vaadin.com/es/forum/-/message_boards/view_message/359139
                });
  ...
  private FileResource getFlagIcon(String code) throws IOException {
    ...
    if (code.contains("_")) {
      ...
      LOG.log(FINE, "Converting code from: {0} to {1}!", new Object[] {code, newCode});
      code = newCode;
    }
    LOG.log(FINE, "Requested icon for code: {0}", code);
    WebApplicationContext context = (WebApplicationContext) getContext();
    File iconsFolder =
        new File(
            context.getHttpSession().getServletContext().getRealPath("/VAADIN/themes/xinco")
    ...
    File tempIcon =
        new File(
            iconsFolder.getAbsolutePath()
                + System.getProperty("file.separator")
                + code.toLowerCase()
                + ".png");
    FileResource resource = null;
    if (tempIcon.exists()) {
      LOG.log(FINE, "Found icon for code: {0}!", code);
      resource = new FileResource(tempIcon, Xinco.this);
    } else {
      LOG.log(FINE, "Unable to find icon for: {0}", code);
    }
    return resource;
  }
  ...
  protected FileResource getIcon(String extension) throws IOException {
    ...
    if (!tempIconFile.exists()) {
      ...
      if (tempIcon != null) {
        ...
        BufferedImage buffered =
            new BufferedImage(image.getWidth(null), image.getHeight(null), TYPE_INT_RGB);
    ...
    FileResource resource = new FileResource(tempIconFile, Xinco.this);
    return resource;
  }
  ...
  public Tree getXincoTree() {
    if (xincoTree == null) {
      try {
        ...
        xincoTree.setDropHandler(
            new DropHandler() {
              private static final long serialVersionUID = -8_570_590_471_545_360_238L;
                ...
                XincoCoreNodeServer targetN =
                    new XincoCoreNodeServer(
                        valueOf(
                            targetItemId
                                .toString()
                                .substring(targetItemId.toString().indexOf('-') + 1)));

                // On which side of the target the item was dropped
                VerticalDropLocation location = target.getDropLocation();

                // Drop right on an item -> make it a child
                if (checkAccess(loggedUser, (ArrayList) (targetN).getXincoCoreAcl())
                        .isWritePermission()
                    && location == MIDDLE) {
                  try {
                      ...
                      XincoCoreDataServer source =
                          new XincoCoreDataServer(
                              parseInt(
                                  sourceItemId
                                      .toString()
                                      .substring(sourceItemId.toString().indexOf('-') + 1)));
              ...
              public AcceptCriterion getAcceptCriterion() {
                ...
                return new Or(get(), new Not(VerticalLocationIs.MIDDLE));
              }
            });
      } catch (XincoException ex) {
        LOG.log(SEVERE, null, ex);
  ...
  private Table getXincoTable() {
    if (xincoTable == null) {
       ...
       */
      xincoTable.addContainerProperty(
          getInstance().getResource().getString("window.repository.table.attribute"),
          String.class,
          null);
      xincoTable.addContainerProperty(
          getInstance().getResource().getString("window.repository.table.details"),
          com.vaadin.ui.Component.class,
          null);
  ...
  private void addChildrenNodes(String parent) throws XincoException {
    XincoCoreNodeServer xcns =
        new XincoCoreNodeServer(valueOf(parent.substring(parent.indexOf('-') + 1)));
    xcns.getXincoCoreNodes().stream()
        .map(
            (subnode) -> {
              LOG.log(FINE, "Checking permissions for: {0}", subnode.getDesignation());
              return subnode;
            })
        .forEachOrdered(
            (subnode) -> {
              XincoCoreACE access = checkAccess(loggedUser, subnode.getXincoCoreAcl());
              if (access.isAdminPermission() || access.isReadPermission()) {
                String id = "node-" + subnode.getId();
                Item item = xincoTreeContainer.addItem(id);
                if (item == null) {
                  item = xincoTreeContainer.getItem(id);
                }
  ...
  private void addChildrenData(String parent) throws XincoException {
    ...
    for (XincoCoreData temp : xcns.getXincoCoreData()) {
      LOG.log(FINE, "Checking permissions for: {0}", temp.getDesignation());
      XincoCoreACE access = checkAccess(loggedUser, temp.getXincoCoreAcl());
      if (access.isAdminPermission() || access.isReadPermission()) {
            ...
            try {
              ...
              switch (temp.getXincoCoreDataType().getId()) {
                ...
                case 5: // Rendering
                  if (name != null
                      && name.contains(".")
                      && name.substring(name.lastIndexOf('.') + 1, name.length()).length() >= 3) {
                    icon1 = getIcon(name.substring(name.lastIndexOf('.') + 1, name.length()));
                  }
                  break;
                case 2: // Text
                  icon1 = getIcon("txt");
                ...
                default:
                  throw new RuntimeException(
                      "Invalid Data Type: " + temp.getXincoCoreDataType().getId());
              }
              if (icon1 != null) {
  ...
  private void checkinFile() {
    ...
    try {
      ...
      newlog.setOpDescription(
          getInstance().getResource().getString(getOPCode(newlog.getOpCode()).getName()));
      newlog.setXincoCoreUserId(loggedUser.getId());
      newlog.setXincoCoreDataId(temp.getId());
      newlog.setVersion(
          ((XincoCoreLog) temp.getXincoCoreLogs().get(temp.getXincoCoreLogs().size() - 1))
              .getVersion());
      newlog.setOpDescription(
              ...
              try {
                ...
                try {
                  ...
                  XincoCoreLogServer log =
                      new XincoCoreLogServer(
                          ((XincoCoreLog)
                                  getXincoCoreData()
                                      .getXincoCoreLogs()
                                      .get(getXincoCoreData().getXincoCoreLogs().size() - 1))
                              .getId());
  ...
  private void buildLogDialog(XincoCoreDataServer temp, Form form, VersionSelector versionSelector)
      throws XincoException {
    buildLogDialog(temp, form, versionSelector, null);
  }

  private void buildLogDialog(
      XincoCoreDataServer temp, Form form, VersionSelector versionSelector, OPCode opcode)
      throws XincoException {
    form.addField(
        "action",
    ...
    final int log_index = temp.getXincoCoreLogs().size() - 1;
    form.getField("action")
        .setValue(((XincoCoreLog) temp.getXincoCoreLogs().get(log_index)).getOpDescription());
    form.getField("action").setEnabled(false);
    form.addField("reason", new com.vaadin.ui.TextArea());
    OPCode code;
    if (opcode == null) {
      code = getOPCode(((XincoCoreLog) temp.getXincoCoreLogs().get(log_index)).getOpCode());
    } else {
      code = opcode;
    }
    switch (code) {
    ...
    form.getField("reason")
        .setRequiredError(getInstance().getResource().getString("message.warning.reason"));
        ...
        if (temp.getXincoCoreDataType().getId() == 1) {
          ...
          try {
            ...
            if (getService().doXincoCoreDataCheckout(temp, loggedUser) != null) {
              ...
              getMainWindow()
                  .showNotification(
                      getInstance().getResource().getString("general.error"), TYPE_WARNING_MESSAGE);
            }
          } catch (MalformedURLException | XincoException ex) {
            LOG.log(SEVERE, null, ex);
          }
  ...
  private void downloadFile(final XincoCoreData data) throws XincoException, MalformedURLException {
    ...
    StreamSource ss =
        new StreamSource() {
          @Serial private static final long serialVersionUID = 7_817_215_035_385_710_852L;
          byte[] bytes = getService().downloadXincoCoreData(data, loggedUser);
    ...
    StreamResource sr =
        new StreamResource(ss, data.getXincoAddAttributes().get(0).getAttribVarchar(), this);
  ...
  private void loadFile(File file, String fileName) {
    try {
        ...
        temp =
            new File(
                file.getParentFile().getAbsolutePath()
                    ...
                    + randomUUID().toString());
        temp.mkdirs();
        path_to_file = temp.getAbsolutePath() + FileSystems.getDefault().getSeparator() + fileName;
        if (!file.renameTo(new File(path_to_file))) {
          throw new RuntimeException(
              "Unable to rename file from " + file.getAbsolutePath() + " to " + path_to_file);
        }
        file.deleteOnExit();
        temp.deleteOnExit();
      }
      ...
      byte[] byteArray = null;
      try {
        cin = new CheckedInputStream(new FileInputStream(path_to_file), new CRC32());
        ...
        out.close();
        // update attributes
        ((XincoAddAttribute) getXincoCoreData().getXincoAddAttributes().get(0))
            .setAttribVarchar(fileName);
        getXincoCoreData().setDesignation(fileName);
        ((XincoAddAttribute) getXincoCoreData().getXincoAddAttributes().get(1))
            .setAttribUnsignedint(totalLen);
        ((XincoAddAttribute) getXincoCoreData().getXincoAddAttributes().get(2))
            .setAttribVarchar("" + cin.getChecksum().getValue());
        ((XincoAddAttribute) getXincoCoreData().getXincoAddAttributes().get(3))
      ...
      if (cin != null) {
        ...
        // upload file
        if (getService().uploadXincoCoreData(getXincoCoreData(), byteArray, loggedUser)
            != totalLen) {
          cin.close();
  ...
  private void showRenderingDialog() throws XincoException {
    if (renderingSupportEnabled) {
      ...
      final ArrayList<com.bluecubs.xinco.core.server.persistence.XincoCoreData> renderings =
          (ArrayList<com.bluecubs.xinco.core.server.persistence.XincoCoreData>)
              getRenderings(
                  parseInt(
                      xincoTree
                          .getValue()
      ...
      table.addContainerProperty(
          getInstance().getResource().getString("general.name"), com.vaadin.ui.Label.class, null);
      table.addContainerProperty(
          getInstance().getResource().getString("general.version"),
          com.vaadin.ui.Label.class,
          null);
      table.addContainerProperty(
      ...
      renderings.stream()
          .map((xcd) -> new XincoCoreDataServer(xcd.getId()))
          .forEachOrdered(
              (xcds) -> {
                ...
                table.addItem(
                    new Object[] {
                      new com.vaadin.ui.Label(xcds.getDesignation()),
                      new com.vaadin.ui.Label(
                          xVersion.getVersionHigh()
                              + "."
      ...
      final com.vaadin.ui.Button commit =
          new com.vaadin.ui.Button(
      ...
      commit.addListener(
          (com.vaadin.ui.Button.ClickListener)
              (com.vaadin.ui.Button.ClickEvent event) -> {
                try {
                  final XincoCoreDataServer parent =
                      new XincoCoreDataServer(
                          valueOf(
                              xincoTree
                                  .getValue()
                  ...
                  getXincoCoreData()
                      .setXincoCoreDataType(
                          new XincoCoreDataTypeServer(parent.getXincoCoreDataType().getId()));
                          ...
                          try {
                            ...
                            getMainWindow()
                                .showNotification(
                                    ...
                                    getInstance()
                                        .getResource()
                                        .getString("message.error.association.exists"),
                                    TYPE_ERROR_MESSAGE);
                          }
  ...
  private void showACLDialog() {
    ...
    table.addContainerProperty(
        getInstance().getResource().getString("general.acl.adminpermission"), CheckBox.class, null);
    table.addContainerProperty(
        getInstance().getResource().getString("general.acl.readpermission"), CheckBox.class, null);
    table.addContainerProperty(
        getInstance().getResource().getString("general.acl.writepermission"), CheckBox.class, null);
    table.addContainerProperty(
    ...
    for (Iterator it = getXincoCoreGroups().iterator(); it.hasNext(); ) {
      ...
      String value =
          getInstance().getResource().containsKey(designation)
              ? getInstance().getResource().getString(designation)
              : designation;
      name = new com.vaadin.ui.Label(value);
      type = new com.vaadin.ui.Label(getInstance().getResource().getString("general.group"));
      final CheckBox admin =
          new CheckBox(getInstance().getResource().getString("general.acl.adminpermission"));
      admin.addListener(
          (ValueChangeEvent event) -> {
            if (admin.getData() != null) {
              aceList
                  .get(admin.getData().toString())
                  .setAdminPermission((Boolean) admin.getValue());
            }
          });
      admin.setImmediate(true);
      final CheckBox execute =
          new CheckBox(getInstance().getResource().getString("general.acl.executepermission"));
      execute.addListener(
          (ValueChangeEvent event) -> {
            if (execute.getData() != null) {
              aceList
                  .get(execute.getData().toString())
                  .setExecutePermission((Boolean) execute.getValue());
            }
          });
      execute.setImmediate(true);
      final CheckBox read =
          new CheckBox(getInstance().getResource().getString("general.acl.readpermission"));
      read.addListener(
          (ValueChangeEvent event) -> {
      ...
      final CheckBox write =
          new CheckBox(getInstance().getResource().getString("general.acl.writepermission"));
      write.addListener(
          (ValueChangeEvent event) -> {
            if (write.getData() != null) {
              aceList
                  .get(write.getData().toString())
                  .setWritePermission((Boolean) write.getValue());
            }
          });
          ...
          try {
            acl =
                new XincoCoreACEServer(
                    0, 0, group.getId(), tempNode.getId(), 0, false, false, false, false);
          } catch (XincoException ex) {
            LOG.log(SEVERE, null, ex);
          }
          for (Object o : tempNode.getXincoCoreAcl()) {
            XincoCoreACEServer temp = (XincoCoreACEServer) o;
      ...
      } else if (xincoTree.getValue() != null
          && xincoTree.getValue().toString().startsWith("data")) {
        try {
          XincoCoreDataServer tempData =
              new XincoCoreDataServer(
                  valueOf(
                      xincoTree
                          .getValue()
          ...
          for (Object o : tempData.getXincoCoreAcl()) {
            XincoCoreACEServer temp = (XincoCoreACEServer) o;
            if (temp.getXincoCoreGroupId() == group.getId()) {
              admin.setValue(temp.isAdminPermission());
              execute.setValue(temp.isExecutePermission());
      ...
      table.addItem(new Object[] {name, type, admin, read, write, execute}, itemId);
      acls.addItem(value);
    }
      ...
      if (xincoTree.getValue() != null && xincoTree.getValue().toString().startsWith("node")) {
        try {
          ...
          try {
            acl =
                new XincoCoreACEServer(
                    0, loggedUser.getId(), 0, tempNode.getId(), 0, false, false, false, false);
          } catch (XincoException ex) {
            LOG.log(SEVERE, null, ex);
          }
        ...
        try {
          ...
          try {
            acl =
                new XincoCoreACEServer(
                    0, 0, loggedUser.getId(), 0, tempData.getId(), false, false, false, false);
          } catch (XincoException ex) {
            LOG.log(SEVERE, null, ex);
          }
          for (Object o : tempData.getXincoCoreAcl()) {
    ...
    commit.addListener(
        (com.vaadin.ui.Button.ClickListener)
            (com.vaadin.ui.Button.ClickEvent event) -> {
              for (Iterator<Entry<String, XincoCoreACEServer>> it = aceList.entrySet().iterator();
                  it.hasNext(); ) {
                boolean skip = false;
                ...
                int thisOwner =
                    Integer.parseInt(e.getKey().substring(e.getKey().lastIndexOf("-") + 1));
                ...
                if (!skip) {
                  ...
                  if (stored != null
                      && (stored.isAdminPermission() != e.getValue().isAdminPermission()
                  ...
                  try {
                    if (stored != null && modified && !processed.contains(e.getValue().getId())) {
  ...
  private void showLoginDialog() {
    ...
    com.vaadin.ui.TextField username =
        new com.vaadin.ui.TextField(
            getInstance().getResource().getString("general.username") + ":");
    ...
    form.getField("username")
        .setRequiredError(getInstance().getResource().getString("message.missing.username"));
    form.getField("password").setRequired(true);
    form.getField("password")
        .setRequiredError(getInstance().getResource().getString("message.missing.password"));
    form.setFooter(new HorizontalLayout());
    // Used for validation purposes
    final com.vaadin.ui.Button commit =
        new com.vaadin.ui.Button(
            ...
            (com.vaadin.ui.Button.ClickEvent event) -> {
              ...
              if (!validCredentials(
                ...
                if (result.isEmpty()) {
                  ...
                  result =
                      createdQuery(
                          "SELECT x FROM XincoCoreUser x WHERE x.username='" + username1 + "'");
                  if (!result.isEmpty()) {
                    XincoCoreUserServer temp_user =
                        new XincoCoreUserServer(
                            (com.bluecubs.xinco.core.server.persistence.XincoCoreUser)
                                result.get(0));
                    long attempts =
                        getSetting(new XincoCoreUserServer(1), "password.attempts").getLongValue();
                    // If user exists increase the atempt tries in the db. If limit reached lock
                    // account
                    if (temp_user.getAttempts() >= attempts && temp_user.getId() != 1) {
                      ...
                      // Reason for change
                      temp_user.setReason(
                          getInstance().getResource().getString("password.attempt.limitReached"));
                      ...
                      getMainWindow()
                          .showNotification(
                              getInstance().getResource().getString("password.login.fail"),
                              TYPE_WARNING_MESSAGE);
                    }
                  }
                }
  ...
  private void refreshGroupTable(final Table table) {
    ...
    for (XincoCoreGroupServer group : (Iterable<XincoCoreGroupServer>) allgroups) {
      final com.vaadin.ui.Button edit =
          new com.vaadin.ui.Button(getInstance().getResource().getString("general.edit"));
  ...
  private void showGroupAdminWindow() {
    ...
    table.addStyleName("striped");
    table.addContainerProperty(
        getInstance().getResource().getString("general.id"), Integer.class, null);
    table.addContainerProperty(
        getInstance().getResource().getString("general.name"), String.class, null);
    table.addContainerProperty(
        getInstance().getResource().getString("general.edit"), com.vaadin.ui.Component.class, null);
    ...
    form.getField("name")
        .setRequiredError(getInstance().getResource().getString("message.missing.groupname"));
    // Used for validation purposes
    final com.vaadin.ui.Button commit =
        new com.vaadin.ui.Button(
        ...
        (com.vaadin.ui.Button.ClickListener)
            (com.vaadin.ui.Button.ClickEvent event) -> {
              try {
                ...
                cancel.setEnabled(false);
                // Process the data
                new XincoCoreGroupServer(0, form.getField("name").getValue().toString(), 1)
                    .write2DB();
                refreshGroupTable(table);
              } catch (XincoException ex) {
                LOG.log(SEVERE, null, ex);
  ...
  private void showDataTypeAttrAdminWindow(final int dataTypeId) {
    ...
    table.addStyleName("striped");
    table.addContainerProperty(
        getInstance().getResource().getString("general.position"), Integer.class, null);
    table.addContainerProperty(
        getInstance().getResource().getString("general.designation"), String.class, null);
    table.addContainerProperty(
        getInstance().getResource().getString("general.datatype"), String.class, null);
    table.addContainerProperty(
        getInstance().getResource().getString("general.size"), String.class, null);
    table.addContainerProperty("", com.vaadin.ui.Component.class, null);
    table.setSortContainerPropertyId(getInstance().getResource().getString("general.position"));
    form.getLayout().addComponent(table);
    refreshFileTypeAttrTable(table, dataTypeId);
    form.addField(
        "position",
        new com.vaadin.ui.TextField(
            getInstance().getResource().getString("general.position") + ":"));
    form.getField("position").setRequired(true);
    form.getField("position")
        .setRequiredError(getInstance().getResource().getString("message.missing.position"));
    form.addField(
        "designation",
        new com.vaadin.ui.TextField(
            getInstance().getResource().getString("general.designation") + ":"));
    form.getField("designation").setRequired(true);
    form.getField("designation")
        .setRequiredError(getInstance().getResource().getString("message.missing.designation"));
    Select types = new Select(getInstance().getResource().getString("general.datatype") + ":");
    ...
    typeDefs.entrySet().stream()
        .peek((entry) -> types.addItem(entry.getKey()))
        .forEachOrdered(
            (entry) -> {
    ...
    form.getField("type")
        .setRequiredError(getInstance().getResource().getString("message.missing.datatype"));
    form.addField(
        "size",
        new com.vaadin.ui.TextField(getInstance().getResource().getString("general.size") + ":"));
    form.getField("size").setRequired(true);
    form.getField("size")
        .setRequiredError(getInstance().getResource().getString("message.missing.size"));
    form.getField("size")
        .setDescription(
            getInstance().getResource().getString("message.admin.attribute.req4string"));
    // Used for validation purposes
    final com.vaadin.ui.Button commit =
        new com.vaadin.ui.Button(
            getInstance().getResource().getString("general.add.attribute"), form, "commit");
    final com.vaadin.ui.Button cancel =
        new com.vaadin.ui.Button(
            getInstance().getResource().getString("general.cancel"),
            (com.vaadin.ui.Button.ClickEvent event) -> {
              ...
              try {
                ...
                XincoCoreDataTypeAttributeServer tempAttribute =
                    new XincoCoreDataTypeAttributeServer(
                        dataTypeId,
                        parseInt(form.getField("position").getValue().toString()),
                        form.getField("designation").getValue().toString(),
                        form.getField("type").getValue().toString(),
                        parseInt(form.getField("size").getValue().toString()));
    ...
    form.getLayout()
        .addComponent(
            new com.vaadin.ui.Label(
                getInstance().getResource().getString("message.warning.attribute.remove")));
    form.getFooter().setSizeUndefined();
    form.getFooter().addComponent(commit);
    form.getFooter().addComponent(cancel);
  ...
  private void showChangePasswordDialog() {
    ...
    form.getLayout()
        .addComponent(
            new com.vaadin.ui.Label(getInstance().getResource().getString("password.aged")));
    form.addField(
        "password", new PasswordField(getInstance().getResource().getString("general.password")));
    form.addField(
        "confirm",
        new PasswordField(getInstance().getResource().getString("general.verifypassword")));
    form.getField("password").setRequired(true);
    form.getField("password")
        .setRequiredError(getInstance().getResource().getString("message.missing.password"));
    form.getField("confirm").setRequired(true);
    form.getField("confirm")
        .setRequiredError(getInstance().getResource().getString("message.missing.designation"));
    final com.vaadin.ui.Button commit = new com.vaadin.ui.Button("Submit", form, "commit");
    commit.addListener(
        (com.vaadin.ui.Button.ClickListener)
            (com.vaadin.ui.Button.ClickEvent event) -> {
              if (!form.getField("password")
                  .getValue()
                  .equals(form.getField("confirm").getValue())) {
                getMainWindow()
                    .showNotification(
                        getInstance().getResource().getString("window.userinfo.passwordmismatch"),
                        TYPE_WARNING_MESSAGE);
              } else {
                boolean passwordIsUsable =
                    loggedUser.isPasswordUsable(form.getField("password").getValue().toString());
                if (passwordIsUsable) {
                  try {
                    ...
                    getMainWindow()
                        .showNotification(
                            getInstance().getResource().getString("password.changed"),
                  ...
                  getMainWindow()
                      .showNotification(
                          getInstance().getResource().getString("password.unusable"),
                          TYPE_WARNING_MESSAGE);
                }
              }
            });
  ...
  private Select getLanguageOptions() {
    ...
    ArrayList<String> locales =
        new ArrayList<>(asList(lrb.getString("AvailableLocales").split(",")));
    locales.stream()
        .peek(languages::addItem)
        .peek(
            (tempLocale) ->
  ...
  private void showLanguageSelection() {
    ...
    form.getLayout().addComponent(new com.vaadin.ui.Label("Please choose a language:"));
    final Select languages = getLanguageOptions();
  ...
  private void showAuditWindow() throws XincoException {
    ...
    table.addStyleName("striped");
    table.addContainerProperty(
        getInstance().getResource().getString("general.table"), String.class, null);
    table.addContainerProperty(
        getInstance().getResource().getString("general.audit.action"),
        com.vaadin.ui.Component.class,
        null);
    table.setSortContainerPropertyId(getInstance().getResource().getString("general.table"));
        ...
        (type) -> {
          ...
          if (type.getJavaType().getSuperclass() == XincoAuditedObject.class) {
            java.util.List<Object> result =
                createdQuery(
                    "select distinct x from " + type.getJavaType().getSimpleName() + "T x");
            if (!result.isEmpty()) {
              final com.vaadin.ui.Button cont =
                  new com.vaadin.ui.Button(
                      getInstance().getResource().getString("general.continue"));
              cont.setData(type);
              cont.addListener(
                  (com.vaadin.ui.Button.ClickListener)
                      (com.vaadin.ui.Button.ClickEvent event) -> {
  ...
  private Table showEntitiesInTable(java.util.List entities) throws XincoException {
    ...
    if (!entities.isEmpty()) {
      ...
      EntityType<?> entityType = getEntityManagerFactory().getMetamodel().entity(entityClass);
      LinkedHashMap<String, PersistentAttributeType> typeMap = new LinkedHashMap<>();
      for (Attribute<?, ?> attribute : entityType.getAttributes()) {
        table.addContainerProperty(attribute.getName(), com.vaadin.ui.Component.class, null);
        typeMap.put(attribute.getName(), attribute.getPersistentAttributeType());
      }
      // Now add the audit fields
      table.addContainerProperty(
          getInstance().getResource().getString("general.reason"),
          com.vaadin.ui.Component.class,
          null);
      table.addContainerProperty(
          getInstance().getResource().getString("general.audit.modtime"),
          com.vaadin.ui.Component.class,
          null);
      table.addContainerProperty(
          getInstance().getResource().getString("general.user"),
      ...
      for (Object entity : entities) {
        ...
        for (Entry<String, PersistentAttributeType> entry : typeMap.entrySet()) {
          try {
            ...
            if (entry.getKey().equals("recordId")) {
              recordId = parseInt(method.invoke(o).toString());
            }
            String value = method.invoke(o).toString();
            if (Objects.requireNonNull(entry.getValue()) == BASIC) {
              values.add(
                  new Label(
                      entry.getKey().contains("password")
                          ? "**********"
                          : (getInstance().getResource().containsKey(value)
                              ? getInstance().getResource().getString(value)
                              : value)));
            } else {
        ...
        XincoCoreUserModifiedRecord record =
            (XincoCoreUserModifiedRecord)
                namedQuery("XincoCoreUserModifiedRecord.findByRecordId", parameters).get(0);
        values.add(
            new com.vaadin.ui.Label(
                getInstance().getResource().containsKey(record.getModReason())
                    ? getInstance().getResource().getString(record.getModReason())
                    : record.getModReason()));
        values.add(new com.vaadin.ui.Label(record.getModTime().toString()));
        values.add(
            new com.vaadin.ui.Label(
                record.getXincoCoreUser().getFirstName()
                    + " "
  ...
  private void showAuditDetails(final EntityType entity) {
    try {
      final com.vaadin.ui.Window audit = new com.vaadin.ui.Window();
      final com.vaadin.ui.TextField tf =
          new com.vaadin.ui.TextField(entity.getId(entity.getIdType().getJavaType()).getName());
      tf.focus();
      final Table table =
          showEntitiesInTable(
              createdQuery(
                  "select distinct x from " + entity.getJavaType().getSimpleName() + "T x"));
      table.setSortDisabled(false);
      tf.addListener(
          new TextChangeListener() {
            @Serial private static final long serialVersionUID = -7795404080421449032L;
      ...
      if (entity.getId(entity.getIdType().getJavaType()).getPersistentAttributeType() == BASIC) {
  ...
  private void showAttrAdminWindow() {
    ...
    table.addContainerProperty(
        getInstance().getResource().getString("general.designation"), String.class, null);
    table.addContainerProperty(
        getInstance().getResource().getString("general.description"), String.class, null);
    table.addContainerProperty("", com.vaadin.ui.Component.class, null);
    table.setSortContainerPropertyId(getInstance().getResource().getString("general.id"));
    form.getLayout().addComponent(table);
  ...
  private void showLangAdminWindow() {
    ...
    form.addField(
        "sign",
        new com.vaadin.ui.TextField(getInstance().getResource().getString("general.sign") + ":"));
    form.getField("sign").setRequired(true);
    form.getField("sign")
        .setRequiredError(getInstance().getResource().getString("message.missing.sign"));
    // Used for validation purposes
    final com.vaadin.ui.Button commit =
        new com.vaadin.ui.Button(
            getInstance().getResource().getString("general.add.language"), form, "commit");
    final com.vaadin.ui.Button cancel =
        new com.vaadin.ui.Button(
            getInstance().getResource().getString("general.cancel"),
            (com.vaadin.ui.Button.ClickEvent event) -> {
        ...
        (com.vaadin.ui.Button.ClickListener)
            (com.vaadin.ui.Button.ClickEvent event) -> {
              try {
                ...
                form.getField("designation").setEnabled(false);
                /*
                 * Check for duplicate designation. This is done at DB level but won't work for
                 * default designation since those are translated.
                 */
                for (XincoCoreLanguageServer lang1 : getXincoCoreLanguages()) {
                  if (getInstance().getResource().containsKey(lang1.getDesignation())
                      && getInstance()
                          .getResource()
                          .getString(lang1.getDesignation())
                          .equals(form.getField("designation").getValue().toString())) {
                    // Duplicate designation
                    throw new XincoException();
                  }
                }
                // Process the data
                XincoCoreLanguageServer lang2 =
                    new XincoCoreLanguageServer(
                        0,
                        form.getField("sign").getValue().toString(),
                        form.getField("designation").getValue().toString());
                lang2.setChangerID(loggedUser.getId());
                lang2.write2DB();
              } catch (XincoException ex) {
                getMainWindow()
                    .showNotification(
                        getInstance().getResource().getString("general.error"),
                        getInstance().getResource().getString("error.language.add.duplicate"),
                        TYPE_WARNING_MESSAGE);
                LOG.log(SEVERE, null, ex);
              } finally {
                refreshLanguageTable(table);
    ...
    form.getLayout()
        .addComponent(
            new com.vaadin.ui.Label(
                getInstance().getResource().getString("error.language.delete.referenced")));
    form.getFooter().setSizeUndefined();
    form.getFooter().addComponent(commit);
    form.getFooter().addComponent(cancel);
    lang.addComponent(form);
  ...
  private void refreshLanguageTable(final Table table) {
    ...
    boolean is_used;
    table.removeAllItems();
    for (XincoCoreLanguageServer lang : (Iterable<XincoCoreLanguageServer>) allLanguages) {
      is_used = isLanguageUsed(lang);
      final com.vaadin.ui.Button delete =
          new com.vaadin.ui.Button(getInstance().getResource().getString("general.delete"));
      ...
      delete.addListener(
          (com.vaadin.ui.Button.ClickListener)
              (com.vaadin.ui.Button.ClickEvent event) -> {
                try {
                  deleteFromDB(
                      new XincoCoreLanguageServer((Integer) delete.getData()), loggedUser.getId());
                  refreshLanguageTable(table);
                } catch (Exception e) {
  ...
  private void showUserAdminWindow(final boolean userAdmin) {
    ...
    if (userAdmin) {
      ...
      table.addContainerProperty(
          getInstance().getResource().getString("general.id"), Integer.class, null);
      table.addContainerProperty(
          getInstance().getResource().getString("general.username"), String.class, null);
      table.addContainerProperty(
          getInstance().getResource().getString("general.firstname"), String.class, null);
      table.addContainerProperty(
          getInstance().getResource().getString("general.lastname"), String.class, null);
      table.addContainerProperty(
          getInstance().getResource().getString("general.email"), String.class, null);
      table.addContainerProperty(
          getInstance().getResource().getString("general.lock")
              + "/"
              + getInstance().getResource().getString("general.unlock"),
          com.vaadin.ui.Component.class,
          null);
      table.addContainerProperty(
          getInstance().getResource().getString("general.password.reset") + "*",
          com.vaadin.ui.Component.class,
          null);
    ...
    form.addField(
        "pass",
        new com.vaadin.ui.PasswordField(
            getInstance().getResource().getString("general.password") + ":"));
    ...
    form.addField(
        "verify",
        new com.vaadin.ui.PasswordField(
            getInstance().getResource().getString("general.verifypassword") + ":"));
    form.getField("verify").setRequired(userAdmin);
    form.getField("verify")
        .setRequiredError(getInstance().getResource().getString("message.missing.password"));
    form.addField(
        "firstname",
        new com.vaadin.ui.TextField(
            getInstance().getResource().getString("general.firstname") + ":"));
    form.getField("firstname").setRequired(userAdmin);
    form.getField("firstname")
        .setRequiredError(getInstance().getResource().getString("message.missing.firstname"));
    if (!userAdmin) {
    ...
    form.addField(
        "lastname",
        new com.vaadin.ui.TextField(
            getInstance().getResource().getString("general.lastname") + ":"));
    form.getField("lastname").setRequired(userAdmin);
    form.getField("lastname")
        .setRequiredError(getInstance().getResource().getString("message.missing.lastname"));
    if (!userAdmin) {
      form.getField("lastname").setValue(loggedUser.getLastName());
    }
    form.addField(
        "email",
        new com.vaadin.ui.TextField(getInstance().getResource().getString("general.email") + ":"));
    form.getField("email").setRequired(userAdmin);
    form.getField("email")
        .setRequiredError(getInstance().getResource().getString("message.missing.email"));
    if (!userAdmin) {
    ...
    final com.vaadin.ui.Button commit =
        new com.vaadin.ui.Button(
            userAdmin
                ? getInstance().getResource().getString("general.add.user")
    ...
    commit.addListener(
        new com.vaadin.ui.Button.ClickListener() {
          @Serial private static final long serialVersionUID = -7612425956145479100L;
          ...
          public void buttonClick(com.vaadin.ui.Button.ClickEvent event) {
            try {
              ...
              if (userAdmin) {
                ...
                if (!form.getField("pass").getValue().toString().isEmpty()
                  ...
                  if (!form.getField("pass")
                      ...
                      .equals(form.getField("verify").getValue().toString())) {
                    getMainWindow()
                        .showNotification("window.userinfo.passwordmismatch", TYPE_WARNING_MESSAGE);
                    return;
                  } else {
              ...
              temp_user.setChange(true);
              boolean passwordIsUsable =
                  loggedUser.isPasswordUsable(form.getField("pass").getValue().toString());
              if (!passwordIsUsable) {
                getMainWindow()
                    .showNotification(
                        getInstance().getResource().getString("password.unusable"),
  ...
  private com.vaadin.ui.Component getSideMenu() throws XincoException {
    ...
    menu.addTab(
        accountPanel, getInstance().getResource().getString("window.connection.profile"), null);
    adminPanel = new com.vaadin.ui.Panel();
    userAdmin =
        new com.vaadin.ui.Button(getInstance().getResource().getString("message.admin.userAdmin"));
    userAdmin.setWidth(100, UNITS_PERCENTAGE);
    userAdmin.addListener(
        (com.vaadin.ui.Button.ClickListener)
            (com.vaadin.ui.Button.ClickEvent event) -> {
    ...
    groupAdmin =
        new com.vaadin.ui.Button(getInstance().getResource().getString("message.admin.groupAdmin"));
    groupAdmin.setWidth(100, UNITS_PERCENTAGE);
    groupAdmin.addListener(
        (com.vaadin.ui.Button.ClickListener)
            (com.vaadin.ui.Button.ClickEvent event) -> {
    ...
    langAdmin =
        new com.vaadin.ui.Button(getInstance().getResource().getString("message.admin.language"));
    langAdmin.setWidth(100, UNITS_PERCENTAGE);
    langAdmin.addListener(
        (com.vaadin.ui.Button.ClickListener)
            (com.vaadin.ui.Button.ClickEvent event) -> {
              showLangAdminWindow();
            });
    ...
    attrAdmin =
        new com.vaadin.ui.Button(getInstance().getResource().getString("message.admin.attribute"));
    attrAdmin.setWidth(100, UNITS_PERCENTAGE);
    attrAdmin.addListener(
        (com.vaadin.ui.Button.ClickListener)
            (com.vaadin.ui.Button.ClickEvent event) -> {
              showAttrAdminWindow();
            });
    ...
    trashAdmin =
        new com.vaadin.ui.Button(getInstance().getResource().getString("message.admin.trash"));
    trashAdmin.setWidth(100, UNITS_PERCENTAGE);
    trashAdmin.addListener(
        (com.vaadin.ui.Button.ClickListener)
            (com.vaadin.ui.Button.ClickEvent event) -> {
              try {
    ...
    indexAdmin =
        new com.vaadin.ui.Button(getInstance().getResource().getString("message.admin.index"));
    indexAdmin.setWidth(100, UNITS_PERCENTAGE);
    final Table table = new Table();
    table.addStyleName("striped");
    table.addContainerProperty(
        getInstance().getResource().getString("message.data.sort.designation"),
        com.vaadin.ui.Label.class,
        null);
    table.addContainerProperty(
        getInstance().getResource().getString("message.indexing.status"),
        com.vaadin.ui.Label.class,
        null);
    indexAdmin.addListener(
        new com.vaadin.ui.Button.ClickListener() {
          @Serial private static final long serialVersionUID = -8412830528395601232L;
          final ProgressIndicator indicator = new ProgressIndicator(0.0f);
          com.vaadin.ui.Button ok =
              new com.vaadin.ui.Button(getInstance().getResource().getString("general.ok"));

          @Override
          public void buttonClick(com.vaadin.ui.Button.ClickEvent event) {
            try {
              java.util.List result =
                  createdQuery("SELECT x FROM " + "XincoCoreData x ORDER BY x.designation");
              ...
              progress.addComponent(
                  new com.vaadin.ui.Label(
                      getInstance().getResource().getString("message.index.rebuild")));
              progress.addComponent(
                  new com.vaadin.ui.Label(
                      getInstance().getResource().getString("message.warning.index.rebuild")));
              progress.addComponent(table);
              ok.addListener(
                  (com.vaadin.ui.Button.ClickListener)
                      (com.vaadin.ui.Button.ClickEvent event1) -> {
              ...
              try {
                ...
                int work_units =
                    result.size()
                        + (indexDirectory.exists() ? indexDirectory.list().length + 1 : 0)
                        + 1;
                int index = 0;
                int count = 0;
                if (indexDirectory.exists()) {
                  indexDirectoryFileList = indexDirectory.list();
                  for (String indexDirectoryFileList1 : indexDirectoryFileList) {
                  ...
                  table.addItem(
                      new Object[] {
                        ...
                        new com.vaadin.ui.Label(
                            indexDirectoryDeleted
                                ? getInstance().getResource().getString("general.ok") + "!"
                                : getInstance().getResource().getString("general.fail") + "!")
                      },
                      index++);
                  ...
                  for (Object o : result) {
                    ...
                    if (!isRendering(xdataTemp.getId())) {
                      ...
                      table.addItem(
                          new Object[] {
                            new com.vaadin.ui.Label(xdataTemp.getDesignation()),
                            new com.vaadin.ui.Label(
                                index_result
                                    ? getInstance().getResource().getString("general.ok") + "!"
                                    : getInstance().getResource().getString("general.fail"))
                  ...
                  table.addItem(
                      new Object[] {
                        new com.vaadin.ui.Label(
                            getInstance().getResource().getString("message.index.optimize")),
                        new com.vaadin.ui.Label(
                            index_result
                                ? getInstance().getResource().getString("general.ok") + "!"
                                : getInstance().getResource().getString("general.fail"))
    ...
    auditAdmin =
        new com.vaadin.ui.Button(getInstance().getResource().getString("general.audit.menu"));
    auditAdmin.setWidth(100, UNITS_PERCENTAGE);
    auditAdmin.addListener(
        (com.vaadin.ui.Button.ClickListener)
            (com.vaadin.ui.Button.ClickEvent event) -> {
              try {
                showAuditWindow();
    ...
    settingAdmin =
        new com.vaadin.ui.Button(
            getInstance().getResource().getString("message.admin.settingAdmin"));
    settingAdmin.setWidth(100, UNITS_PERCENTAGE);
    settingAdmin.addListener(
        (com.vaadin.ui.Button.ClickListener)
            (com.vaadin.ui.Button.ClickEvent event) -> {
  ...
  private void refreshUserTable(final Table table) {
    ...
    allusers.forEach(
        (tempUser) -> {
          com.vaadin.ui.Button lock =
              new com.vaadin.ui.Button(getInstance().getResource().getString("general.lock"));
          ...
          lock.addListener(
              (com.vaadin.ui.Button.ClickListener)
                  (com.vaadin.ui.Button.ClickEvent event) -> {
                    try {
                      XincoCoreUserServer temp_user =
                          new XincoCoreUserServer((Integer) event.getButton().getData());
          ...
          com.vaadin.ui.Button unlock =
              new com.vaadin.ui.Button(getInstance().getResource().getString("general.unlock"));
  ...
  private void refreshGroupContentsTables(final Form form, final int groupId) {
    ...
    for (XincoCoreUserServer tempUser : allusers) {
      ...
      final com.vaadin.ui.Button remove =
          new com.vaadin.ui.Button(
              getInstance().getResource().getString("general.group.removeuser"));
                ...
                if (!(((groupId == 1) && ((Integer) remove.getData() == 1)) || (groupId == 2))) {
                  try {
                    java.util.List results =
                        createdQuery(
                            "SELECT x FROM XincoCoreUserHasXincoCoreGroup x "
                                + "WHERE x.xincoCoreUserHasXincoCoreGroupPK.xincoCoreUserId = "
                                + (Integer) remove.getData()
                                + " and x.xincoCoreUserHasXincoCoreGroupPK.xincoCoreGroupId = "
                                + groupId);
                    for (Object o : results) {
                      new XincoCoreUserHasXincoCoreGroupJpaController(getEntityManagerFactory())
                          .destroy(
                              ((XincoCoreUserHasXincoCoreGroup) o)
                  ...
                  getMainWindow()
                      .showNotification(
                          getInstance().getResource().getString("error.user.remove.mainUserGroup"),
                          TYPE_WARNING_MESSAGE);
                }
                refreshGroupContentsTables(form, groupId);
              });
      final com.vaadin.ui.Button add =
          new com.vaadin.ui.Button(getInstance().getResource().getString("general.group.adduser"));
              ...
              (com.vaadin.ui.Button.ClickEvent event) -> {
                try {
                  XincoCoreUserHasXincoCoreGroup uhg =
                      new XincoCoreUserHasXincoCoreGroup((Integer) add.getData(), groupId);
                  uhg.setXincoCoreGroup(
                      new XincoCoreGroupJpaController(getEntityManagerFactory())
                          .findXincoCoreGroup(groupId));
                  uhg.setXincoCoreUser(
                      new XincoCoreUserJpaController(getEntityManagerFactory())
                          .findXincoCoreUser((Integer) add.getData()));
                  new XincoCoreUserHasXincoCoreGroupJpaController(getEntityManagerFactory())
                      .create(uhg);
                } catch (Exception ex) {
                  LOG.log(SEVERE, null, ex);
    ...
    form.getLayout()
        .addComponent(
            new com.vaadin.ui.Label(getInstance().getResource().getString("general.group.member")));
    form.getLayout().addComponent(table1);
    form.getLayout()
        .addComponent(
            new com.vaadin.ui.Label(
                getInstance().getResource().getString("general.group.notmember")));
  ...
  private void refreshFileTypeAttrTable(final Table table, final int dataTypeId) {
    ArrayList<XincoCoreDataTypeAttribute> alldatatypes =
        (ArrayList<XincoCoreDataTypeAttribute>)
            getXincoCoreDataType(dataTypeId).getXincoCoreDataTypeAttributes();
    table.removeAllItems();
    alldatatypes.forEach(
        (type) -> {
          final com.vaadin.ui.Button delete =
              new com.vaadin.ui.Button(
                  getInstance().getResource().getString("general.delete") + "*");
              ...
              (com.vaadin.ui.Button.ClickListener)
                  (com.vaadin.ui.Button.ClickEvent event) -> {
                    try {
                      XincoCoreDataTypeAttributeServer tempAttribute =
                          new XincoCoreDataTypeAttributeServer(
                              dataTypeId, (Integer) delete.getData());
          ...
          table.addItem(
              new Object[] {
                ...
                type.getDataType(),
                type.getSize(),
                ((dataTypeId == 1) && (type.getAttributeId() <= 8))
                        || ((dataTypeId == 2) && (type.getAttributeId() <= 1))
                        || ((dataTypeId == 3) && (type.getAttributeId() <= 1))
                    ? null
                    : delete
              },
              type.getAttributeId());
                    ...
                    MultiFileUpload fileUpload =
                        new MultiFileUpload() {
                          ...
                          protected void handleFile(
                              File file, String fileName, String mimeType, long length) {
                            if (length == 0) {
                              ...
                              if (sb.toString().isEmpty()) {
                                sb.append(
                                    getInstance()
                                        .getResource()
                                        .getString("window.massiveimport.notsupported"));
                              }
                              sb.append("\n").append(fileName);
                              getMainWindow()
                                  .showNotification(sb.toString(), TYPE_TRAY_NOTIFICATION);
                            } else {
                              getMainWindow()
                                  .showNotification(
                                      getInstance()
                                          .getResource()
                                          .getString("window.massiveimport.progress"),
                                      TYPE_TRAY_NOTIFICATION);
                              data = new XincoCoreData();
                              getXincoCoreData()
                                  .setXincoCoreDataType(new XincoCoreDataTypeServer(1));
                              getXincoCoreData().setStatusNumber(1);
                              getXincoCoreData()
                                  .setXincoCoreLanguage(new XincoCoreLanguageServer(2));
                              getXincoCoreData()
                                  .setXincoCoreNodeId(
                                      valueOf(
                                          xincoTree
                                              ...
                                              .toString()
                                              .substring(
                                                  xincoTree.getValue().toString().indexOf('-')
                                                      + 1)));
                              addDefaultAddAttributes(getXincoCoreData());
    ...
    item =
        new XincoMenuItemBuilder()
            ...
            .setCommand(
                (com.vaadin.ui.MenuBar.MenuItem selectedItem) -> {
                  try {
                    final XincoCoreDataServer xdata =
                        new XincoCoreDataServer(
                            valueOf(
                                xincoTree
                                    .getValue()
  ...
  private void showCommentDataDialog(final XincoCoreDataServer data, final OPCode opcode)
      throws XincoException {
    final Form form = new Form();
        ...
        (com.vaadin.ui.Button.ClickListener)
            (com.vaadin.ui.Button.ClickEvent event) -> {
              try {
                ...
                String text;
                addLog(data, opcode);
                log_index1 = data.getXincoCoreLogs().size() - 1;
                XincoCoreLogServer newLog =
                    new XincoCoreLogServer(
                        ((XincoCoreLog) data.getXincoCoreLogs().get(log_index1)).getId());
                // Reason really needed only for checkin
                text =
                    newLog.getOpDescription()
                        + " "
                        + (((XincoCoreLog) data.getXincoCoreLogs().get(log_index1)).getOpCode() == 3
                            ? getInstance().getResource().getString("general.status.checkedout")
                            : form.getField("reason").getValue());
                ...
                text =
                    versionSelector.getVersion().getVersionPostfix() == null
                        ? ""
  ...
  private void showDownloadRevDialog() {
    try {
      final com.vaadin.ui.Window revWindow =
          new com.vaadin.ui.Window(getInstance().getResource().getString("window.revision"));
      final Form form = new Form();
      form.getLayout()
          .addComponent(
              new com.vaadin.ui.Label(getInstance().getResource().getString("window.revision")));
      ...
      for (i = 0; i < xdata.getXincoCoreLogs().size(); i++) {
        if ((((XincoCoreLog) xdata.getXincoCoreLogs().get(i)).getOpCode() == 1)
          ...
          Date time =
              ((XincoCoreLog) xdata.getXincoCoreLogs().get(i))
                  .getOpDatetime()
                  .toGregorianCalendar()
                  .getTime();
          ...
          cal.add(
              MILLISECOND,
              (ngc.get(ZONE_OFFSET) - realcal.toGregorianCalendar().get(ZONE_OFFSET))
                  - (ngc.get(DST_OFFSET) + realcal.toGregorianCalendar().get(DST_OFFSET)));
          ...
          text =
              text
                  ...
                  + getInstance().getResource().getString("general.version")
                  + " "
                  + ((XincoCoreLog) xdata.getXincoCoreLogs().get(i)).getVersion().getVersionHigh()
                  + "."
      ...
      form.getField("rev")
          .setRequiredError(getInstance().getResource().getString("message.missing.rev"));
      form.setFooter(new HorizontalLayout());
      // Used for validation purposes
      final com.vaadin.ui.Button commit =
          new com.vaadin.ui.Button(
          ...
          (com.vaadin.ui.Button.ClickListener)
              (com.vaadin.ui.Button.ClickEvent event) -> {
                try {
                  ...
                  int idToLook = valueOf(form.getField("rev").getValue().toString());
                  for (int i1 = 0; i1 < xdata.getXincoCoreLogs().size(); i1++) {
                    if (((XincoCoreLog) xdata.getXincoCoreLogs().get(i1)).getId() == idToLook) {
                      revLog = (XincoCoreLog) xdata.getXincoCoreLogs().get(i1);
                      break;
                    }
                  }
        ...
        new WizardStep() {
          ...
          public boolean onAdvance() {
            ...
            if (dataDialog.getLanguages().getValue() == null) {
              ...
              // Process data
              getXincoCoreData()
                  .setDesignation(dataDialog.getDesignationField().getValue().toString());
              getXincoCoreData()
                  .setXincoCoreLanguage(
                      (XincoCoreLanguage)
                          getXincoCoreLanguages()
                              .get(valueOf(dataDialog.getLanguages().getValue().toString())));
            }
            return value;
          }
                ...
                dataTypeDialog
                    ...
                    .addListener(
                        new ValueChangeListener() {
                          private static final long serialVersionUID = -5138905733637830673L;
                          ArrayList<WizardStep> temp = new ArrayList<>();
                          ...
                          public void valueChange(ValueChangeEvent event) {
                              ...
                              getXincoCoreData()
                                  .setXincoCoreDataType(
                                      new XincoCoreDataTypeServer(
                                          valueOf(
                                              dataTypeDialog.getTypes().getValue().toString())));
                            } catch (XincoException ex) {
                              LOG.log(SEVERE, null, ex);
                            }
                            ...
                            switch (getXincoCoreData().getXincoCoreDataType().getId()) {
                              ...
                              case 1:
                                ...
                                temp.add(fileStep);
                                wizard.addStep(
                                    temp.get(temp.size() - 1), wizard.getLastCompleted() + 1);
                                // revision model
                                getXincoCoreData()
                                ...
                                temp.add(
                                    new WizardStep() {
                                      ...
                                      public String getCaption() {
                                        return getInstance()
                                            .getResource()
                                            .getString("window.addattributestext");
                                      }
                                        ...
                                        if (tArea.getValue().toString().isEmpty()) {
                                          getMainWindow()
                                              .showNotification(
                                                  getInstance()
                                                      .getResource()
                                                      .getString("general.error"),
                                                  getInstance()
                                                      .getResource()
                                                      .getString("message.missing.text"),
                                                  TYPE_WARNING_MESSAGE);
                                          return false;
                                        } else {
                                          // Process the data
                                          getXincoCoreData()
                                              .getXincoAddAttributes()
                                              .get(0)
                                              .setAttribText(tArea.getValue().toString());
                                          return true;
                                        }
                                      }
                          ...
                          private void clearTempSteps() {
                            temp.stream()
                                .filter((ws) -> (wizard.getSteps().contains(ws)))
                                .forEachOrdered(
                                    (ws) -> {
  ...
  private void addLog(XincoCoreData data, OPCode code)
      throws XincoException, MalformedURLException {
    // Add log
    XincoCoreLog newlog = new XincoCoreLog();
  ...
  private void showDataFolderDialog(final boolean newFolder) {
    try {
      ...
      // ID
      com.vaadin.ui.TextField idField =
          new com.vaadin.ui.TextField(getInstance().getResource().getString("general.id") + ":");
      ...
      // Designation
      com.vaadin.ui.TextField designationField =
          new com.vaadin.ui.TextField(
              getInstance().getResource().getString("general.designation") + ":");
      ...
      // Language selection
      final Select languages =
          new Select(getInstance().getResource().getString("general.language") + ":");
      int i = 0;
        ...
        if (getInstance().getResource().containsKey(designation)) {
          ...
          if ((newFolder && ((XincoCoreLanguageServer) language).getSign().equals("en"))
              || (!newFolder
                  ...
                  && node.getXincoCoreLanguage()
                      .getDesignation()
                      .equals(designation))) // Select by default
          {
      ...
      form.getField("lang")
          .setRequiredError(getInstance().getResource().getString("message.missing.language"));
      // Status
              ...
              (com.vaadin.ui.Button.ClickEvent event) -> {
                try {
                  ...
                  if (newFolder) {
                    tempNode =
                        new XincoCoreNodeServer(
                            ...
                            ((XincoCoreLanguage)
                                    getXincoCoreLanguages()
                                        .get(valueOf(languages.getValue().toString())))
                                .getId(),
                            form.getField("designation").getValue().toString(),
                            finalStatus);
                  } else {
                    tempNode =
                        new XincoCoreNodeServer(valueOf(form.getField("id").getValue().toString()));
                    // Update with changes
                    tempNode.setDesignation(form.getField("designation").getValue().toString());
                    tempNode.setXincoCoreLanguage(
                        ((XincoCoreLanguage)
                            getXincoCoreLanguages().get(valueOf(languages.getValue().toString()))));
                  }
                  ...
                  if (getService().setXincoCoreNode(tempNode, loggedUser) == null) {
                    getMainWindow()
                        .showNotification(
                            getInstance().getResource().getString("error.accessdenied"),
    ...
    public void wizardCancelled(WizardCancelledEvent event) {
      try {
        if (newData && getXincoCoreData() != null) {
          ...
          removeFromDB(loggedUser.getId(), getXincoCoreData().getId());
        }
        ...
        try {
          if (getXincoCoreData().getXincoCoreLanguage() == null) {
            ...
            for (int j = 0; j < getXincoCoreLanguages().size(); j++) {
              if (((XincoCoreLanguage) getXincoCoreLanguages().get(j))
                      .getSign()
                      .toLowerCase()
                      .compareTo(getDefault().getLanguage().toLowerCase())
                  == 0) {
                selection = j;
                break;
              }
          ...
          addLog(
              getXincoCoreData(),
              getXincoCoreData().getXincoCoreLogs().isEmpty() ? CREATION : CHECKIN);
  ...
  private void refresh() throws XincoException {
    ...
    if (xcns != null) {
      ...
      root.getItemProperty("caption")
          .setValue(xcns.getDesignation() == null ? "" : xcns.getDesignation());
      addChildren(id);
      // Expand root node
  ...
  private void processTreeSelection(String source) {
    ...
    xeSplitPanel.setSplitPosition(getLoggedUser() != null ? 25 : 100, UNITS_PERCENTAGE);
      ...
      if (source != null && source.startsWith("node")) {
        ...
        if (xcns != null) {
          ...
          try {
            tempAce =
                checkAccess(
                    new XincoCoreUserServer(getLoggedUser().getId()),
          ...
          xincoTable.addItem(
              new Object[] {
                ...
                new com.vaadin.ui.Label("" + xcns.getId())
              },
              i++);
          xincoTable.addItem(
              new Object[] {
                getInstance().getResource().getString("general.designation"),
                new com.vaadin.ui.Label(xcns.getDesignation())
              },
              i++);
          xincoTable.addItem(
              new Object[] {
        ...
        try {
          ...
          tempAce = checkAccess(new XincoCoreUserServer(1), (ArrayList) (temp).getXincoCoreAcl());
        } catch (XincoException ex) {
          LOG.log(SEVERE, null, ex);
        }
        if (temp != null) {
          xincoTable.addItem(
              new Object[] {
                getInstance().getResource().getString("general.id"),
                new com.vaadin.ui.Label("" + temp.getId())
              },
              i++);
          xincoTable.addItem(
              new Object[] {
                getInstance().getResource().getString("general.designation"),
                new com.vaadin.ui.Label(temp.getDesignation())
              },
              i++);
          xincoTable.addItem(
              new Object[] {
                ...
                new com.vaadin.ui.Label(
                    getInstance()
                            .getResource()
                            .getString(temp.getXincoCoreLanguage().getDesignation())
                        + " ("
          ...
          xincoTable.addItem(
              new Object[] {
                ...
                new com.vaadin.ui.Label(
                    getInstance()
                            .getResource()
                            .getString(temp.getXincoCoreDataType().getDesignation())
                        + " ("
                        + getInstance()
                            .getResource()
                            .getString(temp.getXincoCoreDataType().getDescription())
                        + ")")
              },
              i++);
          xincoTable.addItem(new Object[] {"\u00a0", new com.vaadin.ui.Label()}, i++);
          ...
          java.util.List<XincoAddAttribute> attributes = temp.getXincoAddAttributes();
          ...
          if (!attributes.isEmpty()) {
            for (int j = 0; j < attributes.size(); j++) {
              ...
              if (dataTypeAttributes.get(j).getDataType().equalsIgnoreCase("datetime")) {
                ...
                cal.setTime(time);
                value =
                    (cal.get(MONTH) == 11 && cal.get(YEAR) == 2 && cal.get(DAY_OF_MONTH) == 31)
                        ? ""
                        : "" + time;
              }
                ...
                xincoTable.addItem(
                    new Object[] {
                      ...
                      header.equals("URL")
                              ...
                              && getSetting(
                                      loggedUser,
                                      "setting.allowoutsidelinks",
                                      loggedUser.isHashPassword())
                                  .isBoolValue()
                          ? link
                          : new com.vaadin.ui.Label(value)
                    },
          ...
          for (int j = temp.getXincoCoreLogs().size() - 1; j >= 0; j--) {
              ...
              try {
                ...
                cal =
                    ((XMLGregorianCalendar)
                            ((XincoCoreLog) temp.getXincoCoreLogs().get(j)).getOpDatetime().clone())
                        .toGregorianCalendar();
            ...
            String desc = ((XincoCoreLog) temp.getXincoCoreLogs().get(j)).getOpDescription();
            if (getInstance().getResource().containsKey(desc)) {
              desc = getInstance().getResource().getString(desc);
            }
            value = "(" + ((XincoCoreLog) temp.getXincoCoreLogs().get(j)).getOpCode() + ") " + desc;
            xincoTable.addItem(new Object[] {header, new com.vaadin.ui.Label(value)}, i++);
            header = "";
            try {
              value =
