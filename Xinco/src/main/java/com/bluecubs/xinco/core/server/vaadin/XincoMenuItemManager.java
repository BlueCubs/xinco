...
public class XincoMenuItemManager {
  ...
  private static boolean canAdd(XincoMenuItem item) {
    String selection =
        getInstance().getXincoTree().getValue() == null
            ? null
            : getInstance().getXincoTree().getValue().toString();
    boolean add = true;
    boolean hasAccess = false;
    XincoCoreACE tempAce = null;
    if (getInstance().getLoggedUser() != null
        && selection != null
        && selection.startsWith("node")) {
      ...
      try {
        tempAce =
            checkAccess(
                new XincoCoreUserServer(getInstance().getLoggedUser().getId()),
                (ArrayList) node.getXincoCoreAcl());
      } catch (XincoException ex) {
        getLogger(XincoMenuItemManager.class.getSimpleName()).log(SEVERE, null, ex);
      }
    } else if (getInstance().getLoggedUser() != null
        && selection != null
        && selection.startsWith("data")) {
    ...
    if (selection != null && selection.startsWith("data") && item.getDataTypes() != null) {
    ...
    if (selection != null && selection.startsWith("data") && item.getStatuses() != null) {
      XincoCoreDataServer temp = null;
      try {
        temp = new XincoCoreDataServer(valueOf(selection.substring(selection.indexOf('-') + 1)));
      } catch (XincoException ex) {
