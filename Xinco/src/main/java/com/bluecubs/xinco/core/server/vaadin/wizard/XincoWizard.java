 ...
 *
 * <p>The steps of the wizard must be implementations of the {@link WizardStep} interface. Use the
 * {@link #addStep(WizardStep)} method to add these steps in the same order they are supposed to be
 * displayed.
 *
 * <p>The wizard also supports navigation through URI fragments. This feature is disabled by
 * default, but you can enable it using setUriFragmentEnabled(boolean) method. Each step will get a
 * generated identifier that is used as the URI fragment. If you wish to override these with your
 * own identifiers, you can add the steps using the overloaded {@link #addStep(WizardStep, String)}
 * method.
 *
 * <p>To react on the progress, cancellation or completion of this {@code XincoWizard} you should
 * add one or more listeners that implement the {@link WizardProgressListener} interface. These
 * listeners are added using the {@link #addListener(WizardProgressListener)} method and removed
 * with the {@link #removeListener(WizardProgressListener)}. Based on code from Teemu P̦ntelin /
 * Vaadin Ltd
 *
...
public final class XincoWizard extends CustomComponent {
  ...
  /**
   * Cancels this XincoWizard triggering a {@link WizardCancelledEvent}. This method is called when
  ...
  /**
   * Triggers a {@link WizardCompletedEvent} if the current step is the last step and it allows
   * advancing (see {@link WizardStep#onAdvance()}). This method is called when user clicks the
   * finish button.
   */
  public void finish() {
  ...
  /**
   * Activates the next {@link WizardStep} if the current step allows advancing (see {@link
   * WizardStep#onAdvance()}) or calls the {@link #finish()} method the current step is the last
   * step. This method is called when user clicks the next button.
   */
  public void next() {
    progress++;
  ...
  /**
   * Activates the previous {@link WizardStep} if the current step allows going back (see {@link
   * WizardStep#onBack()}) and the current step is not the first step. This method is called when
  ...
  /**
   * Sets a {@link Component} that is displayed on top of the actual content. Set to {@code null} to
   * remove the header altogether.
   *
   * @param newHeader {@link Component} to be displayed on top of the actual content or {@code null}
  ...
  /**
   * Method for adding steps not at the end of the wizard steps. If you are trying to add one at the
   * end use use addStep(WizardStep step) instead.
   *
  ...
  private void activateStep(WizardStep step) {
    ...
    if (currentStep != null) {
      ...
      lastCompleted = steps.indexOf(currentStep);
      if (lastCompletedStep == null || steps.indexOf(lastCompletedStep) < lastCompleted) {
