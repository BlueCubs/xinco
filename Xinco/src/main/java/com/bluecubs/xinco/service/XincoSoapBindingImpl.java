    ...
    try {
      ...
      if (ace.isRead_permission()) {
        ...
        if ((data.getXinco_core_logs().size() > 1) && (in0.getXinco_core_logs().size() == 1)) {
          // find id of log
          int LogId = 0;
          if ((((XincoCoreLog) in0.getXinco_core_logs().elementAt(0)).getOp_code() == 1)
              || (((XincoCoreLog) in0.getXinco_core_logs().elementAt(0)).getOp_code() == 5)) {
            ...
            new CheckedInputStream(
                new FileInputStream(
                    XincoCoreDataServer.getXincoCoreDataPath(
                        dbm.config.FileRepositoryPath, data.getId(), data.getId() + revision)),
                new CRC32());
        if (useSAAJ) {
      ...
      for (int i = 0; i < tv.size(); i++) {
        XincoCoreACE ace =
            XincoCoreACEServer.checkAccess(
                user, ((XincoCoreData) tv.elementAt(i)).getXinco_core_acl());
        if (ace.isRead_permission()) {
      ...
      for (int i = 0; i < tv2.size(); i++) {
        tvp =
            XincoCoreNodeServer.getXincoCoreNodeParents(
                ((XincoCoreData) tv2.elementAt(i)).getXinco_core_node_id(), dbm);
        rp = true;
        for (int j = 0; j < tvp.size(); j++) {
          XincoCoreACE ace =
              XincoCoreACEServer.checkAccess(
                  user, ((XincoCoreNode) tvp.elementAt(j)).getXinco_core_acl());
          if (!ace.isRead_permission()) {
            rp = false;
            break;
          }
    ...
    try {
      ...
      if ((ace.isWrite_permission()) && (parent_ace.isWrite_permission())) {
        ...
        if (insertnewnode) {
          ...
          // load new ACL
          node.setXinco_core_acl(
              XincoCoreACEServer.getXincoCoreACL(node.getId(), "xinco_core_node_id", dbm));
        }
        dbm.con.close();
        return (XincoCoreNode) node;
      } else {
    ...
    try {
      ...
      if ((ace.isWrite_permission()) && (parent_ace.isWrite_permission())) {
        ...
        if (insertnewdata) {
          ...
          // load new ACL
          data.setXinco_core_acl(
              XincoCoreACEServer.getXincoCoreACL(data.getId(), "xinco_core_data_id", dbm));
        }
        dbm.con.close();
        return (XincoCoreData) data;
      } else {
