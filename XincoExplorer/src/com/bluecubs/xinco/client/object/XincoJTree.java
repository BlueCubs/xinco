...
public final class XincoJTree extends JTree
{
  ...
  public XincoJTree(XincoExplorer explorer)
  {
    ...
    addTreeSelectionListener(new javax.swing.event.TreeSelectionListener()
    {
      ...
      {
        getExplorer().getProgressBar().setTitle(getExplorer()
                .getResourceBundle().getString("message.explorer.verifyingcredentials"));
        ...
        {
          tempAce = XincoCoreACEClient.checkAccess(getExplorer().getSession()
                  .getUser(), ((XincoCoreNode) node.getUserObject()).getXinco_core_acl());
        }
        if (node.getUserObject().getClass() == XincoCoreData.class)
        {
          tempAce = XincoCoreACEClient.checkAccess(getExplorer().getSession()
                  .getUser(), ((XincoCoreData) node.getUserObject()).getXinco_core_acl());
        }
        // Intelligent menu
        // reset menus
        ...
        if (node.getUserObject().getClass() == XincoCoreData.class)
        {
          ...
          if (getExplorer().getXdata().getXinco_add_attributes().size() > 0)
          {
            ...
            {
              if (i < getExplorer().getXdata().getXinco_core_data_type()
                      .getXinco_core_data_type_attributes().size())
              {
                rdata[0] = ((XincoCoreDataTypeAttribute) getExplorer().getXdata()
                        .getXinco_core_data_type().getXinco_core_data_type_attributes().
                        elementAt(i)).getDesignation();
                if (((XincoCoreDataTypeAttribute) getExplorer().getXdata()
                        .getXinco_core_data_type().getXinco_core_data_type_attributes().
                        elementAt(i)).getData_type().equalsIgnoreCase("int"))
                {
                  rdata[1] = ""
                          + ((XincoAddAttribute) getExplorer().getXdata()
                                  .getXinco_add_attributes().elementAt(i)).getAttrib_int();
                }
                if (((XincoCoreDataTypeAttribute) getExplorer().getXdata()
                        .getXinco_core_data_type().getXinco_core_data_type_attributes().
                        elementAt(i)).getData_type().equalsIgnoreCase("unsignedint"))
                {
                  rdata[1] = ""
                          + ((XincoAddAttribute) getExplorer().getXdata()
                ...
                if (((XincoCoreDataTypeAttribute) getExplorer().getXdata()
                        .getXinco_core_data_type().getXinco_core_data_type_attributes().
                        elementAt(i)).getData_type().equalsIgnoreCase("double"))
                {
                  rdata[1] = ""
                          + ((XincoAddAttribute) getExplorer().getXdata()
                                  .getXinco_add_attributes().elementAt(i)).getAttrib_double();
                }
                if (((XincoCoreDataTypeAttribute) getExplorer().getXdata()
                        .getXinco_core_data_type().getXinco_core_data_type_attributes().
                        elementAt(i)).getData_type().equalsIgnoreCase("varchar"))
                {
                  rdata[1] = ""
                          + ((XincoAddAttribute) getExplorer().getXdata()
                                  .getXinco_add_attributes().elementAt(i)).getAttrib_varchar();
                }
                if (((XincoCoreDataTypeAttribute) getExplorer().getXdata()
                        .getXinco_core_data_type().getXinco_core_data_type_attributes().
                        elementAt(i)).getData_type().equalsIgnoreCase("text"))
                {
                  rdata[1] = ""
                          + ((XincoAddAttribute) getExplorer().getXdata()
                                  .getXinco_add_attributes().elementAt(i)).getAttrib_text();
                }
                if (((XincoCoreDataTypeAttribute) getExplorer().getXdata()
                        .getXinco_core_data_type().getXinco_core_data_type_attributes().
                        elementAt(i)).getData_type().equalsIgnoreCase("datetime"))
                {
                  Date time = ((XincoAddAttribute) getExplorer().getXdata()
                          .getXinco_add_attributes().elementAt(i)).getAttrib_datetime().getTime();
          ...
          {
            if (((XincoCoreLog) getExplorer().getXdata().getXinco_core_logs()
                    .elementAt(i)).getOp_datetime()
                    != null)
            {
              try
              {
                // convert clone from remote time to local time
                cal = (Calendar) ((XincoCoreLog) getExplorer().getXdata()
                        .getXinco_core_logs().elementAt(i)).getOp_datetime().clone();
                realcal = (((XincoCoreLog) getExplorer().getXdata()
                        .getXinco_core_logs().elementAt(i)).getOp_datetime());
            ...
            try
            {
              rdata[1] = getExplorer().getResourceBundle().getString("general.version")
                      + " "
                      + ((XincoCoreLog) getExplorer().getXdata().getXinco_core_logs()
                              .elementAt(i)).getVersion().getVersion_high()
                      + "."
                      + ((XincoCoreLog) getExplorer().getXdata().getXinco_core_logs()
                              .elementAt(i)).getVersion().getVersion_mid()
                      + "."
                      + ((XincoCoreLog) getExplorer().getXdata().getXinco_core_logs()
                              .elementAt(i)).getVersion().getVersion_low()
                      + ""
                      + ((XincoCoreLog) getExplorer().getXdata().getXinco_core_logs()
                              .elementAt(i)).getVersion().getVersion_postfix();
            } catch (Exception ex)
            {
              Logger.getLogger(XincoJTree.class.getSimpleName()).log(
        ...
        if (selRow != -1)
        {
          ...
          } else if (e.getClickCount() == 2)
          {
            ...
            if (getExplorer().getSession().getCurrentTreeNodeSelection().getUserObject().getClass()
                    == XincoCoreData.class)
            {
              if (((XincoCoreData) getExplorer().getSession()
                      .getCurrentTreeNodeSelection().getUserObject()).getStatus_number()
              ...
              // file = 1
              if (((XincoCoreData) getExplorer().getSession()
                      .getCurrentTreeNodeSelection().getUserObject())
                      .getXinco_core_data_type().getId() == 1)
              {
              ...
              if (((XincoCoreData) getExplorer().getSession()
                      .getCurrentTreeNodeSelection().getUserObject())
                      .getXinco_core_data_type().getId() == 2)
              {
