 ...
 * <p>************************************************************ ACLDialog.java
 *
 * <p>Created on October 31, 2006, 8:21 AM
 */
...
public final class ACLDialog extends AbstractDialog {
  ...
  public ACLDialog(java.awt.Frame parent, boolean modal, XincoExplorer explorer) {
    ...
    this.aclRemoveLabel.setText(
        explorer.getResourceBundle().getString("window.acl.removeacelabel"));
  ...
  protected void loadACLGroupListACL() {
    ...
    for (int i = 0; i < this.explorer.getSession().getServerGroups().size(); i++) {
      list[i] =
          ((XincoCoreGroup) this.explorer.getSession().getServerGroups().elementAt(i))
              .getDesignation();
      try {
  ...
  public void reloadACLListACL() {
    ...
    if (this.explorer.getSession().getCurrentTreeNodeSelection().getUserObject().getClass()
        == XincoCoreNode.class) {
      temp_vector =
          ((XincoCoreNode) this.explorer.getSession().getCurrentTreeNodeSelection().getUserObject())
              .getXinco_core_acl();
    }
    if (this.explorer.getSession().getCurrentTreeNodeSelection().getUserObject().getClass()
        == XincoCoreData.class) {
      temp_vector =
          ((XincoCoreData) this.explorer.getSession().getCurrentTreeNodeSelection().getUserObject())
              .getXinco_core_acl();
    }
    if (temp_acl != null) {
    ...
    for (i = 0; i < temp_vector.size(); i++) {
      ...
      if (temp_ace.getXinco_core_user_id() > 0) {
        temp_string =
            this.explorer.getResourceBundle().getString("general.user")
                + ": "
      ...
      if (temp_ace.getXinco_core_group_id() > 0) {
        for (j = 0; j < this.explorer.getSession().getServerGroups().size(); j++) {
          if (((XincoCoreGroup) this.explorer.getSession().getServerGroups().elementAt(j)).getId()
              == temp_ace.getXinco_core_group_id()) {
            temp_string = this.explorer.getResourceBundle().getString("general.group") + ": ";
            String group =
                ((XincoCoreGroup) this.explorer.getSession().getServerGroups().elementAt(j))
                    .getDesignation();
            try {
  ...
  private void RemoveACEActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_RemoveACEActionPerformed
    if (this.currentACLList.getSelectedIndex() >= 0) {
      try {
        ...
        if (this.explorer.getSession().getCurrentTreeNodeSelection().getUserObject().getClass()
          ...
          temp_acl =
              ((XincoCoreNode)
                      this.explorer.getSession().getCurrentTreeNodeSelection().getUserObject())
                  .getXinco_core_acl();
        ...
        if (temp_ace.getXinco_core_user_id() > 0) {
          throw new XincoException(
              this.explorer.getResourceBundle().getString("window.acl.cannotremoveowner"));
        }
        if (!this.explorer
            .getSession()
            .getXinco()
            .removeXincoCoreACE(temp_ace, this.explorer.getSession().getUser())) {
          throw new XincoException(
              this.explorer.getResourceBundle().getString("error.noadminpermission"));
        }
        ...
        JOptionPane.showMessageDialog(
            this,
            this.explorer.getResourceBundle().getString("window.acl.removefailed")
                + " "
                + this.explorer.getResourceBundle().getString("general.reason")
                + ": "
                + xe.toString(),
            this.explorer.getResourceBundle().getString("general.error"),
  ...
  private void AddACEActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_AddACEActionPerformed
    int i = 0;
    temp_acl = new Vector();
    if (this.groupList.getSelectedIndex() >= 0) {
      try {
        ...
        for (i = 0; i < temp_acl.size(); i++) {
          if (((XincoCoreACE) temp_acl.elementAt(i)).getXinco_core_group_id()
              == ((XincoCoreGroup)
                      this.explorer
                          .getSession()
                          .getServerGroups()
                          .elementAt(this.groupList.getSelectedIndex()))
                  .getId()) {
            throw new XincoException(
                this.explorer.getResourceBundle().getString("window.acl.groupexists"));
          }
        }
        // create new ACE
        XincoCoreACE newace = new XincoCoreACE();
        newace.setXinco_core_group_id(
            ((XincoCoreGroup)
                    this.explorer
                        .getSession()
                        .getServerGroups()
        ...
        if ((newace =
                this.explorer
                    .getSession()
                    .getXinco()
                    .setXincoCoreACE(newace, this.explorer.getSession().getUser()))
            == null) {
          throw new XincoException(
              this.explorer.getResourceBundle().getString("error.noadminpermission"));
        }
        ...
        JOptionPane.showMessageDialog(
            this,
            this.explorer.getResourceBundle().getString("window.acl.addacefailed")
                + " "
                + this.explorer.getResourceBundle().getString("general.reason")
                + ": "
                + xe.toString(),
