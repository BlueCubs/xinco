 ...
 * <p>************************************************************ SearchDialog.java
 *
 * <p>Created on January 27, 2010, 8:45 AM
 */
...
public class SearchDialog extends AbstractDialog {
  ...
  public SearchDialog(java.awt.Frame parent, boolean modal, XincoExplorer e) {
    ...
    String[] cn = {
      xerb.getString("window.search.table.designation"), xerb.getString("window.search.table.path")
    };
    resultTable.setModel(
        new DefaultTableModel(
            new Object[][] {},
            new String[] {
              xerb.getString("window.search.table.designation"),
  ...
  public void setToDefaults() {
    ...
    for (i = 0; i < explorer.getSession().getServerDatatypes().size(); i++) {
      ...
      for (j = 0; j < xcdt.getXinco_core_data_type_attributes().size(); j++) {
        text =
            ((XincoCoreDataTypeAttribute) xcdt.getXinco_core_data_type_attributes().elementAt(j))
                .getDesignation();
    ...
    for (i = 0; i < explorer.getSession().getServerLanguages().size(); i++) {
      text =
          ((XincoCoreLanguage) explorer.getSession().getServerLanguages().elementAt(i))
                  .getDesignation()
              + " ("
              + ((XincoCoreLanguage) explorer.getSession().getServerLanguages().elementAt(i))
                  .getSign()
              + ")";
      list.add(text);
      if (((XincoCoreLanguage) explorer.getSession().getServerLanguages().elementAt(i))
              .getSign()
              .toLowerCase()
      ...
      if (((XincoCoreLanguage) explorer.getSession().getServerLanguages().elementAt(i)).getId()
          == 1) {
        alt_selection = i;
      }
    }
  ...
  private void initComponents() {
    ...
    allLanguagesCheckBox.addItemListener(
    ...
    goToSelectionButton.addActionListener(
        new java.awt.event.ActionListener() {
          public void actionPerformed(java.awt.event.ActionEvent evt) {
  ...
  private void goToSelectionButtonActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_goToSelectionButtonActionPerformed
    if (resultTable.getSelectedRow() < 0) {
      return;
    }
    Vector v =
        (Vector)
            explorer.getSession().getCurrentSearchResult().elementAt(resultTable.getSelectedRow());
    int i = 0;
    int j = 0;
    int k = 0;
    TreePath tp = null;
    try {
      ...
      if (xmtn.getUserObject().getClass() == XincoCoreNode.class) {
        if (((XincoCoreNode) xmtn.getUserObject()).getId()
            == ((XincoCoreNode) v.elementAt(1)).getId()) {
          ...
          if (1 == (v.size() - 1)) {
            for (k = 0; k < xmtn.getChildCount(); k++) {
              if (((XincoMutableTreeNode) xmtn.getChildAt(k)).getUserObject().getClass()
                  == XincoCoreData.class) {
                if (((XincoCoreData) ((XincoMutableTreeNode) xmtn.getChildAt(k)).getUserObject())
                        .getId()
                    == ((XincoCoreData) v.elementAt(0)).getId()) {
      ...
      for (i = 2; i < v.size(); i++) {
        for (j = 0; j < xmtn.getChildCount(); j++) {
          if (((XincoMutableTreeNode) xmtn.getChildAt(j)).getUserObject().getClass()
              == XincoCoreNode.class) {
            if (((XincoCoreNode) ((XincoMutableTreeNode) xmtn.getChildAt(j)).getUserObject())
                    .getId()
                == ((XincoCoreNode) v.elementAt(i)).getId()) {
              ...
              if (i == (v.size() - 1)) {
                for (k = 0; k < xmtn.getChildCount(); k++) {
                  if (((XincoMutableTreeNode) xmtn.getChildAt(k)).getUserObject().getClass()
                      == XincoCoreData.class) {
                    if (((XincoCoreData)
                                ((XincoMutableTreeNode) xmtn.getChildAt(k)).getUserObject())
                            .getId()
                        == ((XincoCoreData) v.elementAt(0)).getId()) {
                      tp = new TreePath(((XincoMutableTreeNode) xmtn.getChildAt(k)).getPath());
                      explorer.jTreeRepository.setSelectionPath(tp);
  ...
  private void allLanguagesCheckBoxItemStateChanged(
      java.awt.event.ItemEvent evt) { // GEN-FIRST:event_allLanguagesCheckBoxItemStateChanged
  ...
  private void addToQueryButtonActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_addToQueryButtonActionPerformed
    String operator = "";
    String field = "";
    ...
    // append to query
    this.queryValueField.setText(
        this.queryValueField.getText() + operator + field + this.variableField.getText() + " ");
    this.variableField.setText("");
  } // GEN-LAST:event_addToQueryButtonActionPerformed

  private void searchButtonActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_searchButtonActionPerformed
  ...
  private class searchThread extends Thread {
    ...
    public void run() {
        ...
        lid =
            (XincoCoreLanguage)
                explorer
                    .getSession()
                    .getServerLanguages()
                    .elementAt(languageList.getSelectedIndex());
      }
      try {
        explorer
            .getSession()
            .setCurrentSearchResult(
                explorer
                    ...
                    .getXinco()
                    .findXincoCoreData(
                        queryValueField.getText(), lid, explorer.getSession().getUser()));
      ...
      for (i = 0; i < explorer.getSession().getCurrentSearchResult().size(); i++) {
        rdata[0] =
            ((XincoCoreData)
                        (((Vector) explorer.getSession().getCurrentSearchResult().elementAt(i))
                            .elementAt(0)))
                    .getDesignation()
                + " ("
                + ((XincoCoreData)
                ...
                + ((XincoCoreData)
                        (((Vector) explorer.getSession().getCurrentSearchResult().elementAt(i))
                            .elementAt(0)))
                    .getXinco_core_language()
                    .getSign()
                + ")";
        rdata[1] = new String("");
        for (j = 1;
            j < ((Vector) explorer.getSession().getCurrentSearchResult().elementAt(i)).size();
            j++) {
          rdata[1] =
              rdata[1]
                  + ((XincoCoreNode)
