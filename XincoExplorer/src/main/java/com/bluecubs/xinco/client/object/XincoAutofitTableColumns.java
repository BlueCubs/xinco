 ...
 *
 * <p>Name: AutoFitTableColumns
 *
 * <p>Description: Auto Fit Table Columns
 *
 ...
 * <p>Who? When? What?
 *
 * <p>************************************************************ AutoFitTableColumns.java
 *
 * <p>Created on December 19, 2006, 2:05 PM
 */
...
public class XincoAutofitTableColumns {
   ...
   * @returns The table width, just in case the caller wants it...
   */
  protected static int autoResizeTable(
      JTable aTable, boolean includeColumnHeaderWidth, int columnPadding) {
    ...
    if (columnCount > 0) // must have columns !
    {
      ...
      if (rowHeight == 0) rowHeight = 16; // default rowheight

      // System.out.println ("Row Height : " + rowHeight );

      dim.height =
          headerDim.height + ((rowHeight + interCellSpacing.height) * aTable.getRowCount());
      dim.width = tableWidth;

      // System.out.println ("XincoAutofitTableColumns.autoResizeTable() - Setting Table size to ( "
  ...
  private static int getMaxColumnWidth(
      JTable aTable, int columnNo, boolean includeColumnHeaderWidth, int columnPadding) {
      ...
      if (headerRenderer != null) {
        comp =
            headerRenderer.getTableCellRendererComponent(
                aTable, column.getHeaderValue(), false, false, 0, columnNo);
    ...
    for (int i = 0; i < aTable.getRowCount(); i++) {
      ...
      comp =
          tableCellRenderer.getTableCellRendererComponent(
              aTable, aTable.getValueAt(i, columnNo), false, false, i, columnNo);
