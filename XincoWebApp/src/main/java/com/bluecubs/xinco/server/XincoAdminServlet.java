 ...
 *
 * <p>Name: XincoAdminServlet
 *
 * <p>Description: administration servlet
 *
 ...
 * <p>Who? When? What?
 *
 * <p>Alexander Manes 11/01/2006 Bugfix: Remove user from selected group (not from all groups!)
 *
 * <p>Javier A. Ortiz 09/20/2006 Modified processRequest in order to reflect changes to
 * XincoCoreUser and XincoCoreUserServer for FDA 21 CFR regulation compliance.
 *
 * <p>Javier A. Ortiz 09/22/2006 Changed the text saved into the audit trail to the actual key entry
 * name in the XincoExplorer.properties file so the reason can be displayed in the selected
 * language!
 *
 * <p>Javier A. Ortiz 11/06/2006 Bugfix: java.lang.NumberFormatException: null when account is
 * locked, requesting a null parameter.
 *
 * <p>Javier A. Ortiz 01/08/2007 Bugfix: java.lang.NullPointerException: null when changing password
 * ************************************************************
 */
...
public class XincoAdminServlet extends HttpServlet {
  ...
  protected synchronized void processRequest(
    ...
    if (session.getAttribute("XincoAdminServlet.status") == null) {
      status = 0;
      session.setAttribute("XincoAdminServlet.status", status);
      ...
      session.setAttribute("XincoAdminServlet.current_user_selection", current_user_selection);
      current_group_selection = 0;
      session.setAttribute("XincoAdminServlet.current_group_selection", current_group_selection);
      current_datatype_selection = 0;
      session.setAttribute(
          "XincoAdminServlet.current_datatype_selection", current_datatype_selection);
    } else {
      status = ((Integer) session.getAttribute("XincoAdminServlet.status"));
      current_location = ((String) session.getAttribute("XincoAdminServlet.current_location"));
      current_location_desc =
          ((String) session.getAttribute("XincoAdminServlet.current_location_desc"));
      current_user_selection =
          ((Integer) session.getAttribute("XincoAdminServlet.current_user_selection"));
      current_group_selection =
          ((Integer) session.getAttribute("XincoAdminServlet.current_group_selection"));
      current_datatype_selection =
          ((Integer) session.getAttribute("XincoAdminServlet.current_datatype_selection"));
      if (status == 0) {
        current_location = "MainLogin";
        session.setAttribute("XincoAdminServlet.current_location", current_location);
        current_location_desc = rb.getString("message.location.desc.login");
    ...
    if (request.getParameter("DialogLoginSubmit") != null) {
      try {
        try {
          ...
          // Know who's logged in as administrator since temp_user might be used for other purposes
          // later
          ...
          ResultSet rs =
              stmt.executeQuery(
                  "SELECT id FROM xinco_core_user WHERE username='"
                      + request.getParameter("DialogLoginUsername")
                      + "' AND status_number<>2");
      ...
      try {
        ...
        temp_user =
            new XincoCoreUserServer(
                ...
                request.getParameter("DialogNewUserEmail"),
                1,
                0,
    ...
    if (request.getParameter("DialogEditGroupRemoveUser") != null) {
      ...
      if (!(((current_group_selection == 1)
              && (Integer.parseInt(request.getParameter("DialogEditGroupRemoveUser")) == 1))
          || (current_group_selection == 2))) {
        try {
          try (Statement stmt = dbm.con.createStatement()) {
            stmt.executeUpdate(
                "DELETE FROM xinco_core_user_has_xinco_core_group WHERE xinco_core_user_id="
                    + Integer.valueOf(request.getParameter("DialogEditGroupRemoveUser"))
                    + " AND xinco_core_group_id="
                    + current_group_selection);
          }
          dbm.con.commit();
        } catch (Exception e) {
    ...
    if (request.getParameter("DialogEditGroupAddUser") != null) {
      try {
        try (Statement stmt = dbm.con.createStatement()) {
          stmt.executeUpdate(
              "INSERT INTO xinco_core_user_has_xinco_core_group VALUES ("
                  + Integer.valueOf(request.getParameter("DialogEditGroupAddUser"))
                  + ", "
                  + current_group_selection
                  + ", "
                  + "1)");
    ...
    if (request.getParameter("DialogEditUserProfileSubmit") != null) {
      try {
        temp_user =
            new XincoCoreUserServer(
                Integer.parseInt(request.getParameter("DialogEditUserProfileID")), dbm);
    ...
    if (request.getParameter("DialogAdminLanguagesDelete") != null) {
      try {
        temp_language =
            new XincoCoreLanguageServer(
                Integer.parseInt(request.getParameter("DialogAdminLanguagesDelete")), dbm);
    ...
    if (request.getParameter("DialogAdminDataTypeSelect") != null) {
      current_datatype_selection =
          Integer.parseInt(request.getParameter("DialogAdminDataTypeSelect"));
      session.setAttribute(
          "XincoAdminServlet.current_datatype_selection",
          Integer.valueOf(current_datatype_selection));
    ...
    if (request.getParameter("changePassword") != null) {
      ...
      try {
        ...
        sql =
            "select id from xinco_core_user where username='"
                + request
                    .getParameter("user")
                    .substring(0, request.getParameter("user").length() - 1)
                + "'";
      ...
      // issue # 2994751: Unable to reproduce but this should take care of the NPE at this line
      // atleast.
      boolean passwordIsUsable =
          temp_user == null ? false : temp_user.isPasswordUsable(request.getParameter("confirm"));
      if (!request.getParameter("new").equals(request.getParameter("confirm"))) {
        ...
        out.println(
            rb.getString("password.noMatch")
                + "<br><br>"
                + "<input type='submit' value='"
                + rb.getString("general.continue")
                + "' name='changePassword' />");
        out.println(
      ...
      if (!passwordIsUsable) {
        ...
        out.println("<form name='changePassword' action='changePassword.jsp' method='post'>");
        out.println(
            rb.getString("password.unusable")
                + "<br><br>"
                + "<input type='submit' value='"
                + rb.getString("general.continue")
                + "' name='changePassword' />");
        ...
        try {
          ...
          // The logged in admin does the locking if none loged in the default admin does the
          // locking
          if (login_user == null) {
            temp_user.setChangerID(1);
          } else {
    ...
    out.println(
        "<body "
            + (!dbm.config.isAllowOutsideLinks() ? "oncontextmenu='return false;' " : " ")
    ...
    switch (status) {
      case 0 -> {
        ...
        out.println(
            "<td class=\"text\"><input type=\"text\" name=\"DialogLoginUsername\""
                + " size=\"40\"/></td>");
        out.println("</tr>");
        out.println("<tr>");
        out.println("<td class=\"text\">" + rb.getString("general.password") + ":</td>");
        out.println(
            "<td class=\"text\"><input type=\"password\" name=\"DialogLoginPassword\""
                + " size=\"40\"/></td>");
        out.println("</tr>");
        out.println("<tr>");
        out.println("<td class=\"text\">&nbsp;</td>");
        out.println(
            "<td class=\"text\"><input type=\"submit\" name=\"DialogLoginSubmit\" value=\""
                + rb.getString("general.login")
                + "\"/>"
                + "<input type='hidden' name='list' value='"
                + request.getParameter("list")
                + "'/></td>");
      ...
      default -> {
        ...
        out.println("<tr>");
        out.println(
            "<td class=\"text\" rowspan=\"3\"><img src='resources/images/blueCubsSmall.gif'"
                + " border=\"0\"/></td>");
        ...
        out.println("<tr>");
        out.println(
            "<td class=\"text\"><a href=\"XincoAdmin?MenuMainOverview=Overview&list="
                + request.getParameter("list")
                + "\" class=\"link\">"
                + rb.getString("message.admin.overview")
                + "</a></td>");
        out.println("<td class=\"text\">|</td>");
        out.println(
            "<td class=\"text\"><a href=\"XincoAdmin?MenuMainAdminUsers=AdminUsers&list="
                + request.getParameter("list")
                + "\" class=\"link\">"
                + rb.getString("message.admin.userAdmin")
                + "</a></td>");
        out.println("<td class=\"text\">|</td>");
        out.println(
            "<td class=\"text\"><a href=\"XincoAdmin?MenuMainAdminGroups=AdminGroups&list="
                + request.getParameter("list")
                + "\" class=\"link\">"
                + rb.getString("message.admin.groupAdmin")
                + "</a></td>");
        out.println("<td class=\"text\">|</td>");
        out.println(
            "<td class=\"text\"><a href=\"XincoAdmin?MenuMainEditUserProfile=EditProfile&list="
                + request.getParameter("list")
                + "\" class=\"link\">"
                + rb.getString("message.admin.userProfile")
                + "</a></td>");
        out.println("<td class=\"text\">|</td>");
        out.println(
            "<td class=\"text\"><a href=\"XincoAdmin?MenuMainAdminLanguages=AdminLanguages&list="
                + request.getParameter("list")
                + "\" class=\"link\">"
                + rb.getString("message.admin.language")
                + "</a></td>");
        out.println("<td class=\"text\">|</td>");
        out.println(
            "<td class=\"text\"><a href=\"XincoAdmin?MenuMainAdminAttributes=AdminAttributes&list="
                + request.getParameter("list")
                + "\" class=\"link\">"
                + rb.getString("message.admin.attribute")
                + "</a></td>");
        out.println("<td class=\"text\">|</td>");
        out.println(
            "<td class=\"text\"><a href=\"XincoAdmin?MenuMainEmptyTrash=EmptyTrash&list="
                + request.getParameter("list")
                + "\" class=\"link\">"
                + rb.getString("message.admin.trash")
                + "</a></td>");
        out.println("<td class=\"text\">|</td>");
        out.println(
            "<td class=\"text\"><a href=\"XincoAdmin?MenuMainRebuildIndex=RebuildIndex&list="
                + request.getParameter("list")
                + "\" class=\"link\">"
                + rb.getString("message.admin.index")
                + "</a></td>");
        out.println("<td class=\"text\">|</td>");
        out.println(
            "<td class=\"text\"><a href=\"XincoAdmin?MenuAudit=AuditMenu&list="
                + request.getParameter("list")
                + "\" class=\"link\">"
                + rb.getString("general.audit.menu")
                + "</a></td>");
        out.println("<td class=\"text\">|</td>");
        out.println(
            "<td class=\"text\"><a href=\"XincoAdmin?MenuMainLogout=Logout&list="
                + request.getParameter("list")
                + "\" class=\"link\">"
                + rb.getString("general.logout")
                + "</a></td>");
        out.println("</tr>");
        out.println("</table>");
        out.println("<br><br>");
        if (current_location.compareTo("MainMenu") == 0) {
          ...
          out.println(
              "<td class=\"text\">" + rb.getString("message.admin.overview.message") + "</td>");
          out.println("</tr>");
          out.println("<tr>");
          out.println("<td class=\"bigtext\">" + rb.getString("message.admin.userAdmin") + "</td>");
          out.println(
              "<td class=\"text\">" + rb.getString("message.admin.userAdmin.message") + "</td>");
          out.println("</tr>");
          out.println("<tr>");
          out.println(
              "<td class=\"bigtext\">" + rb.getString("message.admin.groupAdmin") + "</td>");
          out.println(
              "<td class=\"text\">" + rb.getString("message.admin.groupAdmin.message") + "</td>");
          out.println("</tr>");
          out.println("<tr>");
          out.println(
              "<td class=\"bigtext\">" + rb.getString("message.admin.userProfile") + "</td>");
          out.println(
              "<td class=\"text\">" + rb.getString("message.admin.userProfile.message") + "</td>");
          out.println("</tr>");
          out.println("<tr>");
          out.println("<td class=\"bigtext\">" + rb.getString("message.admin.language") + "</td>");
          out.println(
              "<td class=\"text\">" + rb.getString("message.admin.language.message") + "</td>");
          out.println("</tr>");
          out.println("<tr>");
          out.println("<td class=\"bigtext\">" + rb.getString("message.admin.attribute") + "</td>");
          out.println(
              "<td class=\"text\">" + rb.getString("message.admin.attribute.message") + "</td>");
          out.println("</tr>");
          out.println("<tr>");
          out.println("<td class=\"bigtext\">" + rb.getString("message.admin.trash") + "</td>");
          out.println(
              "<td class=\"text\">" + rb.getString("message.admin.trash.message") + "</td>");
          out.println("</tr>");
          out.println("<tr>");
          out.println("<td class=\"bigtext\">" + rb.getString("message.admin.index") + "</td>");
          out.println(
              "<td class=\"text\">" + rb.getString("message.admin.index.message") + "</td>");
          out.println("</tr>");
          out.println("</table>");
        }
        if (current_location.compareTo("UserAdmin") == 0) {
          ...
          out.println("<td class=\"text\">" + rb.getString("general.password") + ":</td>");
          out.println(
              "<td class=\"text\"><input type=\"password\" name=\"DialogNewUserPassword\""
                  + " size=\"40\"/></td>");
          out.println("</tr>");
          out.println("<tr>");
          out.println("<td class=\"text\">" + rb.getString("general.firstname") + ":</td>");
          out.println(
              "<td class=\"text\"><input type=\"text\" name=\"DialogNewUserFirstname\""
                  + " size=\"40\"/></td>");
          out.println("</tr>");
          out.println("<tr>");
          out.println("<td class=\"text\">" + rb.getString("general.lastname") + ":</td>");
          out.println(
              "<td class=\"text\"><input type=\"text\" name=\"DialogNewUserLastname\""
                  + " size=\"40\"/></td>");
          out.println("</tr>");
          out.println("<tr>");
          out.println("<td class=\"text\">" + rb.getString("general.email") + ":</td>");
          out.println(
              "<td class=\"text\"><input type=\"text\" name=\"DialogNewUserEmail\""
                  + " size=\"40\"/></td>");
          out.println("</tr>");
          out.println("<tr>");
          out.println("<td class=\"text\">&nbsp;</td>");
          out.println(
              "<td class=\"text\"><input type='hidden' name='list' value='"
                  + request.getParameter("list")
                  + "'/><input type=\"submit\" name=\"DialogNewUserSubmit\" value=\""
                  + rb.getString("general.add.user")
                  + "\"/></td>");
          out.println("</tr>");
          out.println("</table>");
          out.println("</form>");
          ...
          for (i = 0; i < allusers.size(); i++) {
            ...
            out.println(
                "<td class=\"text\">"
                    + ((XincoCoreUserServer) allusers.elementAt(i)).getUsername()
                    + "</td>");
            out.println(
                "<td class=\"text\">"
                    + ((XincoCoreUserServer) allusers.elementAt(i)).getFirstname()
                    + "</td>");
            out.println(
                "<td class=\"text\">"
                    + ((XincoCoreUserServer) allusers.elementAt(i)).getName()
                    + "</td>");
            out.println(
                "<td class=\"text\">"
                    + ((XincoCoreUserServer) allusers.elementAt(i)).getEmail()
                    + "</td>");
            if (((XincoCoreUserServer) allusers.elementAt(i)).getStatus_number() == 1) {
              out.println(
                  "<td class=\"text\"><a href=\"XincoAdmin?DialogAdminUsersLock="
                      + ((XincoCoreUserServer) allusers.elementAt(i)).getId()
                      + "&list="
                      + request.getParameter("list")
                      + "\" class=\"link\">["
                      + rb.getString("general.lock")
                      + "]</a>&nbsp;<a href=\"XincoAdmin?DialogAdminUsersResetPW="
                      + ((XincoCoreUserServer) allusers.elementAt(i)).getId()
                      + "&list="
                      + request.getParameter("list")
                      + "\" class=\"link\">["
                      + rb.getString("general.password.reset")
                      + "*]</a></td>");
            }
            if (((XincoCoreUserServer) allusers.elementAt(i)).getStatus_number() == 2) {
              out.println(
                  "<td class=\"text\"><b>"
                      + rb.getString("general.status.locked")
                      + "</b> <a href=\"XincoAdmin?DialogAdminUsersUnlock="
                      + ((XincoCoreUserServer) allusers.elementAt(i)).getId()
                      + "&list="
                      + request.getParameter("list")
                      + "\" class=\"link\">["
                      + rb.getString("general.unlock")
                      + "]</a>&nbsp;<a href=\"XincoAdmin?DialogAdminUsersResetPW="
                      + ((XincoCoreUserServer) allusers.elementAt(i)).getId()
                      + "&list="
                      + request.getParameter("list")
          ...
          out.println(
              "<td colspan=\"6\" class=\"text\">* "
                  + rb.getString("general.password.note")
                  + "</td>");
          out.println("</tr>");
          out.println("</table>");
        }
        if (current_location.compareTo("GroupAdmin") == 0) {
          ...
          out.println(
              "<td class=\"text\"><input type=\"text\" name=\"DialogNewGroupName\""
                  + " size=\"40\"/></td>");
          out.println("</tr>");
          out.println("<tr>");
          out.println("<td class=\"text\">&nbsp;</td>");
          out.println(
              "<td class=\"text\"><input type='hidden' name='list' value='"
                  + request.getParameter("list")
                  + "'/><input type=\"submit\" name=\"DialogNewGroupSubmit\" value=\""
                  + rb.getString("general.add.group")
                  + "\"/></td>");
          out.println("</tr>");
          out.println("</table>");
          out.println("</form>");
          ...
          for (i = 0; i < allgroups.size(); i++) {
            ...
            out.println(
                "<td class=\"text\"><a href=\"XincoAdmin?DialogAdminGroupsSelect="
                    + ((XincoCoreGroupServer) allgroups.elementAt(i)).getId()
                    + "&list="
                    + request.getParameter("list")
                    + "\" class=\"link\">["
                    + rb.getString("general.edit")
                    + "]</a></td>");
            out.println("</tr>");
          }
          out.println("</table>");
        ...
        if (current_location.compareTo("GroupAdminSingle") == 0) {
          ...
          try {
            ...
            out.println(
                "<td class=\"text\"><input type=\"text\" name=\"DialogEditGroupName\" size=\"40\""
                    + " value=\""
                    + designation
                    + "\"/></td>");
            out.println("</tr>");
            ...
            out.println(
                "<td class=\"text\"><input type='hidden' name='list' value='"
                    + request.getParameter("list")
                    + "'/><input type=\"hidden\" name=\"DialogEditGroupID\" value=\""
                    + current_group_selection
                    + "\"/><input type=\"submit\" name=\"DialogEditGroupSubmit\" value=\""
                    + rb.getString("general.save")
                    + "!\"/></td>");
            out.println("</tr>");
            out.println("</table>");
            out.println("</form>");
          ...
          out.println("<tr>");
          out.println(
              "<td class=\"bigtext\" colspan=\"6\">"
                  + rb.getString("general.group.member")
                  + ":</td>");
          out.println("</tr>");
          out.println("<tr>");
          out.println("<td class=\"bigtext\">" + rb.getString("general.id") + ":</td>");
          ...
          for (i = 0; i < allusers.size(); i++) {
            ...
            for (j = 0;
                j < ((XincoCoreUserServer) allusers.elementAt(i)).getXinco_core_groups().size();
                j++) {
              if (((XincoCoreGroupServer)
                          ((XincoCoreUserServer) allusers.elementAt(i))
                              .getXinco_core_groups()
                              .elementAt(j))
                      .getId()
                  == current_group_selection) {
            ...
            if (member_of_group) {
              ...
              out.println(
                  "<td class=\"text\">"
                      ...
                      + "</td>");
              out.println(
                  "<td class=\"text\"><a href=\"XincoAdmin?DialogEditGroupRemoveUser="
                      + ((XincoCoreUserServer) allusers.elementAt(i)).getId()
                      + "&list="
                      + request.getParameter("list")
                      + "\" class=\"link\">["
                      + rb.getString("general.group.removeuser")
                      + "]</a></td>");
              out.println("</tr>");
            }
          }
          ...
          out.println(
              "<td class=\"bigtext\" colspan=\"6\">"
                  + rb.getString("general.group.notmember")
                  + ":</td>");
          out.println("</tr>");
          out.println("<tr>");
          out.println("<td class=\"bigtext\">" + rb.getString("general.id") + ":</td>");
            ...
            if (!member_of_group) {
              ...
              out.println(
                  "<td class=\"text\"><a href=\"XincoAdmin?DialogEditGroupAddUser="
                      ...
                      + request.getParameter("list")
                      + "\" class=\"link\">["
                      + rb.getString("general.group.adduser")
                      + "]</a></td>");
              out.println("</tr>");
            }
          }
        ...
        if (current_location.compareTo("UserProfileEdit") == 0) {
          ...
          try {
            ...
            out.println("<td class=\"text\">" + rb.getString("general.username") + ":</td>");
            out.println(
                "<td class=\"text\"><input type=\"text\" name=\"DialogEditUserProfileUsername\""
                    + " value=\""
            ...
            out.println("<td class=\"text\">" + rb.getString("general.password") + ":</td>");
            out.println(
                "<td class=\"text\"><input type=\"password\" name=\"DialogEditUserProfilePassword\""
                    + " value=\""
                    + temp_user.getUserpassword()
                    + "\" size=\"40\"/></td>");
            out.println("</tr>");
            out.println("<tr>");
            out.println("<td class=\"text\">" + rb.getString("general.firstname") + ":</td>");
            out.println(
                "<td class=\"text\"><input type=\"text\" name=\"DialogEditUserProfileFirstname\""
                    + " value=\""
                    + temp_user.getFirstname()
                    + "\" size=\"40\"/></td>");
            out.println("</tr>");
            out.println("<tr>");
            out.println("<td class=\"text\">" + rb.getString("general.lastname") + ":</td>");
            ...
            out.println("<td class=\"text\">" + rb.getString("general.email") + ":</td>");
            out.println(
                "<td class=\"text\"><input type=\"text\" name=\"DialogEditUserProfileEmail\""
                    + " value=\""
                    + temp_user.getEmail()
                    + "\" size=\"40\"/></td>");
            out.println("</tr>");
            ...
            out.println(
                "<td class=\"text\"><input type='hidden' name='list' value='"
                    + request.getParameter("list")
                    + "'/><input type=\"hidden\" name=\"DialogEditUserProfileID\" value=\""
                    + current_user_selection
                    + "\"/><input type=\"submit\" name=\"DialogEditUserProfileSubmit\" value=\""
                    + rb.getString("general.save")
                    + "!\"/></td>");
            out.println("</tr>");
            out.println("</table>");
        ...
        if (current_location.compareTo("LanguageAdmin") == 0) {
          ...
          out.println(
              "<td class=\"text\"><input type=\"text\" name=\"DialogNewLanguageDesignation\""
                  + " size=\"40\"/></td>");
          out.println("</tr>");
          out.println("<tr>");
          out.println("<td class=\"text\">" + rb.getString("general.sign") + ":</td>");
          out.println(
              "<td class=\"text\"><input type=\"text\" name=\"DialogNewLanguageSign\""
                  + " size=\"5\"/></td>");
          out.println("</tr>");
          out.println("<tr>");
          out.println("<td class=\"text\">&nbsp;</td>");
          out.println(
              "<td class=\"text\"><input type='hidden' name='list' value='"
                  + request.getParameter("list")
                  + "'/><input type=\"submit\" name=\"DialogNewLanguageSubmit\" value=\""
                  + rb.getString("general.add.language")
                  + "\"/></td>");
          out.println("</tr>");
          out.println("</table>");
          out.println("</form>");
          ...
          for (i = 0; i < alllanguages.size(); i++) {
            ...
            out.println(
                "<td class=\"text\">"
                    + ((XincoCoreLanguageServer) alllanguages.elementAt(i)).getSign()
                    + "</td>");
            out.println(
                "<td class=\"text\">"
                    + ((XincoCoreLanguageServer) alllanguages.elementAt(i)).getDesignation()
                    + "</td>");
            is_used =
                XincoCoreLanguageServer.isLanguageUsed(
                    (XincoCoreLanguageServer) alllanguages.elementAt(i), dbm);
            if (!is_used) {
              out.println(
                  "<td class=\"text\"><a href=\"XincoAdmin?DialogAdminLanguagesDelete="
                      + ((XincoCoreLanguageServer) alllanguages.elementAt(i)).getId()
                      + "&list="
                      + request.getParameter("list")
                      + "\" class=\"link\">["
                      + rb.getString("general.delete")
                      + "]</a></td>");
            } else {
              out.println("<td class=\"text\">&nbsp;</td>");
            }
          ...
          out.println(
              "<td colspan=\"4\" class=\"text\">"
                  + rb.getString("error.language.delete.referenced")
                  + "</td>");
          out.println("</tr>");
          out.println("</table>");
        }
        if (current_location.compareTo("AttributesAdmin") == 0) {
          ...
          for (i = 0; i < alldatatypes.size(); i++) {
            ...
            out.println(
                "<td class=\"text\">"
                    + (rb.containsKey(
                            ((XincoCoreDataTypeServer) alldatatypes.elementAt(i)).getDesignation())
                        ? rb.getString(
                            ((XincoCoreDataTypeServer) alldatatypes.elementAt(i)).getDesignation())
                        : ((XincoCoreDataTypeServer) alldatatypes.elementAt(i)).getDesignation())
                    + "</td>");
            out.println(
                "<td class=\"text\">"
                    + (rb.containsKey(
                            ((XincoCoreDataTypeServer) alldatatypes.elementAt(i)).getDescription())
                        ? rb.getString(
                            ((XincoCoreDataTypeServer) alldatatypes.elementAt(i)).getDescription())
                        : ((XincoCoreDataTypeServer) alldatatypes.elementAt(i)).getDescription())
                    + "</td>");
            out.println(
                "<td class=\"text\"><a href=\"XincoAdmin?DialogAdminDataTypeSelect="
                    + ((XincoCoreDataTypeServer) alldatatypes.elementAt(i)).getId()
                    + "&list="
                    + request.getParameter("list")
                    + "\" class=\"link\">["
        ...
        if (current_location.compareTo("AttributesAdminSingle") == 0) {
          ...
          try {
            ...
            out.println(
                "<td class=\"text\"><input type=\"text\" name=\"DialogNewAttributeAttributeId\""
                    + " size=\"5\" value=\"1\"/></td>");
            out.println("</tr>");
            out.println("<tr>");
            out.println("<td class=\"text\">" + rb.getString("general.designation") + ":</td>");
            out.println(
                "<td class=\"text\"><input type=\"text\" name=\"DialogNewAttributeDesignation\""
                    + " size=\"40\" value=\"\"/></td>");
            out.println("</tr>");
            out.println("<tr>");
            out.println("<td class=\"text\">" + rb.getString("general.datatype") + ":</td>");
            out.println(
                "<td class=\"text\"><select name=\"DialogNewAttributeDataType\" size=\"1\"><option"
                    + " value=\"int\">int (Integer)</option><option"
                    + " value=\"unsignedint\">unsignedint (Unsigned Integer)</option><option"
                    + " value=\"double\">double (Floating Point Number)</option><option"
                    + " value=\"varchar\">varchar (String)</option><option value=\"text\">text"
                    + " (Text)</option><option value=\"datetime\">datetime (Date +"
                    + " Time)</option></select></td>");
            out.println("</tr>");
            out.println("<tr>");
            out.println("<td class=\"text\">" + rb.getString("general.size") + ":</td>");
            out.println(
                "<td class=\"text\"><input type=\"text\" name=\"DialogNewAttributeSize\" size=\"5\""
                    + " value=\"0\"/> "
                    + rb.getString("message.admin.attribute.req4string")
                    + "</td>");
            out.println("</tr>");
            out.println("<tr>");
            out.println("<td class=\"text\">&nbsp;</td>");
            out.println(
                "<td class=\"text\"><input type='hidden' name='list' value='"
                    + request.getParameter("list")
                    + "'/><input type=\"hidden\" name=\"DialogNewAttributeDataTypeID\" value=\""
                    + current_datatype_selection
                    + "\"/><input type=\"submit\" name=\"DialogNewAttributeSubmit\" value=\""
                    + rb.getString("general.add.attribute")
                    + "\"/></td>");
            out.println("</tr>");
            out.println("</table>");
            out.println("</form>");
          ...
          try {
            XincoCoreDataTypeServer temp_datatype =
                new XincoCoreDataTypeServer(current_datatype_selection, dbm);
            ...
            out.println("<tr>");
            out.println(
                "<td class=\"bigtext\" colspan=\"5\">"
                    + rb.getString("general.attribute.ofthistype")
                    + ":</td>");
            out.println("</tr>");
            out.println("<tr>");
              ...
              out.println(
                  "<td class=\"text\">"
                      + ((XincoCoreDataTypeAttributeServer)
                              temp_datatype.getXinco_core_data_type_attributes().elementAt(i))
                          .getAttribute_id()
                      + "</td>");
              out.println(
                  "<td class=\"text\">"
              ...
              out.println(
                  "<td class=\"text\">"
                      + ((XincoCoreDataTypeAttributeServer)
                          ...
                          .getSize()
                      + "</td>");
              if (((current_datatype_selection == 1)
                      && (((XincoCoreDataTypeAttributeServer)
                                  temp_datatype.getXinco_core_data_type_attributes().elementAt(i))
                              .getAttribute_id()
                          <= 8))
                  || ((current_datatype_selection == 2)
                      && (((XincoCoreDataTypeAttributeServer)
                                  temp_datatype.getXinco_core_data_type_attributes().elementAt(i))
                              .getAttribute_id()
                          <= 1))
                  || ((current_datatype_selection == 3)
                      && (((XincoCoreDataTypeAttributeServer)
                                  temp_datatype.getXinco_core_data_type_attributes().elementAt(i))
                              .getAttribute_id()
                          <= 1))) {
                ...
                out.println(
                    "<td class=\"text\"><a href=\"XincoAdmin?DialogEditAttributesRemoveAttributeId="
                        + ((XincoCoreDataTypeAttributeServer)
                                temp_datatype.getXinco_core_data_type_attributes().elementAt(i))
                            .getAttribute_id()
                        + "&list="
            ...
            out.println(
                "<td class=\"text\" colspan=\"5\">"
                    + rb.getString("message.warning.attribute.remove")
                    + "</td>");
            out.println("</tr>");
            out.println("</table>");
          } catch (Exception e) {
        ...
        if (current_location.compareTo("AuditTable") == 0) {
          try {
            ...
            out.write("<title>");
            out.println(
                rb.getString("general.audit.results")
                    .replaceAll("%i", request.getParameter("id"))
                    .replaceAll("%t", request.getParameter("table")));
            out.write("</title>\n");
            out.write(
                ...
                dbm.con
                    ...
                    .executeQuery(
                        "select * from "
                            ...
                            + rb.getString("general.user")
                            + "\" , b.mod_time as \""
                            + rb.getString("general.audit.modtime")
                            + "\" ,b.mod_reason as \""
                            + rb.getString("general.reason")
                            + "\" ,b.record_id from xinco_core_user"
                            + " a,xinco_core_user_modified_record b where a.id=b.id ) b where"
                            + " b.record_id =a.record_id and a."
                            + column
                            + " = '"
                            + request.getParameter("id")
        ...
        if (current_location.compareTo("AuditQuery") == 0) {
          try {
            ...
            out.write("            <h1>");
            out.println(
                rb.getString("general.audit.select.record") + request.getParameter("table"));
            out.write("</h1>\n");
            out.write("            ");
        ...
        if (current_location.compareTo("AuditMenu") == 0) {
          try {
            ...
            out.println(
                "<center><table border='0'><tbody><thead><tr><th>"
                    + rb.getString("general.table")
                    + "</th><th>"
                    + rb.getString("general.audit.action")
                    + "</th></tr>");
            while (rs.next()) {
              if (!rs.getString("TABLE_NAME").endsWith("_t")
                ...
                out.println(
                    "<tr><td>"
                        ...
                        + "</td><td><center><input type='submit' value='"
                        + rb.getString("general.continue")
                        + "'/></center></td></tr>"
                        + "<input type='hidden' name='list' value='"
                        + request.getParameter("list")
        ...
        if (current_location.compareTo("RebuildIndex") == 0) {
          ...
          try {
            ...
            out.println("<tr>");
            out.println(
                "<td class=\"bigtext\" colspan=\"2\">"
                    + rb.getString("message.index.rebuild")
                    + ":</td>");
            out.println("</tr>");
            out.println("<tr>");
            out.println(
                "<td class=\"text\" colspan=\"2\">"
                    + rb.getString("message.warning.index.rebuild")
                    + "</td>");
            out.println("</tr>");
            out.println("<tr>");
            ...
            if (indexDirectory.exists()) {
              ...
              out.println("<tr>");
              out.println(
                  "<td class=\"text\"><b>" + rb.getString("message.index.delete") + "</b></td>");
            ...
            out.println(
                "<td class=\"text\"><b>"
                    + rb.getString("message.data.sort.designation")
                    + "</b></td>");
            out.println(
                "<td class=\"text\"><b>" + rb.getString("message.indexing.status") + "</b></td>");
            out.println("</tr>");
    ...
    out.println(
        "<td class=\"text\">&copy; "
            ...
            // Security bug
            (dbm.config.isAllowOutsideLinks()
    ...
    out.println(
        "</table><tr><form action='XincoMenu'><input type='submit' value='"
            ...
            + rb.getString("message.admin.main.back")
