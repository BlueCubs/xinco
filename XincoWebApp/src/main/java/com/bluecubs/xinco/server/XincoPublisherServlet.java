 ...
 *
 * <p>Name: XincoPublisherServlet
 *
 * <p>Description: publisher servlet
 *
  ...
  protected synchronized void processRequest(
          ...
          try {
            ...
            if (xcd.getStatus_number() == XincoDataStatus.PUBLISHED.ordinal() + 1) {
              ...
              for (i = 0; i < xcd.getXinco_core_acl().size(); i++) {
                if ((((XincoCoreACE) xcd.getXinco_core_acl().elementAt(i)).getXinco_core_group_id()
                        == 3)
    ...
    if (fileDownload) { // begin FILE output
      try {
        ...
        FileInputStream in =
            new FileInputStream(XincoCoreDataServer.getLastMajorVersionDataPath(core_data_id, DBM));
      ...
      if (core_data_id == 0) {
        ...
        if (printList) {
          try {
            ...
            ResultSet rs =
                stmt.executeQuery(
                    "SELECT DISTINCT xcd.id, xcd.designation FROM xinco_core_data xcd,"
                        + " xinco_core_ace xca, xinco_core_log xcl WHERE"
                        + " xcd.id=xca.xinco_core_data_id AND xcd.id=xcl.xinco_core_data_id AND"
                        + " (xcd.status_number <>"
                        + (XincoDataStatus.ARCHIVED.ordinal() + 1)
                        + " AND (xca.xinco_core_group_id=3 AND xca.read_permission=1)"
                        + "|| xcd.status_number = "
            ...
            while (rs.next()) {
              ...
              if (xdata_temp.getXinco_core_data_type().getId() == 1) {
                temp_url =
                    ((XincoAddAttribute) xdata_temp.getXinco_add_attributes().elementAt(0))
                        .getAttrib_varchar();
              } else {
              ...
              out.println(
                  "<td class=\"text\">"
                      ...
                      + " | "
                      + xdata_temp.getXinco_core_language().getSign()
                      + ")"
                      + "</td>");
              out.println(
          ...
          try {
            ...
            if (!(request.getParameter("FolderId") == null)) {
              ...
              for (i = 0; i < xnode_temp.getXinco_core_acl().size(); i++) {
                if ((((XincoCoreACE) xnode_temp.getXinco_core_acl().elementAt(i))
                            .getXinco_core_group_id()
                        == 3)
                    && ((XincoCoreACE) xnode_temp.getXinco_core_acl().elementAt(i))
                        .isRead_permission()) {
                  isPublic = true;
                  break;
                }
              ...
              if (isPublic) {
                ...
                out.println(
                    "<td colspan=\"2\" class=\"text\"><b>"
                        + rb.getString("message.xincopublisher.subfolders")
                        + "</b></td>");
                out.println("</tr>");
                out.flush();
                for (i = 0; i < xnode_temp.getXinco_core_nodes().size(); i++) {
                  xnode_temp2 =
                      new XincoCoreNodeServer(
                          ((XincoCoreNodeServer) xnode_temp.getXinco_core_nodes().elementAt(i))
                              .getId(),
                          DBM);
                  isPublic = false;
                  // check read permission for group "public"
                  for (j = 0; j < xnode_temp2.getXinco_core_acl().size(); j++) {
                    if ((((XincoCoreACE) xnode_temp2.getXinco_core_acl().elementAt(j))
                                .getXinco_core_group_id()
                            == 3)
                        && ((XincoCoreACE) xnode_temp2.getXinco_core_acl().elementAt(j))
                            .isRead_permission()) {
                      isPublic = true;
                      break;
                    }
                  ...
                  if (isPublic) {
                    if (temp_path != null) {
                      temp_path2 =
                          temp_path
                              + " / "
                              + xnode_temp2.getDesignation()
                              + " ("
                              + xnode_temp2.getXinco_core_language().getSign()
                              + ")";
                    ...
                    out.println(
                        "<td class=\"text\"><a href=\""
                            + "XincoPublisher?MainMenu=browse&FolderId="
                            + xnode_temp2.getId()
                            + temp_path2
                            + "&list="
                            + request.getParameter("list")
                ...
                out.println(
                    "<td colspan=\"2\" class=\"text\"><b>"
                        + rb.getString("message.xincopublisher.publicdata")
                        + "</b></td>");
                out.println("</tr>");
                out.flush();
                for (i = 0; i < xnode_temp.getXinco_core_data().size(); i++) {
                  xdata_temp =
                      new XincoCoreDataServer(
                          ((XincoCoreDataServer) xnode_temp.getXinco_core_data().elementAt(i))
                              .getId(),
                          DBM);
                  isPublic = false;
                  // check status (5 = published)
                  if (xdata_temp.getStatus_number() == XincoDataStatus.PUBLISHED.ordinal() + 1) {
                    ...
                    if (xdata_temp.getStatus_number() != XincoDataStatus.ARCHIVED.ordinal() + 1) {
                      for (j = 0; j < xdata_temp.getXinco_core_acl().size(); j++) {
                        if ((((XincoCoreACE) xdata_temp.getXinco_core_acl().elementAt(j))
                                    .getXinco_core_group_id()
                                == 3)
                            && ((XincoCoreACE) xdata_temp.getXinco_core_acl().elementAt(j))
                                .isRead_permission()) {
                          isPublic = true;
                          break;
                        }
          ...
          if (config.isAllowPublisherList()) {
            out.println("<tr>");
            out.println(
                "<td class=\"text\" colspan=\"2\"><a href=\"XincoPublisher?MainMenu=list&list="
                    + request.getParameter("list")
                    + "\" class=\"link\">"
                    + rb.getString("message.xincopublisher.list")
                    + "</td>");
            out.println("</tr>");
          }
          out.println("<tr>");
          out.println(
              "<td class=\"text\" colspan=\"2\"><a"
                  + " href=\"XincoPublisher?MainMenu=browse&FolderId=1&Path="
                  + (Base64.encode(("xincoRoot").getBytes()))
                  + "&list="
                  + request.getParameter("list")
                  + "\" class=\"link\">"
                  + rb.getString("message.xincopublisher.browse")
                  + "</td>");
          out.println("</tr>");
        }
        out.println("<tr>");
        ...
        out.println(
            "<td class=\"text\" colspan=\"2\">"
                + rb.getString("message.xincopublisher.howto")
                + "</td>");
        out.println("</tr>");
        out.println("</table>");
      }
        ...
        for (i = 0; i < xcd.getXinco_add_attributes().size(); i++) {
          xcdta =
              ((XincoCoreDataTypeAttribute)
                  xcd.getXinco_core_data_type().getXinco_core_data_type_attributes().elementAt(i));
          xaa = ((XincoAddAttribute) xcd.getXinco_add_attributes().elementAt(i));
          out.println("<tr>");
          out.println(
              "<td class=\"text\" valign=\"top\"><b>" + xcdta.getDesignation() + ":</b></td>");
