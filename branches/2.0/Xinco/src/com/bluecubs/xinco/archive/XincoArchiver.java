        ...
        try {
            ...
            for (j = 0; j < querycount; j++) {
                ...
                while (rs.next()) {
                    ...
                    xnode_temp_vector = XincoCoreNodeServer.getXincoCoreNodeParents(xdata_temp.getXincoCoreNodeId(), dbm);
                    //archive data
                    ArchiveFileDir = "";
                    ...
                    for (i = 0; i < xdata_temp.getXincoCoreLogs().size(); i++) {
                        xlog_temp = ((XincoCoreLogServer) xdata_temp.getXincoCoreLogs().elementAt(i));
                        if ((xlog_temp.getOpCode() == 1) || (xlog_temp.getOpCode() == 5)) {
                    ...
                    for (k = 0; k < OrgFileNames.size(); k++) {
                        FileName = ((String) OrgFileNames.elementAt(k)) + "_" + ((XincoAddAttribute) xdata_temp.getXincoAddAttributes().elementAt(0)).getAttribVarchar();
                    ...
                    //update data + log
                    xdata_temp.setStatusNumber(3);
                    ((XincoAddAttribute) xdata_temp.getXincoAddAttributes().elementAt(7)).setAttribText("[" + ArchiveName + "]" + ArchiveFileDir.replace('\\', '/') + "/" + FileName);
                    xdata_temp.write2DB(dbm);
                    if (xdata_temp.getXincoCoreLogs().size() > 0) {
                        xlog_temp = ((XincoCoreLogServer) xdata_temp.getXincoCoreLogs().elementAt(xdata_temp.getXincoCoreLogs().size() - 1));
                        if (xlog_temp != null) {
                            xlog_temp.setId(0);
                            xlog_temp.setOpCode(8);
                            xlog_temp.setOpDescription("Archived!");
                            xlog_temp.setXincoCoreUserId(1);
