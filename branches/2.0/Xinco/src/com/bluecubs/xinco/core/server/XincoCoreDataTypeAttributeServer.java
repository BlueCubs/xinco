...
public class XincoCoreDataTypeAttributeServer extends XincoCoreDataTypeAttribute {
    ...
    public XincoCoreDataTypeAttributeServer(int attrID1, int attrID2, XincoDBManager DBM) throws XincoException {
        ...
        try {
            ...
            while (rs.next()) {
                RowCount++;
                setXincoCoreDataType_id(rs.getInt("xinco_core_data_type_id"));
    ...
    public XincoCoreDataTypeAttributeServer(int attrID1, int attrID2, String attrD, String attrDT, int attrS) throws XincoException {
        ...
        setXincoCoreDataType_id(attrID1);
    ...
    public int write2DB(XincoDBManager DBM) throws XincoException {
        ...
        try {
            ...
            stmt.executeUpdate("INSERT INTO xinco_core_data_type_attribute VALUES (" + getXincoCoreDataType_id() + ", " + getAttribute_id() + ", '" + getDesignation() + "', '" + getDataType() + "', " + getSize() + ")");
            stmt.close();
            ...
            "xinco_add_attribute.xinco_core_data_id IN (SELECT id FROM xinco_core_data WHERE xinco_core_data.xinco_core_data_type_id=" +
                    getXincoCoreDataType_id()+ ")"},
                    DBM,"audit.datatype.attribute.change",this.getChangerID());
             */
            
            stmt = DBM.getConnection().createStatement();
            stmt.executeUpdate("INSERT INTO xinco_add_attribute SELECT id, " + getAttribute_id() + ", 0, 0, 0, '', '', now() FROM xinco_core_data WHERE xinco_core_data_type_id = " + getXincoCoreDataType_id());
            stmt.close();
    ...
    public static int deleteFromDB(XincoCoreDataTypeAttribute attrCDTA, XincoDBManager DBM, int userID) throws XincoException {
        ...
        try {
            ...
            "xinco_add_attribute.xinco_core_data_id IN (SELECT id FROM xinco_core_data WHERE xinco_core_data.xinco_core_data_type_id=" +
                    attrCDTA.getXincoCoreDataType_id()+ ")"},
            ...
            */
            ...
            stmt = DBM.getConnection().createStatement();
            audit.updateAuditTrail("xinco_core_data_type_attribute",new String [] {"xinco_core_data_type_id=" + attrCDTA.getXincoCoreDataType_id(), "attribute_id=" + attrCDTA.getAttribute_id()},
                        DBM,"audit.general.delete",userID);
            stmt.executeUpdate("DELETE FROM xinco_core_data_type_attribute WHERE xinco_core_data_type_id=" + attrCDTA.getXincoCoreDataType_id() + " AND attribute_id=" + attrCDTA.getAttribute_id());
            stmt.close();
