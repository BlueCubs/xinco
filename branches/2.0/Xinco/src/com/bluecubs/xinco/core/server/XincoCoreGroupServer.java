    ...
    public int write2DB(XincoDBManager DBM) throws XincoException{
        try {
            ...
            if (getId() > 0) {
                ...
                stmt.executeUpdate("UPDATE xinco_core_group SET designation='" + getDesignation().replaceAll("'","\\\\'") + "', status_number=" + getStatusNumber() + " WHERE id=" + getId());
                stmt.close();
            } else {
                setId(DBM.getNewID("xinco_core_group"));
                stmt = DBM.getConnection().createStatement();
                stmt.executeUpdate("INSERT INTO xinco_core_group VALUES (" + getId() + ", '" + getDesignation().replaceAll("'","\\\\'") + "', " + getStatusNumber() + ")");
                stmt.close();
            }
