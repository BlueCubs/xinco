...
public class XincoCoreUserServer extends XincoCoreUser {
    ...
    private void fillXincoCoreGroups(XincoDBManager DBM) throws XincoException {
        ...
        try {
            ...
            if (DBM.getSetting("setting.enable.lockidle").isBoolValue()) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                for (int j = 0; j < getXincoCoreGroups().size(); j++) {
                    if (((XincoCoreGroupServer) getXincoCoreGroups().elementAt(j)).getId() == rs.getInt("xinco_core_group_id")) {
                        found = true;
                        place = j;
                    }
                }
                ...
                if (found) {
                    //Change detected
                    if (getStatusNumber() != rs.getInt("status_number")) {
                        stmt.executeUpdate("update xinco_core_user_has_xinco_core_group set status_number=" +
                                getStatusNumber() + " where xinco_core_user_id=" + getId() + ", xinco_core_group_id=" +
                                ((XincoCoreGroupServer) getXincoCoreGroups().elementAt(place)).getId());
                        audit.updateAuditTrail("xinco_core_user_has_xinco_core_group", new String[]{"xinco_core_user_id =" + getId(),
                            "xinco_core_group_id=" + ((XincoCoreGroupServer) getXincoCoreGroups().elementAt(place)).getId()
                        },
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (rs.getInt("status_number") != 2) {
                    ...
                    long age = DBM.getSetting("password.aging").getIntValue();
                    if (diffDays >= age) {
                        status = 3;
                    } else {
                        status = 1;
            ...
            try {
                ...
                if (rs2.next()) {
                    ...
                    setStatusNumber(rs2.getInt("status_number"));
    ...
    public XincoCoreUserServer(int attrID, XincoDBManager DBM) throws XincoException {
        ...
        try {
            Statement stmt = DBM.getConnection().createStatement();
            rs = stmt.executeQuery("SELECT * FROM xinco_core_user WHERE id=" + attrID);
            //throw exception if no result found
            int RowCount = 0;
            while (rs.next()) {
                RowCount++;
    ...
    public int write2DB(XincoDBManager DBM) throws XincoException {
        ...
        try {
            ...
            if (getAttempts() > DBM.getSetting("password.aging").getIntValue() &&
                    getId() > 1) {
                setStatusNumber(2);
            }
            if (getId() > 0) {
                ...
                sql = "UPDATE xinco_core_user SET username='" +
                        ...
                        getStatusNumber() + ", attempts=" + getAttempts() +
                ...
                sql = "INSERT INTO xinco_core_user VALUES (" + getId() +
                        ...
                        getEmail().replaceAll("'", "\\\\'") + "', " +
                        getStatusNumber() + ", " + getAttempts() + ", '" + ts.toString() + "')";
                stmt.executeUpdate(sql);
                stmt.close();
            }
            if (isWriteGroups()) {
        ...
        try {
            ...
            try {
                ...
                sql = "select userpassword from xinco_core_user_t where id=" +
                        ...
                        DBM.getSetting("password.unusable_period").getIntValue() +
