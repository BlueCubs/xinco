...
public class XincoDBManager{
    ...
    protected void fillSettings(){
        ResultSet rs=null;
        getXincoSettingServer().setXincoSettings(new Vector());
        String string_value="";
        try {
            ...
            while(rs.next()){
                ...
                getXincoSettingServer().getXincoSettings().addElement(new XincoSettingServer(rs.getInt("id"),
                        rs.getString("description"),rs.getInt("int_value"),string_value,
                        rs.getBoolean("bool_value"),0,rs.getLong("long_value"),new Vector()));
            }
            stm.close();
    ...
    public void resetDB(XincoCoreUserServer user) throws XincoException{
        Vector groups=user.getXincoCoreGroups();
            ...
            try {
                ...
                while(rs.next()){
                    if(!rs.getString("TABLE_NAME").equals("xinco_id") &&
                        ...
                        if(getXincoSettingServer().getSetting("setting.enable.developermode").isBoolValue())
