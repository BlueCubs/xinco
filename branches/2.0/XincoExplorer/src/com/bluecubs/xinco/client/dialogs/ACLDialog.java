        ...
        for (i=0;i<temp_vector.size();i++) {
            ...
            if (temp_ace.getXincoCoreGroupId() > 0) {
                for (j=0;j<this.explorer.getSession().getServer_groups().size();j++) {
                    if (((XincoCoreGroup)this.explorer.getSession().getServer_groups().elementAt(j)).getId() == temp_ace.getXincoCoreGroupId()) {
        ...
        if (this.groupList.getSelectedIndex() >= 0) {
            try {
                ...
                for (i=0;i<temp_acl.size();i++) {
                    if (((XincoCoreACE)temp_acl.elementAt(i)).getXincoCoreGroupId() == ((XincoCoreGroup)this.explorer.getSession().getServer_groups().elementAt(this.groupList.getSelectedIndex())).getId()) {
                ...
                if (this.explorer.getSession().getCurrentTreeNodeSelection().getUserObject().getClass() == XincoCoreNode.class) {
                    newace.setXincoCoreNodeId(((XincoCoreNode)this.explorer.getSession().getCurrentTreeNodeSelection().getUserObject()).getId());
                }
                if (this.explorer.getSession().getCurrentTreeNodeSelection().getUserObject().getClass() == XincoCoreData.class) {
                    newace.setXincoCoreDataId(((XincoCoreData)this.explorer.getSession().getCurrentTreeNodeSelection().getUserObject()).getId());
                }
