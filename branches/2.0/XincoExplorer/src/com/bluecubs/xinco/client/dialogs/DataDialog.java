    ...
    public DataDialog(java.awt.Frame parent, boolean modal, XincoExplorer explorer) {
        ...
        if (explorer.getSession().getCurrentTreeNodeSelection().getUserObject() != null) {
            ...
            for (i=0;i<explorer.getSession().getServer_languages().size();i++) {
                ...
                dlm.addElement(text);
                if (((XincoCoreLanguage)explorer.getSession().getServer_languages().elementAt(i)).getId() == ((XincoCoreData)explorer.getSession().getCurrentTreeNodeSelection().getUserObject()).getXincoCoreLanguage().getId()) {
                    selection=i;
                }
...
//set altered values
        ...
        ((XincoCoreData)explorer.getSession().getCurrentTreeNodeSelection().getUserObject()).setXincoCoreLanguage(((XincoCoreLanguage)explorer.getSession().getServer_languages().elementAt(this.language.getSelectedIndex())));
