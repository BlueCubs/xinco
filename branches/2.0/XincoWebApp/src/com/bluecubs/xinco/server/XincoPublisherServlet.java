...
public class XincoPublisherServlet extends HttpServlet {
    ...
    protected synchronized void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
                    ...
                    try {
                        ...
                        if (xcd.getStatusNumber() == 5) {
                            ...
                            for (i = 0; i < xcd.getXinco_core_acl().size(); i++) {
                                if ((((XincoCoreACE) xcd.getXinco_core_acl().elementAt(i)).getXincoCoreGroupId() == 3) && ((XincoCoreACE) xcd.getXinco_core_acl().elementAt(i)).isReadPermission()) {
                                    isPublic = true;
                                    break;
                                }
                            }
        ...
        if (fileDownload) {
            ...
            if (core_data_id == 0) {
                out.println("<title>"+
                        java.util.ResourceBundle.getBundle("com/bluecubs/xinco/web/web_messages").getString("window.title.XincoPublisher")+"</title>");
                ...
                if (printList) {
                    try {
                        ...
                        while (rs.next()) {
                            ...
                            temp_url = "";
                            //file = 1
                            if (xdata_temp.getXincoCoreDataType().getId() == 1) {
                                temp_url = ((XincoAddAttribute) xdata_temp.getXincoAddAttributes().elementAt(0)).getAttribVarchar();
                            } else {
                                temp_url = xdata_temp.getDesignation();
                            }
                            out.println("<tr>");
                            out.println("<td class=\"text\">" + xdata_temp.getDesignation() + " (" + xdata_temp.getXincoCoreDataType().getDesignation() + " | " + xdata_temp.getXincoCoreLanguage().getSign() + ")" + "</td>");
                    ...
                    try {
                        ...
                        if (!(request.getParameter("FolderId") == null)) {
                            ...
                            for (i = 0; i < xnode_temp.getXinco_core_acl().size(); i++) {
                                if ((((XincoCoreACE) xnode_temp.getXinco_core_acl().elementAt(i)).getXincoCoreGroupId() == 3) && ((XincoCoreACE) xnode_temp.getXinco_core_acl().elementAt(i)).isReadPermission()) {
                                    isPublic = true;
                                    break;
                                }
                            }
                            if (isPublic) {
                                ...
                                for (i = 0; i < xnode_temp.getXinco_core_nodes().size(); i++) {
                                    ...
                                    for (j = 0; j < xnode_temp2.getXinco_core_acl().size(); j++) {
                                        if ((((XincoCoreACE) xnode_temp2.getXinco_core_acl().elementAt(j)).getXincoCoreGroupId() == 3) && ((XincoCoreACE) xnode_temp2.getXinco_core_acl().elementAt(j)).isReadPermission()) {
                                            isPublic = true;
                                            break;
                                        }
                                    }
                                    if (isPublic) {
                                        if (temp_path != null) {
                                            temp_path2 = temp_path + " / " + xnode_temp2.getDesignation() + " (" + xnode_temp2.getXincoCoreLanguage().getSign() + ")";
                                        ...
                                        out.println("<td class=\"text\"><a href=\"" + "XincoPublisher?MainMenu=browse&FolderId=" + xnode_temp2.getId() + temp_path2 + "&list=" + request.getParameter("list") + "\">[" + xnode_temp2.getDesignation() + " (" + xnode_temp2.getXincoCoreLanguage().getSign() + ")" + "]</a></td>");
                                        out.println("</tr>");
                                        out.flush();
                                    }
                                }
                                ...
                                for (i = 0; i < xnode_temp.getXinco_core_data().size(); i++) {
                                    ...
                                    isPublic = false;
                                    //check status (5 = published)
                                    if (xdata_temp.getStatusNumber() == 5) {
                                        isPublic = true;
                                    } else {
                                        //check read permission for group "public"
                                        for (j = 0; j < xdata_temp.getXinco_core_acl().size(); j++) {
                                            if ((((XincoCoreACE) xdata_temp.getXinco_core_acl().elementAt(j)).getXincoCoreGroupId() == 3) && ((XincoCoreACE) xdata_temp.getXinco_core_acl().elementAt(j)).isReadPermission()) {
                                                isPublic = true;
                                                break;
                                            }
                                        }
            ...
            if (core_data_id == 0) {
                ...
                out.println("<tr>");
                out.println("<td class=\"text\">&nbsp;</td>");
                out.println("<td class=\"text\">&copy; " + DBM.getSetting("general.copyright.date").getStringValue() + ", " + (DBM.config.isAllowOutsideLinks() ? rb.getString("message.admin.main.footer") : "blueCubs.com and xinco.org"));
                out.println("</tr>");
