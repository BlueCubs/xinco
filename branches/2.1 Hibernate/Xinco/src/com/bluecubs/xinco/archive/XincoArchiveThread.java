...
public class XincoArchiveThread extends Thread {
    ...
    public static synchronized boolean archiveData(XincoDBManager DBM) {
        ...
        try {
            ...
            query[querycount] = new String("SELECT xcd FROM XincoCoreData xcd, XincoAddAttribute xaa1, XincoAddAttribute xaa2 " +
            ...
            query[querycount] = new String("SELECT xcd FROM xincoCoreData xcd, xinco_add_attribute xaa1, xinco_add_attribute xaa2, xincoCore_log xcl " +
                    "WHERE xcd.xincoCoreDataTypeId = 1 " +
                    "AND xcd.status_number <> 3 " +
                    "AND xcd.id = xaa1.xincoCoreDataId " +
                    "AND xcd.id = xaa2.xincoCoreDataId " +
