...
public class XincoSettingServer extends XincoSetting implements XincoAuditableDAO, PersistenceServerObject {
    ...
    @SuppressWarnings("unchecked")
    public static XincoSetting getSetting(String desc) {
        parameters.clear();
        parameters.put("description", desc);
        result = pm.namedQuery("XincoSetting.findByDescription", parameters);
        if (result.isEmpty()) {
    ...
    @Override
    public com.bluecubs.xinco.core.XincoSetting transform() throws Exception {
        return new com.bluecubs.xinco.core.XincoSetting(getId(),
                getDescription(), getIntValue(), getStringValue(), getBoolValue(),
                getChangerID(), getLongValue(), getXincoSettings());
    }
}
