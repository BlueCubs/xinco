...
public class DataFolderDialog extends AbstractDialog {
    ...
    public DataFolderDialog(java.awt.Frame parent, boolean modal, XincoExplorer explorer) {
        ...
        if (explorer.getSession().getCurrentTreeNodeSelection().getUserObject() != null) {
            ...
            for (i = 0; i < explorer.getSession().getServerLanguages().size(); i++) {
                ...
                dlm.addElement(text);
                if (((XincoCoreLanguage) explorer.getSession().getServerLanguages().elementAt(i)).getId() == ((XincoCoreNode) explorer.getSession().getCurrentTreeNodeSelection().getUserObject()).getXincoCoreLanguage().getId()) {
                    selection = i;
                }
    ...
    private void saveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveActionPerformed
        ...
        newnode.setXincoCoreLanguage(((XincoCoreLanguage) explorer.getSession().getServerLanguages().elementAt(this.language.getSelectedIndex())));
        try {
            // optimize node size
            newnode.setXincoCoreNodes(new Vector());
            newnode.setXincoCoreData(new Vector());
