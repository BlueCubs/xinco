...
public class SearchDialog extends AbstractDialog {
    ...
    public SearchDialog(java.awt.Frame parent, boolean modal, XincoExplorer e) {
        ...
        for (i = 0; i < explorer.getSession().getServerDatatypes().size(); i++) {
            ...
            for (j = 0; j < xcdt.getXincoCoreDataTypeAttributes().size(); j++) {
                text = ((XincoCoreDataTypeAttribute) xcdt.getXincoCoreDataTypeAttributes().elementAt(j)).getDesignation();
    ...
    private class searchThread extends Thread {
        ...
        public void run() {
            ...
            for (i = 0; i < explorer.getSession().getCurrentSearchResult().size(); i++) {
                rdata[0] = ((XincoCoreData) (((Vector) explorer.getSession().getCurrentSearchResult().elementAt(i)).elementAt(0))).getDesignation() + " (" + ((XincoCoreData) (((Vector) explorer.getSession().getCurrentSearchResult().elementAt(i)).elementAt(0))).getXincoCoreDataType().getDesignation() + " | " + ((XincoCoreData) (((Vector) explorer.getSession().getCurrentSearchResult().elementAt(i)).elementAt(0))).getXincoCoreLanguage().getSign() + ")";
                rdata[1] = new String("");
