...
import com.bluecubs.xinco.persistence.audit.XincoCoreDataTypeAttributeT;
...
public class XincoCoreDataTypeAttributeServer extends XincoCoreDataTypeAttribute implements AuditableDAO, PersistenceServerObject {
    ...
    public boolean deleteFromDB() {
        ...
        try {
            AuditingDAOHelper.delete(this, getXincoCoreDataTypeAttributePK().getAttributeId());
            return true;
        } catch (Throwable e) {
            try {
                if (XincoSettingServer.getSetting("setting.enable.developermode").getBoolValue()) {
    ...
    public boolean write2DB() {
        try {
            if (getXincoCoreDataTypeAttributePK().getAttributeId() > 0) {
                if (XincoSettingServer.getSetting("setting.enable.developermode").getBoolValue()) {
                    Logger.getLogger(XincoCoreDataTypeAttributeServer.class.getName()).log(Level.INFO, "Updating: " + getXincoCoreDataTypeAttributePK());
                }
                AuditingDAOHelper.update(this, new XincoCoreDataTypeAttribute(getXincoCoreDataTypeAttributePK().getXincoCoreDataTypeId(),
                        getXincoCoreDataTypeAttributePK().getAttributeId()));
            } else {
                if (XincoSettingServer.getSetting("setting.enable.developermode").getBoolValue()) {
                    Logger.getLogger(XincoCoreDataTypeAttributeServer.class.getName()).log(Level.INFO, "Creating: " + getXincoCoreDataTypeAttributePK());
                }
    ...
    public AbstractAuditableObject create(AbstractAuditableObject value) {
        try {
            ...
            if (XincoSettingServer.getSetting("setting.enable.developermode").getBoolValue()) {
                Logger.getLogger(XincoCoreDataTypeAttributeServer.class.getName()).log(Level.INFO, "Creating with new id: " + newValue.getXincoCoreDataTypeAttributePK());
            }
            newValue.setXincoCoreDataTypeAttributePK(temp.getXincoCoreDataTypeAttributePK());
