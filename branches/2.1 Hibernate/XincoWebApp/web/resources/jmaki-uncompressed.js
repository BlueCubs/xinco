        ...
        var checkQueue = function() {
            if (_inprocess.keys().length == 0) {
                if (isDefined(_cb)){                     
                    ...
                    // load the next queued set of scripts
             ...
             callback :  function(req) {
                if (req.responseText != '') {
                    ...
                    if (obj.config) {
                        ...
                        if (obj.config.globalTheme)_jmaki.config.globalTheme = obj.config.globalTheme;
...
// If the text is not JSON parseable, then a SyntaxError is thrown.
                _jmaki.log(jmaki.getMessage('invalid_json'));
            }
        };
    ...
    * 
    * @param - A single object or array
    *
    */
  ...
  this.DContainer = function(args){ 
        ...
        if (args.useIframe && args.useIframe == true) {
            ...
            var iframeTemplate = "<iframe id='" + _self.uuid + "_iframe' name='" + _self.uuid + "_iframe' " + srcString + " frameborder=0 scrolling=" + ((args.overflow == 'hidden') ? 'NO' : 'YES') + "></iframe>";
    ...
    while (_t.indexOf("<script") != -1) {
                    ...
                    styleSourceName = _t.substring(styleSourceLinkStart, styleSourceLinkEnd);
	              	var exists = false;
                        for (var i = 0; i < gstyles.length; i++) {
                            if (isDefined(gstyles[i].href)) {
                                if (gstyles[i].href == styleSourceName) {
                                    exists = true;	
                                }
                            }
                        }
      ...
      this.loadScripts = function(task, initFunction) {    
          ...
          if (task.scriptReferences && task.scriptReferences.length > 0){
              ...
              return _jmaki.addLibraries({ libs : task.scriptReferences.reverse(), callback : _loadEmbeded, cleaup : false});   
          } else {
              _loadEmbeded();
          }
          return true;
