...
- [x] Task: Update Maven Compiler Configuration 77a89c2
    - [x] Update `maven-compiler-plugin` in `pom.xml` to target JDK 11 (as a stepping stone) or JDK 17 directly if feasible.
    - [x] Resolve any immediate compilation errors due to removed APIs (e.g., JAXB).
- [x] Task: Update Dependencies 0600720
    - [x] Run `mvn versions:display-dependency-updates` to identify outdated artifacts.
    - [x] Update critical dependencies (Spring, Hibernate, etc.) to versions compatible with the target JDK.
- [x] Task: Verify Build
    - [x] Run `mvn clean install` to ensure a successful build with the new configuration.

## Phase 2: Docker Environment [checkpoint: d39819b]
- [x] Task: Create Dockerfile 05eaed9
    - [x] Create a `Dockerfile` that uses a modern base image (e.g., `openjdk:17-slim` or `tomcat:9-jdk17`).
    - [x] Configure the Dockerfile to deploy the built WAR file.
- [x] Task: Create Docker Compose 967a325
    - [x] Create `docker-compose.yml` to orchestrate the application and a database (MySQL/PostgreSQL).
    - [x] Configure environment variables for database connection.
- [x] Task: Verify Containerization a3d20fd
    - [x] Run `docker-compose up` and verify the application starts and connects to the DB.
...
- [x] Task: Establish Baseline Tests
...
- [x] Task: Enable Coverage Reporting
    - [ ] Integrate Codecov or publish JaCoCo reports as build artifacts.
- [x] Task: Remove Legacy CI Configs
