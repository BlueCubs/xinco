...
public class XincoCoreDataTypeAttributeServer extends XincoCoreDataTypeAttribute {
	...
	public static int deleteFromDB(XincoCoreDataTypeAttribute attrCDTA, XincoDBManager DBM) throws XincoException {
		...
		try {
			...
			stmt = DBM.con.createStatement();
			stmt.executeUpdate("DELETE FROM xinco_add_attribute WHERE xinco_add_attribute.attribute_id=" + attrCDTA.getAttribute_id() + " AND xinco_add_attribute.xinco_core_data_id IN (SELECT id FROM xinco_core_data WHERE xinco_core_data.xinco_core_data_type_id=" + attrCDTA.getXinco_core_data_type_id() + ")");
			stmt.close();
