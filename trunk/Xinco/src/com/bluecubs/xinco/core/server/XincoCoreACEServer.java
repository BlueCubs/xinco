...
public class XincoCoreACEServer extends XincoCoreACE {
    ...
    public int write2DB(XincoDBManager DBM) throws XincoException {
        ...
        try {
            ...
            if (getId() > 0) {
                ...
                DBM.executeUpdate("INSERT INTO xinco_core_ace VALUES (" + getId() +
                        ", " + xcuid + ", " + xcgid + ", " + xcnid +
                        ", " + xcdid + ", " + rp + ", " + wp + ", " +
                        xp + ", " + ap +  ", " + ad +", " + ow +")");
                DBM.getCon().commit();
    ...
    public static void removeAllFromDB(int data_id,int user_id,XincoDBManager dbm){
        XincoCoreAuditTrail audit= new XincoCoreAuditTrail();
        ResultSet rs=dbm.executeQuery("select id from xinco_core_ace where xinco_core_data_id ="+data_id);
        try {
            while(rs.next()){
    ...
    public static Vector getXincoCoreACL(int attrID, String attrT, XincoDBManager DBM) {
        ...
        try {
            ResultSet rs = DBM.executeQuery("SELECT * FROM xinco_core_ace WHERE " + attrT +
    ...
    public static XincoCoreACE checkAccess(XincoCoreUser attrU, Vector attrACL) {
        ...
        for (i=0;i<attrACL.size();i++) {
            ...
            if (match_ace) {
                ...
                //modify executeUpdate permission
