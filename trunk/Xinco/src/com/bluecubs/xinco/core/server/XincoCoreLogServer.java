...
public class XincoCoreLogServer extends XincoCoreLog {
    ...
    public int write2DB(XincoDBManager DBM) throws XincoException {
        try {
            if (getId() > 0) {
                ...
                DBM.executeUpdate("UPDATE xinco_core_log SET xinco_core_data_id=" + 
                        getXinco_core_data_id() + ", xinco_core_user_id=" + 
                        getXinco_core_user_id() + ", op_code=" + getOp_code() + 
                        ", op_datetime=now(), op_description='" + 
                        getOp_description().replaceAll("'","\\\\'") + 
                        "', version_high=" + getVersion().getVersion_high() +
                        ", version_mid=" + getVersion().getVersion_mid() + 
                        ", version_low=" + getVersion().getVersion_low() + 
                        ", version_postfix='" + getVersion().getVersion_postfix().replaceAll("'","\\\\'") +
                        "' WHERE id=" + getId());
            } else {
                setId(DBM.getNewID("xinco_core_log"));
                DBM.executeUpdate("INSERT INTO xinco_core_log VALUES (" + getId() + 
                        ", " + getXinco_core_data_id() + ", " + 
                        getXinco_core_user_id() + ", " + getOp_code() + ", now(), '" +
                        getOp_description().replaceAll("'","\\\\'") + "', " + 
                        getVersion().getVersion_high() + ", " + 
                        getVersion().getVersion_mid() + ", " + 
                        getVersion().getVersion_low() + ", '" + 
                        getVersion().getVersion_postfix().replaceAll("'","\\\\'") + "')");
            }
            DBM.getCon().commit();
            try {
                DBM.finalize();
    ...
    public static void removeAllFromDB(int data_id,int user_id,XincoDBManager dbm){
        XincoCoreAuditTrail audit= new XincoCoreAuditTrail();
        ResultSet rs=dbm.executeQuery("select id from xinco_core_log where xinco_core_data_id ="+data_id);
        try {
            while(rs.next()){
                audit.updateAuditTrail("xinco_core_log",new String [] {"id ="+rs.getInt("id")},
                        dbm,"audit.general.delete",user_id);
                dbm.executeUpdate("DELETE FROM xinco_core_log WHERE id=" + rs.getInt("id"));
            }
            dbm.getCon().commit();
            try {
                dbm.finalize();
    ...
    public static void removeFromDB(int id,int user_id,XincoDBManager dbm){
        ...
        audit.updateAuditTrail("xinco_core_log",new String [] {"id ="+id},
                dbm,"audit.general.delete",user_id);
        dbm.executeUpdate("DELETE FROM xinco_core_log WHERE id=" + id);
        try {
            dbm.getCon().commit();
        } catch (SQLException ex) {
            try {
