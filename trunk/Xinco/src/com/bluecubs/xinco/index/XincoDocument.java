...
public class XincoDocument {
    ...
    public static Document getXincoDocument(XincoCoreData d, boolean index_content) throws java.io.FileNotFoundException {
        ...
        if (index_content) {
            if ((d.getXinco_core_data_type().getId() == 1) && (d.getStatus_number() != 3)) { //process non-archived file
                ...
                for (l = 0; l < XincoDBManager.config.FileIndexerCount; l++) {
                    for (i = 0; i < ((String[]) XincoDBManager.config.IndexFileTypesExt.elementAt(l)).length; i++) {
                        if (((String[]) XincoDBManager.config.IndexFileTypesExt.elementAt(l))[i].compareTo(file_ext) == 0) {
