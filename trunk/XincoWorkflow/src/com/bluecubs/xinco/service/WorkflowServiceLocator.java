/**
 * WorkflowServiceLocator.java
 *
 * This file was auto-generated from WSDL
 * by the Apache Axis 1.4 Apr 22, 2006 (06:55:48 PDT) WSDL2Java emitter.
 */

package com.bluecubs.xinco.service;

public class WorkflowServiceLocator extends org.apache.axis.client.Service implements com.bluecubs.xinco.service.WorkflowService {

    public WorkflowServiceLocator() {
    }


    public WorkflowServiceLocator(org.apache.axis.EngineConfiguration config) {
        super(config);
    }

    public WorkflowServiceLocator(java.lang.String wsdlLoc, javax.xml.namespace.QName sName) throws javax.xml.rpc.ServiceException {
        super(wsdlLoc, sName);
    }

    // Use to get a proxy class for Workflow
    private java.lang.String Workflow_address = "http://localhost:8084/xinco/services/XincoWorkflow";

    public java.lang.String getWorkflowAddress() {
        return Workflow_address;
    }

    // The WSDD service name defaults to the port name.
    private java.lang.String WorkflowWSDDServiceName = "Workflow";

    public java.lang.String getWorkflowWSDDServiceName() {
        return WorkflowWSDDServiceName;
    }

    public void setWorkflowWSDDServiceName(java.lang.String name) {
        WorkflowWSDDServiceName = name;
    }

    public com.bluecubs.xinco.service.XincoWorkflow getWorkflow() throws javax.xml.rpc.ServiceException {
       java.net.URL endpoint;
        try {
            endpoint = new java.net.URL(Workflow_address);
        }
        catch (java.net.MalformedURLException e) {
            throw new javax.xml.rpc.ServiceException(e);
        }
        return getWorkflow(endpoint);
    }

    public com.bluecubs.xinco.service.XincoWorkflow getWorkflow(java.net.URL portAddress) throws javax.xml.rpc.ServiceException {
        try {
            com.bluecubs.xinco.service.XincoWorkflowSoapBindingStub _stub = new com.bluecubs.xinco.service.XincoWorkflowSoapBindingStub(portAddress, this);
            _stub.setPortName(getWorkflowWSDDServiceName());
            return _stub;
        }
        catch (org.apache.axis.AxisFault e) {
            return null;
        }
    }

    public void setWorkflowEndpointAddress(java.lang.String address) {
        Workflow_address = address;
    }

    /**
     * For the given interface, get the stub implementation.
     * If this service has no port for the given interface,
     * then ServiceException is thrown.
     */
    public java.rmi.Remote getPort(Class serviceEndpointInterface) throws javax.xml.rpc.ServiceException {
        try {
            if (com.bluecubs.xinco.service.XincoWorkflow.class.isAssignableFrom(serviceEndpointInterface)) {
                com.bluecubs.xinco.service.XincoWorkflowSoapBindingStub _stub = new com.bluecubs.xinco.service.XincoWorkflowSoapBindingStub(new java.net.URL(Workflow_address), this);
                _stub.setPortName(getWorkflowWSDDServiceName());
                return _stub;
            }
        }
        catch (java.lang.Throwable t) {
            throw new javax.xml.rpc.ServiceException(t);
        }
        throw new javax.xml.rpc.ServiceException("There is no stub implementation for the interface:  " + (serviceEndpointInterface == null ? "null" : serviceEndpointInterface.getName()));
    }

    /**
     * For the given interface, get the stub implementation.
     * If this service has no port for the given interface,
     * then ServiceException is thrown.
     */
    public java.rmi.Remote getPort(javax.xml.namespace.QName portName, Class serviceEndpointInterface) throws javax.xml.rpc.ServiceException {
        if (portName == null) {
            return getPort(serviceEndpointInterface);
        }
        java.lang.String inputPortName = portName.getLocalPart();
        if ("Workflow".equals(inputPortName)) {
            return getWorkflow();
        }
        else  {
            java.rmi.Remote _stub = getPort(serviceEndpointInterface);
            ((org.apache.axis.client.Stub) _stub).setPortName(portName);
            return _stub;
        }
    }

    public javax.xml.namespace.QName getServiceName() {
        return new javax.xml.namespace.QName("urn:XincoWorkflow", "WorkflowService");
    }

    private java.util.HashSet ports = null;

    public java.util.Iterator getPorts() {
        if (ports == null) {
            ports = new java.util.HashSet();
            ports.add(new javax.xml.namespace.QName("urn:XincoWorkflow", "Workflow"));
        }
        return ports.iterator();
    }

    /**
    * Set the endpoint address for the specified port name.
    */
    public void setEndpointAddress(java.lang.String portName, java.lang.String address) throws javax.xml.rpc.ServiceException {
        
if ("Workflow".equals(portName)) {
            setWorkflowEndpointAddress(address);
        }
        else 
{ // Unknown Port Name
            throw new javax.xml.rpc.ServiceException(" Cannot set Endpoint Address for Unknown Port" + portName);
        }
    }

    /**
    * Set the endpoint address for the specified port name.
    */
    public void setEndpointAddress(javax.xml.namespace.QName portName, java.lang.String address) throws javax.xml.rpc.ServiceException {
        setEndpointAddress(portName.getLocalPart(), address);
    }

}
