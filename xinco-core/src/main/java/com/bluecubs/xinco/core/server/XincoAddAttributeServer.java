 ...
 *
 * <p>Name: XincoAddAttributeServer
 *
 * <p>Description: additional attributes of a data object
 *
...
public final class XincoAddAttributeServer extends XincoAddAttribute {
  ...
  public XincoAddAttributeServer(int xincoCoreDataId, int attributeId) throws XincoException {
    try {
      result =
          createdQuery(
              "SELECT xaa FROM XincoAddAttribute xaa"
                  + " WHERE xaa.xincoAddAttributePK.xincoCoreDataId="
                  + xincoCoreDataId
                  + " AND xaa.xincoAddAttributePK.attributeId="
                  + attributeId);
      if (result.size() > 0) {
  ...
  public int write2DB() throws XincoException {
    ...
    try {
      ...
      if (getAttribDatetime() != null) {
        ...
        cal.add(
            MILLISECOND,
            (ngc.get(ZONE_OFFSET) - cal.get(ZONE_OFFSET))
                - (ngc.get(DST_OFFSET) + cal.get(DST_OFFSET)));
        attrDT =
            ""
                + cal.get(YEAR)
                + "-"
      ...
      com.bluecubs.xinco.core.server.persistence.XincoAddAttribute xaa =
          new XincoAddAttributeJpaController(getEntityManagerFactory())
              .findXincoAddAttribute(
                  new XincoAddAttributePK(getXincoCoreDataId(), getAttributeId()));
      if (xaa == null) {
        create = true;
        xaa = new com.bluecubs.xinco.core.server.persistence.XincoAddAttribute();
        xaa.setXincoCoreData(
            new com.bluecubs.xinco.core.server.persistence.XincoCoreData(getXincoCoreDataId()));
      ...
      XincoAddAttributeJpaController controller =
          new XincoAddAttributeJpaController(getEntityManagerFactory());
      if (create) {
  ...
  public static List<XincoAddAttribute> getXincoAddAttributes(int attrID) {
    ArrayList<XincoAddAttribute> addAttributes = new ArrayList<>();
    try {
      result =
          createdQuery(
              "SELECT xaa FROM XincoAddAttribute xaa "
      ...
      getLogger(XincoAddAttributeServer.class.getSimpleName()).log(SEVERE, null, e);
      addAttributes.clear();
    }
