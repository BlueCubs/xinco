...
public final class XincoCoreACEServer extends XincoCoreACE {
  ...
  public XincoCoreACEServer(int attrID) throws XincoException {
    ...
    com.bluecubs.xinco.core.server.persistence.XincoCoreAce ace =
        controller.findXincoCoreAce(attrID);
    if (ace != null) {
      setId(ace.getId());
  ...
  public int write2DB() throws XincoException {
    try {
      ...
      if (getXincoCoreUserId() != 0) {
        ace.setXincoCoreUser(
            new com.bluecubs.xinco.core.server.persistence.XincoCoreUser(getXincoCoreUserId()));
      }
      if (getXincoCoreGroupId() != 0) {
        ace.setXincoCoreGroup(
            new com.bluecubs.xinco.core.server.persistence.XincoCoreGroup(getXincoCoreGroupId()));
      }
      if (getXincoCoreNodeId() != 0) {
        ace.setXincoCoreNode(
            new com.bluecubs.xinco.core.server.persistence.XincoCoreNode(getXincoCoreNodeId()));
      }
      if (getXincoCoreDataId() != 0) {
        ace.setXincoCoreData(
  ...
  public static int removeFromDB(XincoCoreACE attrCACE, int userID) throws XincoException {
    try {
      XincoCoreAceJpaController controller =
          new XincoCoreAceJpaController(getEntityManagerFactory());
