...
public final class XincoCoreDataServer extends XincoCoreData {
  ...
  public static String getLastMajorVersionDataPath(int xincoCoreDataId)
    ...
    for (Iterator<Object> it = temp.getXincoCoreLogs().iterator(); it.hasNext(); ) {
      ...
      if (log.getVersion().getVersionMid() == 0) {
        return getXincoCoreDataPath(
            CONFIG.fileRepositoryPath, xincoCoreDataId, xincoCoreDataId + "-" + log.getId());
      }
    }
  ...
  public static XincoVersion getCurrentVersion(int xincoCoreDataId) {
    try {
      ...
      if (!temp.getXincoCoreLogs().isEmpty()) {
        return ((XincoCoreLog) temp.getXincoCoreLogs().get(temp.getXincoCoreLogs().size() - 1))
            .getVersion();
      } else {
        return null;
      }
  ...
  public XincoCoreDataServer(
      int attrID, int attrCNID, int attrLID, int attrDTID, String attrD, int attrSN)
  ...
  public XincoCoreDataServer(com.bluecubs.xinco.core.server.persistence.XincoCoreData xcd)
      throws XincoException {
    try {
      setId(xcd.getId());
      setXincoCoreNodeId(xcd.getXincoCoreNode().getId());
  ...
  public static List<XincoCoreData> findXincoCoreData(
      String attrS, int attrLID, boolean attrSA, boolean attrSFD) {
    ...
    try {
      ...
      if (attrSA) {
        result =
            createdQuery(
                "SELECT x FROM XincoAddAttribute x WHERE (x.xincoCoreData.designation LIKE '"
                    + attrS
                    + "%' or "
                    + "x.attribVarchar  LIKE '"
                    + attrS
                    + "%' or x.attribText LIKE '"
                    + attrS
                    + "') "
                    + lang
                    + "order by x.xincoCoreData.designation, x.xincoCoreData.xincoCoreLanguage.id");
      } else {
        result =
            createdQuery(
                "SELECT x FROM XincoAddAttribute x WHERE x.xincoCoreData.designation LIKE '"
  ...
  public int write2DB() throws XincoException {
    ...
    try {
      if (getId() > 0) {
        ...
        xcd.setXincoCoreNode(
            (com.bluecubs.xinco.core.server.persistence.XincoCoreNode)
                namedQuery("XincoCoreNode.findById", parameters).get(0));
        parameters.clear();
        parameters.put("id", getXincoCoreDataType().getId());
        xcd.setXincoCoreDataType(
            (com.bluecubs.xinco.core.server.persistence.XincoCoreDataType)
                namedQuery("XincoCoreDataType.findById", parameters).get(0));
        parameters.clear();
        parameters.put("id", getXincoCoreLanguage().getId());
        xcd.setXincoCoreLanguage(
            (com.bluecubs.xinco.core.server.persistence.XincoCoreLanguage)
      ...
      for (com.bluecubs.xinco.server.service.XincoAddAttribute temp : getXincoAddAttributes()) {
        /**
         * Copy fields from XincoAddAttribute to XincoAddAttribute (Persistence). The delete and
         * create approach doesn't go well with persistence.
         */
        com.bluecubs.xinco.core.server.persistence.XincoAddAttribute xaa;
        // Try to retrieve from database
        xaa =
            new XincoAddAttributeJpaController(getEntityManagerFactory())
                .findXincoAddAttribute(new XincoAddAttributePK(getId(), temp.getAttributeId()));
        create = xaa == null;
        if (create) {
  ...
  public int deleteFromDB() throws XincoException {
    try {
      ...
      if (getXincoCoreDataType().getId() == 1) {
        try {
          (new File(getXincoCoreDataPath(CONFIG.fileRepositoryPath, getId(), "" + getId())))
              .delete();
        } catch (Exception dfe) {
            ...
            try {
              (new File(
                      getXincoCoreDataPath(
                          ...
                          getId(),
                          getId() + "-" + ((XincoCoreLog) getXincoCoreLogs().get(i)).getId())))
                  .delete();
      ...
      for (XincoAddAttribute attr : getXincoAddAttributes()) {
        new XincoAddAttributeJpaController(getEntityManagerFactory())
            .destroy(new XincoAddAttributePK(attr.getXincoCoreDataId(), attr.getAttributeId()));
      }
      // Delete related logs
      loadLogs();
