 ...
 * <p>Modifications:
 *
 * <p>Who? When? What? Javier A. Ortiz 09/20/2006 Related changes made to XincoCoreUser (new fields)
 * in the following methods: write2DB, XincoCoreUserServer, XincoCoreUserServer (x 2),
 * getXincoCoreUsers Javier A. Ortiz 11/06/2006 Moved the logic of locking an account due to login
 * attempts from the XincoAdminServlet Alexander Manes 11/12/2006 Moved the new user features to
 * core class Javier A. Ortiz 11/20/2006 Undo previous changes and corrected a bug that increased
 * twice the attempts in the DB when wrong password was used Javier A. Ortiz 01/08/2007
 * ************************************************************
 */
...
public final class XincoCoreUserServer extends XincoCoreUser {
  ...
  private void fillXincoCoreGroups() throws XincoException {
    ...
    try {
      result.forEach(
          (o) -> {
            XincoCoreGroupServer.getXincoCoreGroups()
                .add(
                    new XincoCoreGroupServer(
                        ((XincoCoreUserHasXincoCoreGroup) o).getXincoCoreGroup()));
          });
    } catch (Exception e) {
  ...
  private void writeXincoCoreGroups() throws XincoException {
    ...
    try {
      ...
      parameters.put("xincoCoreUserId", getId());
      result = namedQuery("XincoCoreUserHasXincoCoreGroup.findByXincoCoreUserId", parameters);
      XincoCoreUserHasXincoCoreGroupJpaController controller =
          new XincoCoreUserHasXincoCoreGroupJpaController(getEntityManagerFactory());
      try {
        for (Object o : result) {
          controller.destroy(
              ((XincoCoreUserHasXincoCoreGroup) o).getXincoCoreUserHasXincoCoreGroupPK());
        }
      ...
      for (i = 0; i < XincoCoreGroupServer.getXincoCoreGroups().size(); i++) {
        XincoCoreUserHasXincoCoreGroup uhg =
            new XincoCoreUserHasXincoCoreGroup(
                new XincoCoreUserHasXincoCoreGroupPK(
                    getId(),
                    ((XincoCoreGroupServer) XincoCoreGroupServer.getXincoCoreGroups().get(i))
                        .getId()),
                1);
        uhg.setXincoCoreUser(
            new XincoCoreUserJpaController(getEntityManagerFactory())
                .findXincoCoreUser(uhg.getXincoCoreUserHasXincoCoreGroupPK().getXincoCoreUserId()));
        uhg.setXincoCoreGroup(
            new XincoCoreGroupJpaController(getEntityManagerFactory())
                .findXincoCoreGroup(
                    uhg.getXincoCoreUserHasXincoCoreGroupPK().getXincoCoreGroupId()));
  ...
  public XincoCoreUserServer(String attrUN, String attrUPW) throws XincoException {
    try {
      result =
          createdQuery(
              "SELECT xcu FROM XincoCoreUser xcu WHERE xcu.username='"
                  + attrUN
                  + "' AND xcu.userpassword='"
                  + encrypt(attrUPW)
   ...
   *
   * @param attempts new value to set
   */
   ...
   *
   * @param lastModified new value to set
   */
  ...
  public int write2DB() throws XincoException {
    ...
    try {
      ...
      if (getAttempts() > getSetting("password.attempts").getIntValue() && getId() > 1) {
        setStatusNumber(2);
      }
          ...
          (o) -> {
            coreUsers.add(
                new XincoCoreUserServer(
                    (com.bluecubs.xinco.core.server.persistence.XincoCoreUser) o));
          });
    } catch (Exception e) {
      getLogger(XincoCoreUserServer.class.getName()).log(SEVERE, null, e);
      coreUsers.clear();
  ...
  public boolean isPasswordUsable(String newPass, boolean hash) {
    ...
    try {
      ...
      result =
          createdQuery(
              "Select x from XincoCoreUser x where x.id="
                  + getId()
                  + " and x.userpassword='"
  ...
  public static boolean validCredentials(String username, String password, boolean encrypt) {
    try {
      ...
      parameters.put(
          "userpassword", encrypt ? encrypt(password.replaceAll("'", "\\\\'")) : password);
      return !createdQuery(
              "SELECT x FROM XincoCoreUser x "
                  + "WHERE x.username = :username and x.userpassword = :userpassword",
              parameters)
          .isEmpty();
