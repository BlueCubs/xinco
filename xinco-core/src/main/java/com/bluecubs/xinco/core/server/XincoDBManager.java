...
public class XincoDBManager {
  ...
  private static final ResourceBundle SETTINGS = getBundle("com.bluecubs.xinco.settings.settings");
  ...
  public static void waitForDB() {
    while (getState() != VALID && getState() != UPDATED && getState() != ERROR) {
      LOG.log(
          INFO,
    ...
    try {
      ...
      for (Field field : fields) {
        if (field.isAnnotationPresent(TableGenerator.class)) {
          ...
          if (namedQuery("XincoId.findByTablename", parameters).isEmpty()) {
              ...
              .forEach(
                  (next) -> {
                    LOG.log(
                        Level.CONFIG,
                        "{0}, {1}, {2}",
                        new Object[] {next.getId(), next.getTablename(), next.getLastId()});
                  });
        }
  ...
  public void setLoc(Locale loc) {
    try {
      ...
      LOG.log(FINE, null, e);
      lrb = getBundle("com.bluecubs.xinco.messages.XincoMessages", getDefault());
    }
  }
      ...
      try {
        if (ds != null) {
          ...
          if (!rs.next()) {
            ...
            LOG.log(INFO, "Amount of settings: {0}", metadata.getColumnCount());
          }
        } else {
  ...
  private static String getDBVersionNumber() {
    try {
      StringBuilder version = new StringBuilder();
      version.append(getSetting("version.high").getIntValue());
      version.append(".");
      version.append(getSetting("version.mid").getIntValue());
      version.append(".");
      version.append(getSetting("version.low").getIntValue());
      return version.toString();
    } catch (XincoException ex) {
      LOG.log(FINE, null, ex);
      return null;
  ...
  public static String getDBVersion() {
    try {
      return getDBVersionNumber()
          + (getSetting("version.postfix").getStringValue().isEmpty()
              ? ""
    ...
    if (emf == null) {
      try {
        ...
        if (isDemo()) {
          try {
            demoResetPeriod =
                (Long) (new InitialContext()).lookup("java:comp/env/xinco/demo-period");
          } catch (NamingException e) {
            LOG.log(SEVERE, null, e);
            demoResetPeriod = 0;
          }
          if (demoResetPeriod > 0) {
            LOG.log(
                WARNING,
                "Instance configured as demo, " + "database will reset" + " each {0} milliseconds",
                demoResetPeriod);
          }
        }
        emf = createEntityManagerFactory(CONFIG.getJNDIDB());
        LOG.log(INFO, "Using context defined database connection: {0}", CONFIG.getJNDIDB());
        usingContext = true;
  ...
  public static List<Object> protectedCreatedQuery(
      String query, HashMap<String, Object> parameters, boolean locked) throws XincoException {
    Query q;
  ...
  @SuppressWarnings("unchecked")
  protected static List<Object> protectedNamedQuery(
      String query, HashMap<String, Object> parameters, boolean locked) throws XincoException {
    Query q;
    getTransaction().begin();
    if (isLocked() && locked) {
  ...
  public static void nativeQuery(String query) throws XincoException {
    ...
    while (query.toLowerCase(getDefault()).contains("md5")) {
      ...
      toEncrypt = toEncrypt.substring(toEncrypt.indexOf('\'') + 1, toEncrypt.lastIndexOf('\''));
      LOG.log(FINE, "To encrypt: {0}", toEncrypt);
      query = query.substring(0, start) + "'" + encrypt(toEncrypt) + "'" + query.substring(end + 1);
      LOG.log(FINE, "Converted script: {0}", query);
    }
   ...
   *
   * <p>This style of implementation throws all exceptions to the caller.
   *
   ...
   */
  public static void setContents(File aFile, ArrayList<String> aContents)
