 ...
 * ************************************************************
 *
 * Name: XincoAddAttributeTJpaController
 *
...
public class XincoAddAttributeTJpaController implements Serializable {

  public XincoAddAttributeTJpaController(EntityManagerFactory emf) {
    this.emf = emf;
  }
  ...
  public void create(XincoAddAttributeT xincoAddAttributeT)
    ...
    try {
      ...
      if (findXincoAddAttributeT(xincoAddAttributeT.getRecordId()) != null) {
        throw new PreexistingEntityException(
            "XincoAddAttributeT " + xincoAddAttributeT + " already exists.", ex);
      }
      throw ex;
    } finally {
      if (em != null) {
  ...
  public void edit(XincoAddAttributeT xincoAddAttributeT)
      ...
      if (msg == null || msg.length() == 0) {
        ...
        if (findXincoAddAttributeT(id) == null) {
          throw new NonexistentEntityException(
              "The xincoAddAttributeT with id " + id + " no longer exists.");
        }
      }
      throw ex;
    } finally {
  ...
  public void destroy(Integer id) throws NonexistentEntityException {
    ...
    try {
      em = getEntityManager();
      em.getTransaction().begin();
      XincoAddAttributeT xincoAddAttributeT = em.find(XincoAddAttributeT.class, id);

      if (xincoAddAttributeT == null) {
        throw new NonexistentEntityException(
            "The xincoAddAttributeT with id " + id + " no longer exists.");
      }
  ...
  private List<XincoAddAttributeT> findXincoAddAttributeTEntities(
