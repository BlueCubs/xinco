...
public class XincoCoreAceTJpaController implements Serializable {
    ...
    try {
      ...
      if (findXincoCoreAceT(xincoCoreAceT.getRecordId()) != null) {
        throw new PreexistingEntityException(
            "XincoCoreAceT " + xincoCoreAceT + " already exists.", ex);
      }
      throw ex;
    } finally {
      if (em != null) {
      ...
      if (msg == null || msg.length() == 0) {
        ...
        if (findXincoCoreAceT(id) == null) {
          throw new NonexistentEntityException(
              "The xincoCoreAceT with id " + id + " no longer exists.");
        }
      }
      throw ex;
    } finally {
  ...
  public void destroy(Integer id) throws NonexistentEntityException {
    ...
    try {
      em = getEntityManager();
      em.getTransaction().begin();
      XincoCoreAceT xincoCoreAceT = em.find(XincoCoreAceT.class, id);

      if (xincoCoreAceT == null) {
        throw new NonexistentEntityException(
            "The xincoCoreAceT with id " + id + " no longer exists.");
      }
  ...
  private List<XincoCoreAceT> findXincoCoreAceTEntities(
