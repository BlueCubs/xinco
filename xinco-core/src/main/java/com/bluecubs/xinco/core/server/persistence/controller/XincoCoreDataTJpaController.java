...
public class XincoCoreDataTJpaController implements Serializable {
    ...
    try {
      ...
      if (findXincoCoreDataT(xincoCoreDataT.getRecordId()) != null) {
        throw new PreexistingEntityException(
            "XincoCoreDataT " + xincoCoreDataT + " already exists.", ex);
      }
      throw ex;
    } finally {
      if (em != null) {
      ...
      if (msg == null || msg.length() == 0) {
        ...
        if (findXincoCoreDataT(id) == null) {
          throw new NonexistentEntityException(
              "The xincoCoreDataT with id " + id + " no longer exists.");
        }
      }
      throw ex;
    } finally {
  ...
  public void destroy(Integer id) throws NonexistentEntityException {
    ...
    try {
      em = getEntityManager();
      em.getTransaction().begin();
      XincoCoreDataT xincoCoreDataT = em.find(XincoCoreDataT.class, id);

      if (xincoCoreDataT == null) {
        throw new NonexistentEntityException(
            "The xincoCoreDataT with id " + id + " no longer exists.");
      }
  ...
  private List<XincoCoreDataT> findXincoCoreDataTEntities(
