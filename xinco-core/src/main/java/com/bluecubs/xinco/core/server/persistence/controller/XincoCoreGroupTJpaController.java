...
public class XincoCoreGroupTJpaController implements Serializable {
    ...
    try {
      ...
      if (findXincoCoreGroupT(xincoCoreGroupT.getRecordId()) != null) {
        throw new PreexistingEntityException(
            "XincoCoreGroupT " + xincoCoreGroupT + " already exists.", ex);
      }
      throw ex;
    } finally {
      if (em != null) {
      ...
      if (msg == null || msg.length() == 0) {
        ...
        if (findXincoCoreGroupT(id) == null) {
          throw new NonexistentEntityException(
              "The xincoCoreGroupT with id " + id + " no longer exists.");
        }
      }
      throw ex;
    } finally {
  ...
  public void destroy(Integer id) throws NonexistentEntityException {
    ...
    try {
      em = getEntityManager();
      em.getTransaction().begin();
      XincoCoreGroupT xincoCoreGroupT = em.find(XincoCoreGroupT.class, id);

      if (xincoCoreGroupT == null) {
        throw new NonexistentEntityException(
            "The xincoCoreGroupT with id " + id + " no longer exists.");
      }
  ...
  private List<XincoCoreGroupT> findXincoCoreGroupTEntities(
