...
public class XincoCoreUserTJpaController implements Serializable {
    ...
    try {
      ...
      if (findXincoCoreUserT(xincoCoreUserT.getRecordId()) != null) {
        throw new PreexistingEntityException(
            "XincoCoreUserT " + xincoCoreUserT + " already exists.", ex);
      }
      throw ex;
    } finally {
      if (em != null) {
      ...
      if (msg == null || msg.length() == 0) {
        ...
        if (findXincoCoreUserT(id) == null) {
          throw new NonexistentEntityException(
              "The xincoCoreUserT with id " + id + " no longer exists.");
        }
      }
      throw ex;
    } finally {
  ...
  public void destroy(Integer id) throws NonexistentEntityException {
    ...
    try {
      em = getEntityManager();
      em.getTransaction().begin();
      XincoCoreUserT xincoCoreUserT = em.find(XincoCoreUserT.class, id);

      if (xincoCoreUserT == null) {
        throw new NonexistentEntityException(
            "The xincoCoreUserT with id " + id + " no longer exists.");
      }
  ...
  private List<XincoCoreUserT> findXincoCoreUserTEntities(
