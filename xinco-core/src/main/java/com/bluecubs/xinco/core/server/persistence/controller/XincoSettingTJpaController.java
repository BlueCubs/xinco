 ...
 * ************************************************************
 *
 * Name: XincoSettingTJpaController
 *
...
public class XincoSettingTJpaController implements Serializable {
    ...
    try {
      ...
      if (findXincoSettingT(xincoSettingT.getRecordId()) != null) {
        throw new PreexistingEntityException(
            "XincoSettingT " + xincoSettingT + " already exists.", ex);
      }
      throw ex;
    } finally {
      if (em != null) {
      ...
      if (msg == null || msg.length() == 0) {
        ...
        if (findXincoSettingT(id) == null) {
          throw new NonexistentEntityException(
              "The xincoSettingT with id " + id + " no longer exists.");
        }
      }
      throw ex;
    } finally {
  ...
  public void destroy(Integer id) throws NonexistentEntityException {
    ...
    try {
      em = getEntityManager();
      em.getTransaction().begin();
      XincoSettingT xincoSettingT = em.find(XincoSettingT.class, id);

      if (xincoSettingT == null) {
        throw new NonexistentEntityException(
            "The xincoSettingT with id " + id + " no longer exists.");
      }
  ...
  private List<XincoSettingT> findXincoSettingTEntities(
