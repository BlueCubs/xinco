...
public class Tool {
   ...
   * @return result
   */
  public static boolean compareNumberStrings(String first, String second, String separator) {
   ...
   *
   * <p>Return true only if aEmailAddress can successfully construct an {@link
   * javax.mail.internet.InternetAddress} when parsed with "@" as delimiter, aEmailAddress contains
   * two tokens which satisfy hirondelle.web4j.util.Util#textHasContent.
   *
   * <p>The second condition arises since local email addresses, simply of the form "albert", for
   * example, are valid for {@link javax.mail.internet.InternetAddress}, but almost always
   * undesired.
   *
  ...
  /**
   * Return <tt>true</tt> only if <tt>aText</tt> is not null, and is not empty after trimming.
   * (Trimming removes both leading/trailing whitespace and ASCII control characters.)
   *
   * <p>For checking argument validity, Args#checkForContent should be used instead of this method.
   *
  ...
  public static void addDefaultAddAttributes(com.bluecubs.xinco.server.service.XincoCoreData data) {
    ...
    LOG.log(FINE, "Added {0} attributes!", data.getXincoAddAttributes().size());
  }
}
