      ...
      XincoCoreDataHasDependencyServer instance =
          new XincoCoreDataHasDependencyServer(
              new XincoCoreDataJpaController(getEntityManagerFactory())
                  .findXincoCoreData(xincoCoreDataParentId),
              new XincoCoreDataJpaController(getEntityManagerFactory())
                  .findXincoCoreData(xincoCoreDataChildrenId),
              new XincoDependencyTypeJpaController(getEntityManagerFactory())
                  .findXincoDependencyType(dependencyTypeId));
      assertEquals(instance.write2DB(), 0);
      deleteFromDB(xincoCoreDataParentId, xincoCoreDataChildrenId, dependencyTypeId);
      parameters.clear();
